...
(function(){
  ...
  function buildBadge(user){
    ...
    if (user && profile){
      div.innerHTML = '<span class="dot"></span><a href="/u/'+escapeHTML(profile.username)+'">'+escapeHTML(profile.username)+'</a><button id="t3blo">logout</button>';
  ...
  function syncOnLogin(){
    ...
    sb.from('solves').select('*').eq('user_id', uid).order('solve_date', {ascending:true}).then(function(r){
      ...
      if (toUpload.length){
        sb.from('solves').insert(toUpload).then(function(){ console.log('[3DEN] migrated '+toUpload.length+' solves'); });
      }
  ...
  function mergeRemoteIntoLocal(remote){
    ...
    if (changed){
      Object.keys(local.puzzle).forEach(function(p){
        var sess = local.puzzle[p].sessions||{};
        Object.keys(sess).forEach(function(sk){ sess[sk].sort(function(a,b){ return (a.d||0)-(b.d||0); }); });
      });
      writeLocal(local);
  ...
  function handleRoute(){
    ...
    if (path === '/leaderboard' || path.indexOf('/leaderboard') === 0){
      renderLeaderboard();
    } else if (path.match(/^\/u\/[a-z0-9_-]+/)){
      var uname = path.split('/u/')[1].split('/')[0];
      renderProfile(uname);
    } else {
  ...
  function renderLeaderboard(){
    ...
    page.style.cssText = 'max-width:1200px;margin:0 auto;padding:40px 20px;color:#fff;min-height:100vh';
    page.innerHTML = ''
      ...
      sb.from(view).select('*').eq('puzzle', puzzle).order(col, {ascending:true}).limit(100).then(function(r){
        ...
        list.innerHTML = rows.map(function(row, i){
          ...
          return '<div class="t3lb-row">'
            ...
            + '  <a href="/u/'+escapeHTML(row.username)+'" class="t3lb-username">'+escapeHTML(row.username)+'</a>'
            + '</div>'
  ...
  function renderProfile(username){
    ...
    var page = document.createElement('div');
    page.id = 't3-profile';
    page.style.cssText = 'max-width:900px;margin:0 auto;padding:40px 20px;color:#fff;min-height:100vh';
    page.innerHTML = '<div style="text-align:center;color:#64748b;padding:80px">Loading profile...</div>';
    document.body.appendChild(page);
  ...
  function init(){
    ...
    // Detect route
    var path = location.pathname.replace(/^\/[a-z]{2}\//, '/');
    var isSubPage = path === '/leaderboard' || path.match(/^\/u\/[a-z0-9_-]+/);
