...
(function(){
  ...
  function loadCubingJs(cb){
    ...
    var s = document.createElement('script');
    s.type = 'module';
    s.textContent = "import 'https://cdn.jsdelivr.net/npm/cubing/dist/lib/cubing/twisty/index.js';\nwindow.__t3cubingLoaded=true;\nwindow.dispatchEvent(new Event('t3cubing-ready'));";
    document.head.appendChild(s);
  ...
  function makeTwisty(setup, opts){
    opts = opts || {};
    var p = document.createElement('twisty-player');
    ...
    p.setAttribute('visualization', opts.visualization || 'PG3D');
    p.style.cssText = 'width:'+(opts.size||100)+'px;height:'+(opts.size||100)+'px;display:block';
    if (opts.cameraLat != null) p.setAttribute('camera-latitude', opts.cameraLat);
    if (opts.cameraLng != null) p.setAttribute('camera-longitude', opts.cameraLng);
    return p;
  }
  ...
  function renderAlgsPLL(){
    ...
    var html = '<div style="max-width:1200px;margin:0 auto">'
      ...
      + PLL_CASES.map(function(c){
          return '<a href="/algs?set=pll&c='+c.id+'" class="t3-alg-card" data-id="'+c.id+'">'
            + '<div class="t3-cube" data-setup="'+escapeHTML(c.setup)+'" style="width:100px;height:100px;display:flex;align-items:center;justify-content:center;color:#64748b;font-size:11px">cube</div>'
    ...
    loadCubingJs(function(){
      page.querySelectorAll('.t3-cube').forEach(function(div){
        var setup = div.getAttribute('data-setup');
        var p = makeTwisty(setup, {size:100, cameraLat:60, cameraLng:30});
        div.innerHTML = '';
        div.appendChild(p);
      });
    });
  ...
  function renderAlgCase(caseSet, caseId){
    ...
    var html = '<div style="max-width:900px;margin:0 auto">'
      ...
      + '    <div class="t3-cube-small" data-setup="'+escapeHTML(prev.setup)+'" style="width:80px;height:80px;opacity:.5"></div>'
      + '  </a>'
      + '  <div class="t3-cube-main" data-setup="'+escapeHTML(c.setup)+'" style="width:200px;height:200px"></div>'
      + '  <a href="/algs?set='+caseSet+'&c='+next.id+'" style="color:#94a3b8;text-decoration:none;display:flex;align-items:center;gap:10px">'
      + '    <div class="t3-cube-small" data-setup="'+escapeHTML(next.setup)+'" style="width:80px;height:80px;opacity:.5"></div>'
      + '    <span style="font-size:24px">›</span>'
      + '  </a>'
      + '</div>'
    ...
    loadCubingJs(function(){
      ...
      if (main){
        var p = makeTwisty(main.getAttribute('data-setup'), {size:200, cameraLat:60, cameraLng:30});
        main.innerHTML = '';
        main.appendChild(p);
      }
      page.querySelectorAll('.t3-cube-small').forEach(function(div){
        var p = makeTwisty(div.getAttribute('data-setup'), {size:80, cameraLat:60, cameraLng:30});
        div.innerHTML = '';
        div.appendChild(p);
      });
    });
