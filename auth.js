...
(function(){
  ...
  function getSolveByNumber(solveNumber){
    // The list shows newest first usually, but row numbering is "1." for oldest in display
    // Looking at sample: "5.18.18" "4.—" "3.—" "2.— 16.72" → "5" is most recent
    // So solveNumber 5 = arr[arr.length-1] (last/newest)
    var puzzle = getActivePuzzle();
    var arr = getSolvesForPuzzle(puzzle);
    if (!arr.length) return null;
    // Solve number 1 = oldest, solve number N = newest? OR opposite?
    // Looking at PR row: "2.— 16.72" was PR. PR is best, not necessarily newest.
    // Most cubing apps: highest number = most recent.
    // Index = solveNumber - 1 (oldest first, so #1 = arr[0])
    // BUT user just made 17.05 which shows as #3 (and it's the newest)
    // Wait — looking: "3.— 17.05" was at top. So #N is most recent and arr[arr.length-1].
    // index = arr.length - solveNumber  (so #3 in 3-element list = arr[0]? no.)
    // Let's use heuristic: if solves count equals the number, take the last one
    var idx = arr.length - solveNumber;  // newest first
  ...
  function renderNoteIndicators(){
    ...
    rows.forEach(function(row){
      ...
      if (!numMatch) return;
      var solveNumber = parseInt(numMatch[1], 10);
      var idx = arr.length - solveNumber;
      var solve = (idx >= 0 && idx < arr.length) ? arr[idx] : null;
      var hasNote = solve && solve.note;
      // Tooltip: show note content if present, otherwise hint at double-click
      row.title = hasNote ? '📝 '+solve.note : 'Double-click to add a note';
      // Add/remove note dot
  ...
  function showPBsModal(){
    ...
    ov.innerHTML = '<div class="t3-auth-modal" style="max-width:780px;width:96vw;max-height:88vh;overflow-y:auto">'
      ...
      + '<h3 style="color:#fff;margin:0;font-size:22px;display:flex;align-items:center;gap:10px">🏆 My Personal Bests</h3>'
