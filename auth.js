...
(function(){
  ...
  function onGanNotification(event){
    ...
    if (parsed.state === 'handsOn'){
      ...
      if (prevState !== 'finished' && finalMs > 500){
        appendBleDebug('  💾 saving ' + seconds.toFixed(3) + 's');
        saveBleSolve(finalMs);
      } else if (finalMs <= 500){
        appendBleDebug('  ✗ skipping — both Gan-ST and live clock too short');
      }
  ...
  function saveBleSolve(timeMs){
    ...
    try {
      ...
      appendBleDebug('  ✅ saved (' + stored.sessions[activePuzzle].length + ' total in ' + activePuzzle + ') — reloading');

      // Odoo timer does not watch localStorage — reload to show the solve in the Time List.
