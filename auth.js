...
(function(){
  ...
  var css = ''
    ...
    + '.t3-auth-badge{position:fixed;top:14px;left:50%;transform:translateX(-50%);background:rgba(0,0,0,.5);backdrop-filter:blur(10px);border:1px solid rgba(255,255,255,.08);border-radius:999px;padding:10px 24px;display:flex;align-items:center;gap:14px;z-index:9000;font-size:17px;color:#e2e8f0;font-weight:700}'
    + '.t3-auth-badge .dot{width:10px;height:10px;border-radius:50%;background:#22c55e}'
    ...
    + '  .t3-auth-badge{left:50% !important;right:auto !important;transform:translateX(-50%) !important;padding:6px 14px;font-size:13px;max-width:200px;overflow:hidden}'
  ...
  function injectCSS(){
    ...
    st.id = 't3-auth-css';
    // Append global tweaks: hide bottom hints bar + celebration animations
    st.textContent = css + '.t3-hints{display:none !important}'
      + '@keyframes t3-celebrate-in{0%{transform:translate(-50%,-50%) scale(.3) rotate(-8deg);opacity:0}60%{transform:translate(-50%,-50%) scale(1.1) rotate(2deg);opacity:1}100%{transform:translate(-50%,-50%) scale(1) rotate(0);opacity:1}}'
      + '@keyframes t3-celebrate-out{from{opacity:1;transform:translate(-50%,-50%) scale(1)}to{opacity:0;transform:translate(-50%,-150%) scale(.8)}}'
      + '@keyframes t3-shake{0%,100%{transform:translateX(0)}10%,30%,50%,70%,90%{transform:translateX(-8px)}20%,40%,60%,80%{transform:translateX(8px)}}'
      + '@keyframes t3-pulse-glow{0%,100%{filter:drop-shadow(0 0 0 transparent)}50%{filter:drop-shadow(0 0 20px currentColor)}}'
      // Scramble font — Orbitron (futuristic, matches the 3DEN branding)
      + '@import url("https://fonts.googleapis.com/css2?family=Orbitron:wght@500;700&display=swap");'
      + '#t3-scramble{font-family:"Orbitron","JetBrains Mono",monospace !important;font-weight:600 !important;letter-spacing:3px !important}';
    document.head.appendChild(st);

    // @import must be first in a stylesheet to apply — inject the font link separately too.
    if (!document.getElementById('t3-font-orbitron')){
      var fl = document.createElement('link');
      fl.id = 't3-font-orbitron';
      fl.rel = 'stylesheet';
      fl.href = 'https://fonts.googleapis.com/css2?family=Orbitron:wght@500;600;700&display=swap';
  ...
  function transformTimerLayout(){
    ...
    // Wire up the native "Personal bests" trophy button (template doesn't bind it)
    var pbBtn = Array.from(document.querySelectorAll('button')).find(function(b){
      return b.title === 'Personal bests' && !b.__t3PbBound;
    });
    if (pbBtn){
      pbBtn.__t3PbBound = true;
      pbBtn.style.cursor = 'pointer';
      pbBtn.addEventListener('click', function(e){
        e.preventDefault(); e.stopPropagation();
        showPBsModal();
      });
    }

    // Wire up Quick Note on every row in Time List (delegated, observes future rows)
    setupQuickNotes();
    // Inject custom sessions (+ button) at bottom of puzzle sidebar
    injectCustomSessions();
    // Override Odoo's 4x4-7x7 scramble with WCA-compliant cstimer-style
    installScrambleWatcher();
    // NOTE: previously tried installCubingJsPreview() — cubing.js doesn't init reliably
    // in our environment (twisty-player registers but never builds shadow DOM).
    // Reverted to native Odoo preview for now.
  ...
  // Real flag image (emoji flags don't render on Windows — they show as "BE", "FR" etc).
  function countryFlag(code){
    if (!code) return '';
    var lc = String(code).toLowerCase();
    return '<img src="https://flagcdn.com/24x18/'+lc+'.png" width="21" height="15" '
      + 'style="display:inline-block;vertical-align:-2px;border-radius:2px;object-fit:cover" '
      + 'alt="'+escapeHTML(code)+'" loading="lazy" '
      + 'onerror="this.outerHTML=this.getAttribute(\'alt\')"/>';
  }
  // Emoji flag (for canvas / contexts that can't use <img>)
  ...
  function buildBadge(user){
    ['t3-auth-badge','t3-trophy','t3-sync','t3-feed-icon','t3-bell-icon','t3-daily-icon','t3-weekly-icon','t3-inbox-icon','t3-lib-icon','t3-help-icon','t3-ble-icon'].forEach(function(id){ var x=document.getElementById(id); if(x)x.remove(); });
    ...
    if (user){
      ...
      help.className = 't3-trophy';
      help.id = 't3-help-icon';
      help.title = 'Keyboard shortcuts (press ?)';
      help.style.right = '564px';
      help.style.color = '#94a3b8';
      help.style.cursor = 'pointer';
      help.style.fontSize = '20px';
      help.innerHTML = '?';
      help.onclick = buildShortcutsModal;
      ...
      streak.className = 't3-trophy';
      streak.id = 't3-streak-icon';
      streak.title = 'Daily streak — click to see calendar';
      streak.style.right = '214px';
      streak.style.cursor = 'pointer';
      streak.style.color = '#fb923c';
      streak.style.fontSize = '15px';
      streak.style.display = 'flex';
      streak.style.flexDirection = 'column';
      streak.style.alignItems = 'center';
      streak.style.justifyContent = 'center';
      streak.style.lineHeight = '1';
      streak.onclick = showStreakModal;
      document.body.appendChild(streak);
      updateStreakBadge();

      // Bluetooth Timer button
      buildBleButton();
    }
    // Trophy
    ...
    if (user && profile){
      var avatar = profile.avatar_type === 'image' && profile.avatar_url
        ? '<span style="width:26px;height:26px;border-radius:50%;background:url('+escapeHTML(profile.avatar_url)+') center/cover;display:inline-block"></span>'
        : '<span class="dot"></span>';
    ...
    // Collapse navigation icons into a single ☰ dropdown menu
    setTimeout(function(){ collapseNavIcons(user); }, 100);
  }

  /* ===== NAV MENU (collapse toolbar icons into ☰) ===== */
  function collapseNavIcons(user){
    if (!user) return;
    // The icons we collapse — id, emoji, label, action ('nav' href OR 'click' targetId)
    var navItems = [
      {id:'t3-trophy',      emoji:'🏆', label:'Leaderboard',         color:'#fbbf24', nav:'/leaderboard'},
      {id:'t3-feed-icon',   emoji:'📰', label:'Feed',                color:'#ef4444', nav:'/feed'},
      {id:'t3-daily-icon',  emoji:'⚡', label:'Daily Challenge',     color:'#fbbf24', nav:'/daily'},
      {id:'t3-weekly-icon', emoji:'🏆', label:'Weekly Competition',  color:'#a855f7', nav:'/weekly'},
      {id:'t3-lib-icon',    emoji:'📚', label:'Algorithms',          color:'#10b981', nav:'/algs?set=pll'},
      {id:'t3-bell-icon',   emoji:'🔔', label:'Notifications',       color:'#fbbf24', click:'t3-bell-icon'},
      {id:'t3-inbox-icon',  emoji:'💬', label:'Messages',            color:'#06b6d4', click:'t3-inbox-icon'},
      {id:'t3-help-icon',   emoji:'❓', label:'Keyboard Shortcuts',  color:'#94a3b8', click:'t3-help-icon'}
    ];
    // Hide the original icons (keep them in DOM for badge/polling logic)
    navItems.forEach(function(it){
      var el = document.getElementById(it.id);
      if (el) el.style.display = 'none';
    });
    // Create the ☰ hamburger button
    if (document.getElementById('t3-nav-menu-btn')) return;
    var btn = document.createElement('div');
    btn.id = 't3-nav-menu-btn';
    btn.className = 't3-trophy';
    btn.title = 'Menu — Feed, Daily, Weekly, Notifications…';
    btn.style.right = '114px';   // takes the place of the old first icon
    btn.style.color = '#cbd5e1';
    btn.style.cursor = 'pointer';
    btn.style.fontSize = '18px';
    btn.innerHTML = '☰';
    // Badge for unread (sum of bell + inbox)
    var badge = document.createElement('span');
    badge.id = 't3-nav-menu-badge';
    badge.style.cssText = 'position:absolute;top:-2px;right:-2px;background:#ef4444;color:#fff;font-size:10px;font-weight:700;width:18px;height:18px;border-radius:50%;display:none;align-items:center;justify-content:center;border:2px solid #0a0e1a;box-sizing:border-box;line-height:1';
    btn.appendChild(badge);
    btn.onclick = function(e){ e.stopPropagation(); toggleNavMenu(btn, navItems); };
    document.body.appendChild(btn);
    // Keep badge in sync with bell+inbox badges
    setInterval(updateNavMenuBadge, 1500);
    updateNavMenuBadge();
  }

  function updateNavMenuBadge(){
    var badge = document.getElementById('t3-nav-menu-badge');
    if (!badge) return;
    var total = 0;
    ['t3-bell-badge','t3-inbox-badge'].forEach(function(id){
  ...
  function toggleNavMenu(anchor, navItems){
    var existing = document.getElementById('t3-nav-menu');
    ...
    navItems.forEach(function(it){
      ...
      var cnt = 0;
      if (it.id === 't3-bell-icon'){ var bb = document.getElementById('t3-bell-badge'); if (bb && bb.style.display!=='none') cnt = parseInt(bb.textContent||'0',10)||0; }
      if (it.id === 't3-inbox-icon'){ var ib = document.getElementById('t3-inbox-badge'); if (ib && ib.style.display!=='none') cnt = parseInt(ib.textContent||'0',10)||0; }
      var i = document.createElement('button');
      i.style.cssText = 'display:flex !important;align-items:center;gap:10px;width:100%;background:transparent !important;border:0 !important;color:#e2e8f0 !important;padding:10px 12px;text-align:left;cursor:pointer;font-size:13px;border-radius:8px;transition:background .12s;font-family:inherit';
      i.onmouseenter = function(){ i.style.setProperty('background','rgba(255,255,255,.08)','important'); };
      i.onmouseleave = function(){ i.style.setProperty('background','transparent','important'); };
      i.innerHTML = '<span style="font-size:17px;width:24px;text-align:center;color:'+it.color+'">'+it.emoji+'</span>'
        + '<span style="flex:1;color:#e2e8f0 !important">'+it.label+'</span>'
        + (cnt > 0 ? '<span style="background:#ef4444;color:#fff;font-size:10px;font-weight:700;min-width:18px;height:18px;border-radius:9px;display:flex;align-items:center;justify-content:center;padding:0 4px">'+cnt+'</span>' : '');
      i.onclick = function(){
        menu.remove();
        if (it.nav){
          location.href = it.nav;
        } else if (it.click){
          var target = document.getElementById(it.click);
  ...
  function syncOnLogin(){
    ...
    sb.from('solves').select('*').eq('user_id', uid).order('solve_date', {ascending:true}).then(function(r){
      ...
      // Subscribe to realtime changes so solves from other devices appear live.
      subscribeRealtimeSolves(uid);
    });
    hookSave();
  }

  function subscribeRealtimeSolves(uid){
    if (window.__t3SolvesChannel) return;
    try {
      window.__t3SolvesChannel = sb.channel('solves-rt-' + uid)
        .on('postgres_changes',
          {event:'*', schema:'public', table:'solves', filter:'user_id=eq.' + uid},
          function(payload){
            var data = readLocal();
            if (!data.sessions) data.sessions = {};
            if (payload.eventType === 'INSERT' || payload.eventType === 'UPDATE'){
              var r = payload.new;
              if (!data.sessions[r.puzzle]) data.sessions[r.puzzle] = [];
              var arr = data.sessions[r.puzzle];
              var existing = arr.find(function(s){ return s.d == r.solve_date && Math.round(s.t) == Math.round(r.time_ms); });
              if (!existing){
                arr.push({t:r.time_ms, s:r.scramble, d:r.solve_date, pen:r.penalty||0});
                arr.sort(function(a,b){ return (a.d||0)-(b.d||0); });
                window.__t3SkipNextSync = true;
                writeLocal(data);
                // Re-render Time List if this puzzle is currently active
                try {
                  var sb2 = document.getElementById('t3-puzzles');
                  var active = sb2 && sb2.querySelector('button.active');
                  if (active) active.click();
                } catch(e){}
                console.log('[3DEN realtime] solve received from other device: '+r.puzzle+' '+r.time_ms+'ms');
              }
            } else if (payload.eventType === 'DELETE'){
              var oldR = payload.old;
              if (data.sessions[oldR.puzzle]){
                data.sessions[oldR.puzzle] = data.sessions[oldR.puzzle].filter(function(s){
                  return !(s.d == oldR.solve_date && Math.round(s.t) == Math.round(oldR.time_ms));
                });
                window.__t3SkipNextSync = true;
                writeLocal(data);
                try {
                  var sb3 = document.getElementById('t3-puzzles');
                  var active2 = sb3 && sb3.querySelector('button.active');
                  if (active2) active2.click();
                } catch(e){}
              }
            }
          })
        .subscribe(function(status){
          if (status === 'SUBSCRIBED') console.log('[3DEN] realtime sync active');
        });
    } catch(e){ console.warn('[3DEN] realtime subscribe failed', e); }
  }
      ...
      if (k === STORAGE_KEY && session){
        try {
          ...
          if (newSolves.length){
            ...
            // Check Supabase for existing solves with same (puzzle, solve_date, time_ms) to avoid duplicates
            var dates = rows.map(function(r){ return r.solve_date; });
            sb.from('solves').select('puzzle,time_ms,solve_date')
              .eq('user_id', session.user.id)
              .in('solve_date', dates)
              .then(function(existing){
                var existSet = {};
                (existing.data||[]).forEach(function(e){ existSet[e.puzzle+'|'+e.solve_date+'|'+e.time_ms] = 1; });
                var toInsert = rows.filter(function(r){ return !existSet[r.puzzle+'|'+r.solve_date+'|'+r.time_ms]; });
                if (!toInsert.length){ setSyncStatus('ok'); return; }
                sb.from('solves').insert(toInsert).then(function(rr){
  ...
  function diffSolves(prev, curr){
    ...
    Object.keys(sessions).forEach(function(p){
      ...
      arr.forEach(function(s){
        ...
        if (!f){
          out.push({puzzle:p, solve:s});
          // Detect PR/PB and celebrate (skip DNFs, only count valid solves)
          if ((s.pen||0) !== 2){
            checkAndCelebratePB(p, s, prevArr);
          } else {
            celebrateDNF();
          }
        }
    ...
    if (out.length){
      // Refresh streak badge after a new solve lands
  ...
  function setupQuickNotes(){
    if (window.__t3QuickNoteBound) return;
    window.__t3QuickNoteBound = true;

    // Single click on a solve row → open the pretty Solve detail modal (intercepts Odoo's prompt)
    document.addEventListener('click', function(e){
      var row = e.target.closest('.t3-times-row');
      if (!row) return;
      if (e.target.closest('button')) return;
      var list = row.parentElement;
      if (!list || !list.classList.contains('t3-times-list')) return;
      var numMatch = (row.textContent||'').match(/^(\d+)\./);
      if (!numMatch) return;
      // Stop Odoo's native click handler (which calls window.prompt)
      e.preventDefault();
      e.stopPropagation();
      if (e.stopImmediatePropagation) e.stopImmediatePropagation();
      var solveNumber = parseInt(numMatch[1], 10);
      openSolveModal(solveNumber);
    }, true);  // capture phase = run before Odoo's bubble-phase listener

    // Keyboard shortcuts:
    //   Q → Quick Note on most recent solve
    //   P → toggle +2 penalty on the last solve (clicks native button)
    document.addEventListener('keydown', function(e){
      var t = e.target;
      if (t && (t.tagName === 'INPUT' || t.tagName === 'TEXTAREA' || t.isContentEditable)) return;
      if (document.querySelector('.t3-auth-overlay')) return;
      if (e.ctrlKey || e.metaKey || e.altKey) return;
      if (e.key === 'q' || e.key === 'Q'){
        var puzzle = getActivePuzzle();
        var arr = getSolvesForPuzzle(puzzle);
        if (!arr.length) return;
        e.preventDefault();
        openSolveModal(arr.length);
      } else if (e.key === 'p' || e.key === 'P'){
        var btn = document.getElementById('t3-pen-2');
        if (btn){
          e.preventDefault();
          btn.click();
        }
    ...
    // Render note indicators after each render
    renderNoteIndicators();
    // Also listen for storage updates / re-render via MutationObserver
    var listObserver = new MutationObserver(function(){
      renderNoteIndicators();
    });
    setInterval(function(){
      var list = document.querySelector('.t3-times-list');
      if (list && !list.__t3Observed){
        list.__t3Observed = true;
  ...
  // Interpolate two hex colors. k=0 → a, k=1 → b
  function mix(a, b, k){
    function p(h){ return [parseInt(h.slice(1,3),16), parseInt(h.slice(3,5),16), parseInt(h.slice(5,7),16)]; }
    var ra=p(a), rb=p(b);
    var r = Math.round(ra[0] + (rb[0]-ra[0])*k);
    var g = Math.round(ra[1] + (rb[1]-ra[1])*k);
    var bl = Math.round(ra[2] + (rb[2]-ra[2])*k);
    return 'rgb('+r+','+g+','+bl+')';
  }

  /* ===== CUSTOM SESSIONS ===== */
  // Stored in localStorage as 't3den_custom_sessions': [{id,name,base,created}]
  function readCustomSessions(){
    try { return JSON.parse(localStorage.getItem('t3den_custom_sessions')||'[]'); }
    catch(e){ return []; }
  }
  function writeCustomSessions(list){
    localStorage.setItem('t3den_custom_sessions', JSON.stringify(list));
  }
  function activeSessionKey(){ return localStorage.getItem('t3den_active_custom') || ''; }
  function setActiveSessionKey(key){
    if (key) localStorage.setItem('t3den_active_custom', key);
    else localStorage.removeItem('t3den_active_custom');
  }
  function customSessionKey(s){ return s.base + ':' + s.id; }

  function injectCustomSessions(){
    var sidebar = document.getElementById('t3-puzzles');
    if (!sidebar || !sidebar.classList.contains('t3-vertical')) return;
    var existing = document.getElementById('t3cs-wrap');
    if (existing) existing.remove();

    var sessions = readCustomSessions();
    var wrap = document.createElement('div');
    wrap.id = 't3cs-wrap';
    wrap.style.cssText = 'border-top:1px solid rgba(255,255,255,.06);margin-top:6px;padding-top:6px;display:flex;flex-direction:column;gap:2px';

    var active = activeSessionKey();
    sessions.forEach(function(s){
      var btn = document.createElement('button');
      btn.className = 't3cs-btn';
      btn.dataset.sessionId = s.id;
      btn.title = s.name + ' (based on ' + s.base + ')\nRight-click: rename / delete';
      btn.style.cssText = 'background:none;border:0;color:#cbd5e1;padding:5px 4px;font-size:11px;cursor:pointer;text-align:center;border-radius:4px;line-height:1.2';
      btn.innerHTML = '<span style="display:block;font-size:9px;opacity:.5">'+escapeHTML(s.base)+'</span>'
        + '<span style="display:block;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:62px;margin:0 auto">'+escapeHTML(s.name)+'</span>';
      if (active === customSessionKey(s)){
        btn.style.background = 'rgba(239,68,68,.15)';
        btn.style.color = '#ef4444';
      }
      btn.onclick = function(e){ e.preventDefault(); activateCustomSession(s); };
      btn.oncontextmenu = function(e){ e.preventDefault(); showSessionContextMenu(s, e.clientX, e.clientY); };
      wrap.appendChild(btn);
    });

    var addBtn = document.createElement('button');
    addBtn.id = 't3cs-add';
    addBtn.title = 'New custom session';
    addBtn.style.cssText = 'background:none;border:1px dashed rgba(255,255,255,.15);border-radius:6px;color:#64748b;padding:6px 4px;font-size:18px;cursor:pointer;margin-top:4px;line-height:1;transition:all .15s';
    addBtn.textContent = '+';
    addBtn.onmouseenter = function(){ addBtn.style.color='#cbd5e1'; addBtn.style.borderColor='rgba(255,255,255,.3)'; };
    addBtn.onmouseleave = function(){ addBtn.style.color='#64748b'; addBtn.style.borderColor='rgba(255,255,255,.15)'; };
    addBtn.onclick = function(e){ e.preventDefault(); openNewSessionModal(); };
    wrap.appendChild(addBtn);

    sidebar.appendChild(wrap);
  }

  function activateCustomSession(s){
    setActiveSessionKey(customSessionKey(s));
    var baseLabelMap = {
      '333':'3x3','222':'2x2','444':'4x4','555':'5x5','666':'6x6','777':'7x7',
      '333oh':'3x3 OH','333bld':'3BLD','333fm':'FMC',
      'pyram':'Pyraminx','skewb':'Skewb','clock':'Clock','minx':'Megaminx','sq1':'Square-1'
    };
    var label = baseLabelMap[s.base] || '3x3';
    var btns = document.querySelectorAll('#t3-puzzles button');
    btns.forEach(function(b){ if ((b.textContent||'').trim() === label) b.click(); });
    setTimeout(injectCustomSessions, 80);
  }
  ...
  function openNewSessionModal(){
    ...
    ov.className = 't3-auth-overlay';
    ov.id = 't3cs-modal';
    ov.innerHTML = '<div class="t3-auth-modal" style="max-width:440px">'
      + '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px">'
        + '<h3 style="color:#fff;margin:0;font-size:18px;display:flex;align-items:center;gap:8px">📁 New Custom Session</h3>'
        + '<button id="t3cs-x" style="background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.08);color:#cbd5e1;width:30px;height:30px;border-radius:7px;cursor:pointer;font-size:13px">✕</button>'
      + '</div>'
      + '<div style="color:#64748b;font-size:12px;margin-bottom:14px">Name your session and pick a base puzzle.</div>'
      + '<label style="display:block;color:#cbd5e1;font-size:12px;margin-bottom:6px;font-weight:600">Session name</label>'
      + '<input id="t3cs-name" maxlength="20" placeholder="Morning warmup" style="width:100%;background:rgba(0,0,0,.3);border:1px solid rgba(255,255,255,.1);border-radius:8px;padding:10px;color:#fff;font-size:14px;box-sizing:border-box;margin-bottom:14px"/>'
      + '<label style="display:block;color:#cbd5e1;font-size:12px;margin-bottom:6px;font-weight:600">Base puzzle</label>'
      + '<select id="t3cs-base" style="width:100%;background:rgba(0,0,0,.3);border:1px solid rgba(255,255,255,.1);border-radius:8px;padding:10px;color:#fff;font-size:14px;box-sizing:border-box">'
      ...
      + '<div style="display:flex;justify-content:flex-end;gap:8px;margin-top:18px">'
        + '<button id="t3cs-cancel" style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);color:#cbd5e1;padding:8px 16px;border-radius:8px;cursor:pointer;font-size:13px">Cancel</button>'
        + '<button id="t3cs-create" style="background:#fbbf24;border:0;color:#0a0e1a;padding:8px 18px;border-radius:8px;cursor:pointer;font-size:13px;font-weight:700">Create</button>'
      + '</div>'
    + '</div>';
    document.body.appendChild(ov);
    var nameInput = ov.querySelector('#t3cs-name');
    setTimeout(function(){ nameInput.focus(); }, 50);
    function close(){ ov.remove(); }
    function create(){
      var name = nameInput.value.trim();
      if (!name) return alert('Name is required');
      var base = ov.querySelector('#t3cs-base').value;
      var sessions = readCustomSessions();
      sessions.push({id:'sess_'+Date.now().toString(36), name:name, base:base, created:Date.now()});
      writeCustomSessions(sessions);
      close();
      injectCustomSessions();
    }
    ov.querySelector('#t3cs-x').onclick = close;
    ov.querySelector('#t3cs-cancel').onclick = close;
    ov.querySelector('#t3cs-create').onclick = create;
    ov.onclick = function(e){ if (e.target === ov) close(); };
    document.addEventListener('keydown', function esc(e){
      if (e.key === 'Escape'){ close(); document.removeEventListener('keydown', esc); }
      else if (e.key === 'Enter' && document.activeElement === nameInput){ create(); document.removeEventListener('keydown', esc); }
    });
  }

  function showSessionContextMenu(s, x, y){
    var old = document.getElementById('t3cs-ctx'); if (old) old.remove();
    var menu = document.createElement('div');
    menu.id = 't3cs-ctx';
    menu.style.cssText = 'position:fixed;top:'+y+'px;left:'+x+'px;z-index:99999;background:#161a2e;border:1px solid rgba(255,255,255,.1);border-radius:8px;padding:4px;min-width:140px;box-shadow:0 10px 30px rgba(0,0,0,.6)';
    menu.innerHTML = '<button id="t3cs-rename" style="display:block;width:100%;background:none;border:0;color:#cbd5e1;padding:8px 12px;text-align:left;cursor:pointer;font-size:13px;border-radius:6px">✏ Rename</button>'
      + '<button id="t3cs-delete" style="display:block;width:100%;background:none;border:0;color:#fca5a5;padding:8px 12px;text-align:left;cursor:pointer;font-size:13px;border-radius:6px">🗑 Delete</button>';
    document.body.appendChild(menu);
    function close(){ menu.remove(); document.removeEventListener('click', closeOnOutside); }
    function closeOnOutside(e){ if (!menu.contains(e.target)) close(); }
    setTimeout(function(){ document.addEventListener('click', closeOnOutside); }, 50);
    menu.querySelector('#t3cs-rename').onclick = function(){
      var newName = prompt('Rename session:', s.name);
      if (newName && newName.trim()){
        var list = readCustomSessions();
        var it = list.find(function(x){ return x.id === s.id; });
        if (it){ it.name = newName.trim().substring(0,20); writeCustomSessions(list); injectCustomSessions(); }
      }
      close();
    };
    menu.querySelector('#t3cs-delete').onclick = function(){
      if (confirm('Delete session "'+s.name+'"?')){
        var list = readCustomSessions().filter(function(x){ return x.id !== s.id; });
        writeCustomSessions(list);
        if (activeSessionKey() === customSessionKey(s)) setActiveSessionKey('');
        injectCustomSessions();
      }
      close();
    };
  ...
  /* ===== UNFOLDED NET PREVIEW (Pyraminx / Skewb) ===== */
  // Build SVG showing all faces of pyram / skewb in cstimer-style net layout.
  // Uses solved colors (no scramble simulation yet) — gives Eden full puzzle view.

  function buildPyraminxNet(){
    // Pyraminx has 4 triangular faces, each subdivided into 9 small triangles.
    // Layout: F (front) in center, U (up) above, L (left) and R (right) flanking.
    // Triangles are pointing up (▲) or down (▽). Each face has 4 up-triangles and 5 down-triangles.
    // We render in a single SVG. viewBox 220x200.
    var colors = { F:'#ef4444', U:'#fbbf24', L:'#10b981', R:'#3b82f6' };  // red, yellow, green, blue
    function face(cx, cy, size, color, flipped){
      // cx,cy = center of face. size = side length of face triangle.
      // 9 sub-triangles arranged in 3 rows.
      // For an upward face: top corner at (cx, cy-h*2/3), bottom at (cx-size/2, cy+h/3) and (cx+size/2, cy+h/3)
      var h = size * Math.sqrt(3)/2;
      var tris = [];
      // Big triangle 9 sub-tris. Generate by row:
      // row 0 (top): 1 up
      // row 1 (mid): 1 up + 2 down (left and right of center up)? — actually: pyraminx face = 9 sub-triangles in 3 rows of 1,3,5.
      // Standard: rows 1,3,5 sub-tris.
      // Each row r (0=top) has 2r+1 sub-tris alternating up/down.
      for (var r = 0; r < 3; r++){
        for (var c = 0; c < 2*r + 1; c++){
          var up = (c % 2 === 0);  // even-indexed are up-pointing
          var subSize = size / 3;
          var subH = subSize * Math.sqrt(3)/2;
          // x of left edge of this column position
          var rowLeftX = cx - (r * subSize) / 1 - subSize/2 + subSize/2;
          // Position: the sub-tri at column c, row r
          // top-left of row r starts at cx - r*subSize/2 - subSize/2
          var startX = cx - r * (subSize/2) - subSize/2;
          var x0 = startX + (c * subSize/2);
          var rowTopY = cy - h/2 + r * subH;
          var p;
          if (up){
            p = [
              [x0 + subSize/2, rowTopY],
              [x0,             rowTopY + subH],
              [x0 + subSize,   rowTopY + subH]
            ];
          } else {
            p = [
              [x0,             rowTopY],
              [x0 + subSize,   rowTopY],
              [x0 + subSize/2, rowTopY + subH]
            ];
          }
          if (flipped){
            // flip vertically
            p = p.map(function(pt){ return [pt[0], 2*cy - pt[1]]; });
          }
          tris.push(p);
        }
      }
      return tris.map(function(t){
        return '<polygon points="'+t.map(function(p){return p[0].toFixed(1)+','+p[1].toFixed(1);}).join(' ')+'" fill="'+color+'" stroke="#0a0e1a" stroke-width="0.7"/>';
      }).join('');
    }

    var size = 60;
    var h = size * Math.sqrt(3) / 2;
    // Layout (cstimer-style cross):
    //         [U]
    //   [L]  [F]  [R]
    // U is flipped (upside down) to match net unfolding from F
    var W = 4 * size + 10, H = 2 * h + 20;
    var svg = '<svg viewBox="0 0 '+W+' '+H+'" width="100%" height="100%" preserveAspectRatio="xMidYMid meet" style="max-width:160px">';
    // F = center
    svg += face(W/2, 10+h*1.5, size, colors.F, false);
    // U = above F, flipped
    svg += face(W/2, 10+h*0.5, size, colors.U, true);
    // L = left of F (rotated 60° not implemented; show as flipped triangle at left)
    svg += face(W/2 - size, 10+h*1.5+h/2-h/2, size, colors.L, true);
    // R = right of F
    svg += face(W/2 + size, 10+h*1.5+h/2-h/2, size, colors.R, true);
    svg += '</svg>';
    return svg;
  }

  function buildSkewbNet(){
    // Skewb has 6 square faces, each split into 5 stickers: 1 center + 4 corner-triangles.
    // Layout: classic 4-up cross net:
    //         [U]
    //   [L]  [F]  [R]  [B]
    //         [D]
    var colors = { U:'#fff', L:'#fb923c', F:'#10b981', R:'#ef4444', B:'#3b82f6', D:'#fbbf24' };
    var s = 36;  // face size in svg units
    function face(x, y, color){
      // 1 center square + 4 corner triangles (skewb face)
      var stroke = 'stroke="#0a0e1a" stroke-width="0.7"';
      var midPad = s * 0.18;
      var html = '';
      // 4 corner triangles (use same color since solved)
      html += '<polygon points="'+x+','+y+' '+(x+s/2)+','+y+' '+(x+s/2)+','+(y+s/2)+'" fill="'+color+'" '+stroke+'/>';
      html += '<polygon points="'+(x+s)+','+y+' '+(x+s/2)+','+y+' '+(x+s/2)+','+(y+s/2)+'" fill="'+color+'" '+stroke+'/>';
      html += '<polygon points="'+x+','+(y+s)+' '+(x+s/2)+','+(y+s)+' '+(x+s/2)+','+(y+s/2)+'" fill="'+color+'" '+stroke+'/>';
      html += '<polygon points="'+(x+s)+','+(y+s)+' '+(x+s/2)+','+(y+s)+' '+(x+s/2)+','+(y+s/2)+'" fill="'+color+'" '+stroke+'/>';
      // Outer border
      html += '<rect x="'+x+'" y="'+y+'" width="'+s+'" height="'+s+'" fill="none" '+stroke+'/>';
      return html;
    }
    var W = 4*s + 8, H = 3*s + 8;
    var svg = '<svg viewBox="0 0 '+W+' '+H+'" width="100%" height="100%" preserveAspectRatio="xMidYMid meet" style="max-width:170px">';
    svg += face(s+4, 4,        colors.U);
    svg += face(4,   s+4+4,    colors.L);
    svg += face(s+4, s+4+4,    colors.F);
    svg += face(2*s+4, s+4+4,  colors.R);
    svg += face(3*s+4, s+4+4,  colors.B);
    svg += face(s+4, 2*s+8+4,  colors.D);
    svg += '</svg>';
    return svg;
  }

  function installPreviewOverrides(){
    if (window.__t3PreviewOverride) return;
    window.__t3PreviewOverride = true;
    var prev = document.getElementById('t3-preview-wrap');
    if (!prev) return;
    ...
    function maybeReplace(){
      if (replacing) return;
      var puzzle = getActivePuzzle();
      if (puzzle !== 'pyram' && puzzle !== 'skewb') return;
      var marker = prev.dataset.t3Net;
      if (marker === puzzle) return;  // already replaced for this puzzle
      replacing = true;
      try {
        if (mo) mo.disconnect();
        if (puzzle === 'pyram') prev.innerHTML = buildPyraminxNet();
        else if (puzzle === 'skewb') prev.innerHTML = buildSkewbNet();
        prev.dataset.t3Net = puzzle;
      } finally {
        if (mo) mo.observe(prev, {childList:true, subtree:true});
        setTimeout(function(){ replacing = false; }, 50);
      }
    }
    mo = new MutationObserver(function(){
      if (replacing) return;
      // If Odoo re-rendered, our marker is gone, so retry
      var puzzle = getActivePuzzle();
      if ((puzzle === 'pyram' || puzzle === 'skewb') && prev.dataset.t3Net !== puzzle){
        setTimeout(maybeReplace, 30);
      }
    });
    mo.observe(prev, {childList:true, subtree:true});
    setTimeout(maybeReplace, 300);
    // Also clear marker when switching puzzles
    document.addEventListener('click', function(e){
      if (e.target.closest('#t3-puzzles')){
        setTimeout(function(){ delete prev.dataset.t3Net; maybeReplace(); }, 100);
      }
    });
  }

  /* ===== WCA-COMPLIANT SCRAMBLE GENERATOR (big cubes) ===== */
  // For 4x4/5x5/6x6/7x7, WCA notation uses: L R U D F B + their 2-versions,
  // plus wide-turns Fw Uw Rw (only one wide variant per face).
  // cstimer-style minimal wide moves.
  var SCRAMBLE_LENGTHS = { '444':46, '555':60, '666':80, '777':100 };
  var SCRAMBLE_FACES = ['U','D','L','R','F','B'];
  var SCRAMBLE_OPPOSITE = {U:'D', D:'U', L:'R', R:'L', F:'B', B:'F'};

  function generateWCAScramble(puzzle){
    var len = SCRAMBLE_LENGTHS[puzzle];
    if (!len) return null;
    // Wide-turns probability per cube size
    var wideRatio = {'444':0.30, '555':0.30, '666':0.40, '777':0.45}[puzzle] || 0.30;
    // For 6x6/7x7, also allow 3-wide (3Fw)
    var allowThreeWide = (puzzle === '666' || puzzle === '777');

    var moves = [];
    var lastFace = '';
    var lastAxisFaces = [];  // track opposite-face pair to avoid 3 same-axis in a row

    for (var i = 0; i < len; i++){
      var face;
      // Pick a face not equal to lastFace and not creating 3-in-a-row on same axis
      var tries = 0;
      do {
        face = SCRAMBLE_FACES[Math.floor(Math.random()*6)];
        tries++;
      } while ((face === lastFace || (lastAxisFaces.length === 1 && lastAxisFaces[0] === SCRAMBLE_OPPOSITE[face] && lastFace === SCRAMBLE_OPPOSITE[face])) && tries < 20);

      // Wide or outer?
      var wide = '';
      if (Math.random() < wideRatio){
        if (allowThreeWide && Math.random() < 0.25) wide = '3'+'w';
        else wide = 'w';
      }
      // Suffix: '', '2', "'"
      var sfx = ['', '2', "'"][Math.floor(Math.random()*3)];
      var token = (wide.indexOf('3') >= 0 ? '3' : '') + face + (wide.indexOf('w') >= 0 ? 'w' : '') + sfx;
      moves.push(token);

      // Track axis
      if (SCRAMBLE_OPPOSITE[face] === lastFace){
        lastAxisFaces = [lastFace, face];
      } else {
        lastAxisFaces = [face];
      }
      lastFace = face;
  ...
  // Watch the scramble element and overwrite the Odoo-generated one with our WCA-style scramble.
  // CRITICAL: pause the observer while we write, otherwise we trigger an infinite loop.
  function installScrambleWatcher(){
    if (window.__t3ScrambleWatcher) return;
    window.__t3ScrambleWatcher = true;
    var scrEl = document.getElementById('t3-scramble');
    if (!scrEl) return;
    ...
    function maybeReplace(){
      if (replacing) return;
      var puzzle = getActivePuzzle();
      if (!SCRAMBLE_LENGTHS[puzzle]) return;  // only 4x4-7x7
      var current = (scrEl.textContent||'').trim();
      if (!current) return;
      // Skip if our generator already wrote this (marked attribute)
      if (scrEl.dataset.t3Wca === current) return;
      var fresh = generateWCAScramble(puzzle);
      if (!fresh) return;
      replacing = true;
      try {
        if (mo) mo.disconnect();
        scrEl.textContent = fresh;
        scrEl.dataset.t3Wca = fresh;
      } finally {
        if (mo) mo.observe(scrEl, {childList:true, subtree:true, characterData:true});
        // small delay before re-arming to swallow same-tick callbacks
        setTimeout(function(){ replacing = false; }, 50);
      }
    }

    mo = new MutationObserver(function(){
      // Debounce — only run on next tick to avoid recursion
      if (replacing) return;
      setTimeout(maybeReplace, 30);
    });
    mo.observe(scrEl, {childList:true, subtree:true, characterData:true});
    setTimeout(maybeReplace, 200);
  }

  /* ===== CUBING.JS PUZZLE PREVIEW =====
   * Uses the official cubing.js library (used by WCA + cstimer + J Perm).
   * Loads <twisty-player> custom element from jsDelivr CDN once.
   * Then for puzzles cubing.js supports better than Odoo (pyram, skewb, minx, sq1, 4-7x7),
   * we replace #t3-preview-wrap content with a twisty-player showing the scramble.
   */
  var CUBING_JS_URL = 'https://cdn.cubing.net/v0/js/cubing/twisty';  // ES module CDN
  // Puzzle keys we let cubing.js handle (others stay native Odoo render)
  var CUBINGJS_PUZZLES = {
    'pyram':  '3x3x3:Pyraminx',  // mapping done below
    'skewb':  'skewb',
    'minx':   'megaminx',
    'sq1':    'square1',
    '444':    '4x4x4',
  ...
  // cubing.js puzzle ID per our puzzle key
  var CUBINGJS_ID = {
    pyram: 'pyraminx', skewb: 'skewb', minx: 'megaminx', sq1: 'square1',
    '333':'3x3x3','222':'2x2x2','444':'4x4x4','555':'5x5x5','666':'6x6x6','777':'7x7x7',
    clock:'clock'
  };

  function loadCubingJs(callback){
    if (window.__cubingJsLoaded){ callback(); return; }
    if (window.__cubingJsLoading){
      window.__cubingJsLoadCbs = window.__cubingJsLoadCbs || [];
      window.__cubingJsLoadCbs.push(callback);
      return;
    }
    window.__cubingJsLoading = true;
    window.__cubingJsLoadCbs = [callback];
    var script = document.createElement('script');
    script.type = 'module';
    // Importing as a module registers the <twisty-player> custom element globally
    script.textContent = 'import "https://cdn.cubing.net/v0/js/cubing/twisty"; window.__cubingJsLoaded = true; (window.__cubingJsLoadCbs||[]).forEach(function(cb){ try { cb(); } catch(e){} }); window.__cubingJsLoadCbs = [];';
    document.head.appendChild(script);
    // Fallback: poll for customElements.get('twisty-player')
    var t0 = Date.now();
    var poll = setInterval(function(){
      if (customElements.get('twisty-player')){
        window.__cubingJsLoaded = true;
        clearInterval(poll);
        (window.__cubingJsLoadCbs || []).forEach(function(cb){ try { cb(); } catch(e){} });
        window.__cubingJsLoadCbs = [];
      } else if (Date.now() - t0 > 15000){
        clearInterval(poll);
        console.warn('[3DEN] cubing.js failed to load within 15s');
      }
    }, 200);
  }

  function installCubingJsPreview(){
    if (window.__t3CubingPreview) return;
    window.__t3CubingPreview = true;
    var prev = document.getElementById('t3-preview-wrap');
    var scrEl = document.getElementById('t3-scramble');
    if (!prev || !scrEl){
      window.__t3CubingPreview = false;
      setTimeout(installCubingJsPreview, 500);
      return;
    }
    ...
    function shouldHandle(puzzle){
      return !!CUBINGJS_ID[puzzle] && puzzle !== '333' && puzzle !== '222';  // let Odoo do 3x3/2x2 (they're fine)
    }

    function render(){
      if (replacing) return;
      var puzzle = getActivePuzzle();
      if (!shouldHandle(puzzle)){
        delete prev.dataset.t3Cubingjs;
        return;
      }
      var scramble = (scrEl.textContent||'').trim();
      var key = puzzle + '|' + scramble;
      if (lastRenderKey === key) return;
      lastRenderKey = key;
      loadCubingJs(function(){
        replacing = true;
        try {
          if (moPrev) moPrev.disconnect();
          var puzzleId = CUBINGJS_ID[puzzle];
          // Build a twisty-player in 2D view (top-down "PG3D" or "2D"), small size
          prev.innerHTML = '';
          var tp = document.createElement('twisty-player');
          tp.setAttribute('puzzle', puzzleId);
          tp.setAttribute('alg', scramble);
          tp.setAttribute('experimental-setup-anchor', 'end');  // show end-state of scramble
          tp.setAttribute('visualization', '2D');               // flat 2D unfolded
          tp.setAttribute('background', 'none');
          tp.setAttribute('control-panel', 'none');
          tp.setAttribute('hint-facelets', 'none');
          tp.style.width = '100%';
          tp.style.height = '100%';
          tp.style.minHeight = '110px';
          tp.style.display = 'block';
          prev.appendChild(tp);
          prev.dataset.t3Cubingjs = key;
        } catch(e){
          console.warn('[3DEN] cubing.js render error', e);
        } finally {
          if (moPrev) moPrev.observe(prev, {childList:true, subtree:true});
          setTimeout(function(){ replacing = false; }, 100);
        }
      });
    }

    moPrev = new MutationObserver(function(){
      if (replacing) return;
      var puzzle = getActivePuzzle();
      if (shouldHandle(puzzle) && prev.dataset.t3Cubingjs !== (puzzle+'|'+(scrEl.textContent||'').trim())){
        setTimeout(render, 50);
      }
    });
    moPrev.observe(prev, {childList:true, subtree:true});

    moScr = new MutationObserver(function(){
      lastRenderKey = '';
      setTimeout(render, 40);
    });
    moScr.observe(scrEl, {childList:true, subtree:true, characterData:true});
  ...
  /* ===== PYRAMINX SCRAMBLE SOLVER + RENDER (legacy, unused) =====
   * Standard WCA hold: GREEN front, YELLOW down, BLUE right, RED left.
   * Faces indexed F=0 (green), L=1 (red), R=2 (blue), D=3 (yellow).
   * Each face has 9 stickers indexed 0-8 in row-major order:
   *     0
   *   1 2 3
   *  4 5 6 7 8
   * Moves (WCA):
   *   U (upper tip + adjacent layer rotation CW around top vertex)
   *   L (left tip + adjacent layer around left vertex)
   *   R (right tip + ... right vertex)
   *   B (back tip + ... back vertex)
   *   Lowercase l, r, u, b = tip-only rotation
   *   ' = inverse, no suffix = CW, suffix omitted = single
   */
  function pyraSolved(){
    // Per face: stickers[0..8] starting from top vertex of that face.
    return {
      F: ['F','F','F','F','F','F','F','F','F'],
      L: ['L','L','L','L','L','L','L','L','L'],
      R: ['R','R','R','R','R','R','R','R','R'],
  ...
  // Color map (hex) for each face label
  var PYRA_COLORS = { F:'#10b981', L:'#ef4444', R:'#3b82f6', D:'#fbbf24' };

  // Define cycles for each large move (whole half affected): index list per face.
  // A move U (clockwise around UP vertex) rotates:
  //   F top-half (stickers 0,1,2,3) → cycles with corresponding stickers on L and R top halves
  //   The tip (sticker 0) cycles with L's top sticker and R's top sticker
  // We define cycles as ordered arrays: a→b→c→a (CW = U)
  // Note: this is a faithful Pyraminx model. We'll define cycles for each move and apply.
  // Big move = tip + adjacent layer. Each big move = 3-cycle of 3 corner stickers + 3-cycle of 3 edge sticker triplets.
  var PYRA_MOVES = {
    // U (upper vertex CW = "U") — affects F, L, R top halves (4 stickers each)
    U: [
      // Three triangle-tip stickers cycle (top corners of F, L, R)
      [['F',0],['L',0],['R',0]],
      // Three "edge" trios that rotate together
      [['F',1],['L',3],['R',1]],
      [['F',2],['L',2],['R',2]],
      [['F',3],['L',1],['R',3]]
    ],
    // L (left vertex CW = "L") — affects F bottom-left, L bottom-right, D bottom-left
    L: [
      [['F',4],['L',8],['D',0]],
      [['F',1],['L',3],['D',1]],
      [['F',5],['L',7],['D',4]],
    ...
    // R (right vertex CW = "R") — affects F bottom-right, R bottom-left, D bottom-right
    R: [
      [['F',8],['R',4],['D',8]],
      [['F',3],['R',1],['D',3]],
      [['F',7],['R',5],['D',7]],
    ...
    // B (back vertex CW = "B") — affects L bottom-left, R bottom-right, D top
    B: [
      [['L',4],['R',8],['D',0]],   // tip
      [['L',1],['R',3],['D',1]],
      [['L',5],['R',7],['D',2]],
    ...
    // Tips only (lowercase) — single corner sticker rotation
    u: [ [['F',0],['L',0],['R',0]] ],
    l: [ [['F',4],['L',8],['D',0]] ],
    r: [ [['F',8],['R',4],['D',8]] ],
    b: [ [['L',4],['R',8],['D',0]] ]
  ...
  function pyraApplyMove(state, moveName){
    var inverse = moveName.endsWith("'");
    var base = inverse ? moveName.slice(0, -1) : moveName;
    var cycles = PYRA_MOVES[base];
    if (!cycles) return;
    cycles.forEach(function(cycle){
      // CW: a→b, b→c, c→a   means new[a]=old[c]
      // CCW (inverse): a→c, c→b, b→a   means new[a]=old[b]
      var values = cycle.map(function(pos){ return state[pos[0]][pos[1]]; });
      cycle.forEach(function(pos, i){
        var src;
        if (inverse) src = values[(i + 1) % cycle.length];
        else src = values[(i - 1 + cycle.length) % cycle.length];
        state[pos[0]][pos[1]] = src;
      });
    });
  }

  function pyraApplyScramble(scramble){
    var state = pyraSolved();
    var tokens = scramble.split(/\s+/).filter(Boolean);
    tokens.forEach(function(tk){
      // Token can be U, U', L, L', etc. (uppercase) or u, u', etc. (tips)
      // Validate
      var clean = tk.replace(/[^ULRBulrb']/g, '');
      if (!/^[ULRBulrb]'?$/.test(clean)) return;
      pyraApplyMove(state, clean);
    });
    return state;
  }

  // Render front face of solved/scrambled state.
  // Layout (9 sub-triangles in pyraminx face):
  //     [0]
  //   [1][2][3]
  //  [4][5][6][7][8]
  // 0,2,5,7 = up-pointing triangles (4)
  // 1,3,4,6,8 = down-pointing (5)
  function buildPyraminxSVG(state){
    var F = state.F;
    var W = 80, H = 75;
    var s = 60;  // face side length
    var cx = W/2, cy = 5;  // top of triangle
    var h = s * Math.sqrt(3)/2;
    // Sub-triangle coordinates
    var subS = s/3;
    var subH = h/3;
    // Index positions (row,col) within face — same as state.F[i]
    // Row 0: 1 sticker at index 0 (up-triangle)
    // Row 1: 3 stickers at indices 1,2,3 (down, up, down)... wait actually pyraminx face layout:
    // Standard: row 0 has 1 up-tri (top tip). Row 1: down-up-down (3 sub-tris). Row 2: down-up-down-up-down (5 sub-tris).
    // But conventional sticker numbering for pyraminx 1+3+5 = top corner first.
    // We render in this order — index → position:
    //   0: top up-tri
    //   1: row 1 left up-tri, 2: row 1 down (in middle), 3: row 1 right up-tri
    // Actually let me redo: rows = 3, each row has (2r+1) sub-tris alternating up/down.
    // r=0: 1 up
    // r=1: down-up-down (indices 1,2,3)
    // r=2: down-up-down-up-down (4,5,6,7,8) — wait this gives 5 ups+downs sequence: actually start with down or up?
    // For pyraminx face: sub-tri at (row 0, col 0) is UP (pointing up like the face).
    // (row 1): 3 sub-tris alternating starting from DOWN-pointing: down, up, down? No — actually they alternate up/down based on column parity.
    // Use formula: at (r, c), up-pointing iff (c % 2 === 0) — this gave us cstimer-like pattern.

    var html = '<svg viewBox="0 0 '+W+' '+H+'" width="100%" height="100%" preserveAspectRatio="xMidYMid meet">';
    var idx = 0;
    for (var r = 0; r < 3; r++){
      var rowTop = cy + r * subH;
      var rowStartX = cx - (r+1) * subS/2 + subS/2;  // hmm
      // Actually rowStartX = cx - r * subS  (left edge of row of (2r+1) tris)
      // The first sub-tri at row r starts at x = cx - r * subS  (offset due to triangle shape)
      // But triangles share edges, so let's place all tris in a row at columns 0..(2r).
      // X position of column c left edge: cx - r*(subS/2) + c*(subS/2) ? No simpler:
      // The row's first sub-tri (column 0) starts at x = cx - (r+1)*subS/2 + subS/2... reset
      // Easiest: for col c at row r, base x for centroid = cx - r*subS/2 + c*subS/2 — but sub-tris have varied tip orientations
      // I'll explicitly compute each sub-triangle's vertices.
      for (var c = 0; c <= 2*r; c++){
        var up = (c % 2 === 0);  // even col = up
        var leftX = cx - r * (subS/2) + Math.floor(c/2) * subS;  // base x of this sub-tri's leftmost point
        // Sub-tri vertices
        var verts;
        if (up){
          var x0 = leftX;
          ...
          // Down-pointing triangle shares two top vertices with adjacent up-tris
          // It sits between two up-tris in same row
          var x0d = leftX + subS/2;
          var y0d = rowTop;
          verts = [
            [x0d, y0d],
            [x0d + subS, y0d],
            [x0d + subS/2, rowTop + subH]
          ];
        }
        var face = F[idx];
        var color = PYRA_COLORS[face] || '#777';
        html += '<polygon points="'+verts.map(function(v){return v[0].toFixed(1)+','+v[1].toFixed(1);}).join(' ')+'" fill="'+color+'" stroke="#0a0e1a" stroke-width="0.7"/>';
        idx++;
      }
    }
    html += '</svg>';
  ...
  function installPyraminxPreview(){
    if (window.__t3PyraPreview) return;
    window.__t3PyraPreview = true;
    var prev = document.getElementById('t3-preview-wrap');
    var scrEl = document.getElementById('t3-scramble');
    if (!prev || !scrEl) {
      // Try again later
      setTimeout(installPyraminxPreview, 500);
      window.__t3PyraPreview = false;
      return;
    }
    var replacing = false;
    var mo;
    ...
    function renderNow(){
      ...
      var puzzle = getActivePuzzle();
      if (puzzle !== 'pyram'){
        delete prev.dataset.t3Pyra;
        return;
      }
      var scramble = (scrEl.textContent||'').trim();
      var key = scramble || '_solved';
      if (prev.dataset.t3Pyra === key) return;
      var state = scramble ? pyraApplyScramble(scramble) : pyraSolved();
      replacing = true;
      try {
        if (mo) mo.disconnect();
        prev.innerHTML = buildPyraminxSVG(state);
        prev.dataset.t3Pyra = key;
      } finally {
        if (mo) mo.observe(prev, {childList:true, subtree:true});
        setTimeout(function(){ replacing = false; }, 50);
      }
    ...
    // Also watch scramble changes
    var moScr = new MutationObserver(function(){ setTimeout(renderNow, 30); });
    moScr.observe(scrEl, {childList:true, subtree:true, characterData:true});

    // And puzzle button clicks
    document.addEventListener('click', function(e){
      if (e.target.closest('#t3-puzzles')){
        delete prev.dataset.t3Pyra;
        setTimeout(renderNow, 150);
      }
    });
  ...
  /* ===== MANUAL TIME ENTRY (StackMat / external timer) ===== */
  function openManualEntryModal(){
    var existing = document.getElementById('t3me-overlay');
    if (existing){ existing.remove(); return; }

    // Read current scramble from Odoo
    var scrEl = document.getElementById('t3-scramble');
    var scramble = scrEl ? (scrEl.textContent || '').trim() : '';

    var ov = document.createElement('div');
    ov.id = 't3me-overlay';
    ov.className = 't3-auth-overlay';
    ov.innerHTML = '<div class="t3-auth-modal" style="max-width:440px;padding:24px">'
      + '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px">'
        + '<h3 style="color:#fff;margin:0;font-size:18px;display:flex;align-items:center;gap:8px">⌨ Manual Time Entry</h3>'
        + '<button id="t3me-x" style="background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.08);color:#cbd5e1;width:30px;height:30px;border-radius:7px;cursor:pointer;font-size:13px">✕</button>'
      + '</div>'
      + '<div style="color:#64748b;font-size:12px;margin-bottom:14px">For StackMat or external timer. Type the time and press Enter.</div>'
      + (scramble ? '<div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.06);border-radius:10px;padding:10px 12px;margin-bottom:14px;font-family:monospace;color:#cbd5e1;font-size:13px;text-align:center;line-height:1.5">'+escapeHTML(scramble)+'</div>' : '')
      + '<label style="display:block;color:#cbd5e1;font-size:12px;margin-bottom:6px;font-weight:600">Time</label>'
      + '<input id="t3me-input" inputmode="decimal" placeholder="e.g. 12.34 or 1:23.45" style="width:100%;background:rgba(0,0,0,.3);border:1px solid rgba(255,255,255,.1);border-radius:10px;padding:14px;color:#fff;font-size:24px;font-family:monospace;text-align:center;box-sizing:border-box;letter-spacing:1px"/>'
      + '<div id="t3me-preview" style="text-align:center;color:#fbbf24;font-size:14px;margin-top:8px;font-family:monospace;min-height:18px"></div>'
      + '<div style="display:flex;gap:6px;justify-content:center;margin-top:14px;flex-wrap:wrap">'
        + '<button class="t3me-pen" data-pen="0" style="background:rgba(34,197,94,.15);border:1px solid rgba(34,197,94,.3);color:#86efac;padding:6px 14px;border-radius:999px;cursor:pointer;font-size:12px;font-weight:600">OK</button>'
        + '<button class="t3me-pen" data-pen="1" style="background:rgba(251,191,36,.1);border:1px solid rgba(251,191,36,.2);color:#fbbf24;padding:6px 14px;border-radius:999px;cursor:pointer;font-size:12px;font-weight:600">+2</button>'
        + '<button class="t3me-pen" data-pen="2" style="background:rgba(239,68,68,.1);border:1px solid rgba(239,68,68,.2);color:#fca5a5;padding:6px 14px;border-radius:999px;cursor:pointer;font-size:12px;font-weight:600">DNF</button>'
      + '</div>'
      + '<div style="display:flex;justify-content:flex-end;gap:8px;margin-top:18px">'
        + '<button id="t3me-cancel" style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);color:#cbd5e1;padding:9px 18px;border-radius:8px;cursor:pointer;font-size:13px">Cancel</button>'
        + '<button id="t3me-save" style="background:#fbbf24;border:0;color:#0a0e1a;padding:9px 22px;border-radius:8px;cursor:pointer;font-size:13px;font-weight:700">Save (Enter)</button>'
      + '</div>'
    + '</div>';
    document.body.appendChild(ov);

    var input = ov.querySelector('#t3me-input');
    var preview = ov.querySelector('#t3me-preview');
    var penalty = 0;
    setTimeout(function(){ input.focus(); }, 50);

    // Highlight selected penalty button
    function refreshPenButtons(){
      ov.querySelectorAll('.t3me-pen').forEach(function(b){
        var active = parseInt(b.dataset.pen, 10) === penalty;
    ...
    ov.querySelectorAll('.t3me-pen').forEach(function(b){
      b.onclick = function(){
        penalty = parseInt(b.dataset.pen, 10);
        refreshPenButtons();
        updatePreview();
        input.focus();
      };
    ...
    // Parse "12.34", "1:23.45", "12345" (typed without dot → centiseconds 123.45)
    function parseTime(raw){
      raw = String(raw||'').trim();
      if (!raw) return null;
      // Format with colon: M:SS.ms
      var colonMatch = raw.match(/^(\d+):(\d{1,2}(?:\.\d{1,3})?)$/);
      if (colonMatch){
        var m = parseInt(colonMatch[1], 10);
        var s = parseFloat(colonMatch[2]);
        return Math.round((m*60 + s) * 1000);
      }
      // Plain decimal with dot: e.g. 12.34, 123.4
      if (/^\d+\.?\d*$/.test(raw)){
        var v = parseFloat(raw);
        if (isNaN(v)) return null;
        // If user typed "1234" without dot, treat as centiseconds → 12.34
        if (!raw.includes('.') && raw.length >= 3){
          var asCs = parseInt(raw, 10) / 100;
          // Heuristic: if >= 60 -> probably they typed seconds.cs (e.g. "8523" = 85.23s = 1:25.23)
          if (asCs > 0 && asCs < 600) return Math.round(asCs * 1000);
        }
    ...
    function fmtPreview(ms){
      var s = ms/1000;
      if (s >= 60){ var m = Math.floor(s/60); var r = s - m*60; return m+':'+(r<10?'0':'')+r.toFixed(2); }
      return s.toFixed(2);
    }
    ...
    function updatePreview(){
      ...
      var displayMs = ms + (penalty === 1 ? 2000 : 0);
      var s = penalty === 2 ? 'DNF' : (fmtPreview(displayMs) + (penalty===1?'+':''));
    ...
    function save(){
      var ms = parseTime(input.value);
      if (ms == null && penalty !== 2){
        input.style.borderColor = '#ef4444';
        setTimeout(function(){ input.style.borderColor = 'rgba(255,255,255,.1)'; }, 800);
        return;
      }
      if (penalty === 2 && ms == null) ms = 0;
      saveManualSolve(ms, penalty, scramble);
      close();
    }
    ov.querySelector('#t3me-x').onclick = close;
    ov.querySelector('#t3me-cancel').onclick = close;
    ov.querySelector('#t3me-save').onclick = save;
    ov.onclick = function(e){ if (e.target === ov) close(); };
    document.addEventListener('keydown', function esc(e){
      if (e.key === 'Escape'){ close(); document.removeEventListener('keydown', esc); }
      else if (e.key === 'Enter' && !e.shiftKey){ save(); document.removeEventListener('keydown', esc); }
    });
  }

  function saveManualSolve(timeMs, penalty, scramble){
    var puzzle = getActivePuzzle();
    var data = JSON.parse(localStorage.getItem(STORAGE_KEY)||'{}');
    if (!data.sessions) data.sessions = {};
    if (!data.sessions[puzzle]) data.sessions[puzzle] = [];
    // Odoo timer stores solve time in MILLISECONDS (integer like 13190 = 13.19s).
    var solve = {
      t: Math.round(timeMs),
      s: scramble || (data.currentScramble || ''),
      d: Date.now(),
      pen: penalty || 0,
      manual: 1
    };
    data.sessions[puzzle].push(solve);
    localStorage.setItem(STORAGE_KEY, JSON.stringify(data));
    // Force Odoo timer to reload Time List: switch puzzles away and back.
    try {
      var sidebar = document.getElementById('t3-puzzles');
      if (sidebar){
        var buttons = Array.from(sidebar.querySelectorAll('button'));
        var active = buttons.find(function(b){ return b.classList.contains('active'); }) ||
                     buttons.find(function(b){
                       return b.style.background && (b.style.background.indexOf('239')>=0 || b.style.background.indexOf('ef4444')>=0 || b.style.background.indexOf('3b82f6')>=0);
                     });
        var other = buttons.find(function(b){ return b !== active && !b.classList.contains('active') && (b.textContent||'').trim() && (b.textContent||'').trim() !== '+'; });
        if (active && other){
          setTimeout(function(){
            other.click();
            setTimeout(function(){ active.click(); }, 80);
          }, 150);
        } else if (active){
          setTimeout(function(){ active.click(); }, 150);
        }
      }
    } catch(e){}
  }
  ...
  /* ===== DAILY STREAK ===== */
  // Returns map { 'YYYY-MM-DD': solveCount } for last 365 days
  function getDayMap(){
    try {
      var data = JSON.parse(localStorage.getItem(STORAGE_KEY)||'{}');
  ...
  function computeStreak(){
    ...
    // Walk back from today; if today has no solve, start from yesterday (grace period)
    var todayKey = d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+String(d.getDate()).padStart(2,'0');
    if (!map[todayKey]){
    ...
    var todaySolved = !!map[todayKey];
    return {streak: streak, todaySolved: todaySolved};
  }

  function updateStreakBadge(){
    var icon = document.getElementById('t3-streak-icon');
    if (!icon) return;
    var st = computeStreak();
    if (st.streak === 0){
      icon.innerHTML = '<span style="font-size:18px;opacity:.5">🔥</span>';
      icon.title = 'No streak yet — solve today to start one!';
    } else {
      icon.innerHTML = '<span style="font-size:14px">🔥</span><span style="font-size:10px;font-weight:700;margin-top:-2px;color:'+(st.todaySolved?'#fb923c':'#fbbf24')+'">'+st.streak+'</span>';
      icon.title = (st.todaySolved ? '🔥 ' : '⚠ ') + st.streak+'-day streak'
        + (st.todaySolved ? '' : ' — solve today to keep it!');
    }
  }
  ...
  function showStreakModal(){
    var st = computeStreak();
    var map = getDayMap();
    // Last 90 days mini-calendar
    var days = [];
    var today = new Date();
    var d0 = new Date(today);
    d0.setDate(d0.getDate() - 89);
    // Align to Sunday for clean grid
    ...
    days.forEach(function(dt){
      ...
      var bg, txt;
      if (count === 0){ bg = 'rgba(255,255,255,.04)'; txt = ''; }
      else if (count < 5){ bg = 'rgba(251,146,60,.4)'; txt = '🔥'; }
      else if (count < 15){ bg = 'rgba(251,146,60,.7)'; txt = '🔥'; }
      else { bg = '#fb923c'; txt = '🔥'; }
      var border = isToday ? '2px solid #fff' : '1px solid rgba(255,255,255,.05)';
      var label = dt.toLocaleDateString('en-GB',{day:'numeric',month:'short'}) + ': '+count+' solves';
      grid += '<div title="'+label+'" style="width:24px;height:24px;background:'+bg+';border:'+border+';border-radius:5px;display:flex;align-items:center;justify-content:center;font-size:10px">'+txt+'</div>';
    });

    // Compute total
    var totalSolves = Object.values(map).reduce(function(a,b){ return a+b; }, 0);
    ...
    keys.forEach(function(k){
      if (prev){
        var pd = new Date(prev), nd = new Date(k);
        var diff = Math.round((nd - pd) / 86400000);
        if (diff === 1) cur++;
        else cur = 1;
      } else cur = 1;
    ...
    var ov = document.createElement('div');
    ov.className = 't3-auth-overlay';
    ov.innerHTML = '<div class="t3-auth-modal" style="max-width:560px;width:96vw;padding:24px">'
      + '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:18px">'
        + '<h3 style="color:#fff;margin:0;font-size:20px">🔥 Daily Streak</h3>'
        + '<button id="t3st-close" style="background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.08);color:#cbd5e1;width:30px;height:30px;border-radius:7px;cursor:pointer;font-size:13px">✕</button>'
      + '</div>'
      // Hero stat
      + '<div style="text-align:center;padding:24px;background:linear-gradient(135deg,rgba(251,146,60,.15),rgba(239,68,68,.05));border:1px solid rgba(251,146,60,.2);border-radius:14px;margin-bottom:20px">'
        + '<div style="font-size:54px;line-height:1">🔥</div>'
        + '<div style="font-size:42px;font-weight:800;color:#fb923c;font-family:monospace;margin:6px 0">'+st.streak+'</div>'
        + '<div style="color:#cbd5e1;font-size:13px;text-transform:uppercase;letter-spacing:1px">Day streak</div>'
        + (st.todaySolved
            ? '<div style="color:#22c55e;font-size:12px;margin-top:8px">✓ Today is solved</div>'
            : '<div style="color:#fbbf24;font-size:12px;margin-top:8px">⚠ Solve today to keep it alive!</div>')
      + '</div>'
      // Stats row
      + '<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:20px">'
        + '<div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.06);border-radius:10px;padding:12px;text-align:center"><div style="color:#fb923c;font-size:22px;font-weight:800">'+longest+'</div><div style="color:#64748b;font-size:11px;text-transform:uppercase;margin-top:2px">Longest</div></div>'
        + '<div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.06);border-radius:10px;padding:12px;text-align:center"><div style="color:#fbbf24;font-size:22px;font-weight:800">'+activeDays+'</div><div style="color:#64748b;font-size:11px;text-transform:uppercase;margin-top:2px">Active days</div></div>'
        + '<div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.06);border-radius:10px;padding:12px;text-align:center"><div style="color:#22c55e;font-size:22px;font-weight:800">'+totalSolves+'</div><div style="color:#64748b;font-size:11px;text-transform:uppercase;margin-top:2px">Total solves</div></div>'
      + '</div>'
      // Calendar
      + '<div style="color:#94a3b8;font-size:11px;text-transform:uppercase;letter-spacing:1px;margin-bottom:8px">Last 90 days</div>'
      + '<div style="display:grid;grid-template-columns:repeat(7,24px);gap:3px;justify-content:center">'+grid+'</div>'
    + '</div>';
    document.body.appendChild(ov);
    function close(){ ov.remove(); }
    ov.querySelector('#t3st-close').onclick = close;
    ov.onclick = function(e){ if (e.target === ov) close(); };
    document.addEventListener('keydown', function esc(e){
      if (e.key === 'Escape'){ close(); document.removeEventListener('keydown', esc); }
    });
  ...
  function getActivePuzzle(){
    // Prefer the authoritative value the Odoo timer itself persists.
    try {
      var st = JSON.parse(localStorage.getItem(STORAGE_KEY)||'{}');
      if (st && st.puzzle) return st.puzzle;
    } catch(e){}
    // Fall back to the active sidebar button — read data-p, NOT the label
    // (labels are emoji now, so text matching would break).
    var btns = document.querySelectorAll('#t3-puzzles button');
    var found = '333';
    btns.forEach(function(b){
      var isActive = b.classList.contains('active')
        || (b.style.background && (b.style.background.indexOf('239') >= 0 || b.style.background.indexOf('3b82f6') >= 0))
        || (b.style.borderColor && b.style.borderColor.indexOf('ef4444') >= 0);
      if (isActive){
  ...
  // Human-readable name for a puzzle key (buttons now show emoji only).
  function puzzleKeyLabel(key){
    var map = {'333':'3x3x3','222':'2x2x2','444':'4x4x4','555':'5x5x5','666':'6x6x6','777':'7x7x7',
      '333oh':'3x3 One-Handed','333bld':'3x3 Blindfolded','333fm':'Fewest Moves',
      '444bld':'4x4 Blindfolded','555bld':'5x5 Blindfolded','333mbld':'Multi Blindfolded',
      'pyra':'Pyraminx','skewb':'Skewb','clock':'Clock','mega':'Megaminx','sq1':'Square-1',
      'fto':'Face-Turning Octahedron'};
    return map[key] || key;
  }

  function getSolvesForPuzzle(puzzle){
    try {
      var data = JSON.parse(localStorage.getItem(STORAGE_KEY)||'{}');
      return (data.sessions && data.sessions[puzzle]) || [];
    } catch(e){ return []; }
  }

  function getSolveByNumber(solveNumber){
    // Verified mapping (DOM ↔ storage):
    //   solveNumber 1 = oldest = arr[0]
    //   solveNumber N = newest = arr[N-1]
    //   (Display shows newest at top with the highest number, but stored array is chronological.)
    var puzzle = getActivePuzzle();
    var arr = getSolvesForPuzzle(puzzle);
    if (!arr.length) return null;
    var idx = solveNumber - 1;
    if (idx < 0 || idx >= arr.length) return null;
    return {puzzle: puzzle, index: idx, solve: arr[idx]};
  }

  // Detect whether Bluetooth or keyboard input was used for the most recent solve.
  // Stored on solve.ble = 1 by Gan timer save path.
  function describeInput(solve){
    if (solve && solve.ble) return '📶 Bluetooth';
    if (solve && solve.manual) return '⌨ Manual (StackMat)';
    return '⌨ Desktop';
  }

  function fmtSolveTime(t, pen){
    if (pen === 2) return 'DNF';
    var ms = t + (pen === 1 ? 2000 : 0);
    var s = ms / 1000;
    var out = (s >= 60)
      ? (function(){ var m = Math.floor(s/60); var r = s - m*60; return m+':'+(r<10?'0':'')+r.toFixed(2); })()
      : s.toFixed(2);
    return out + (pen === 1 ? '+' : '');
  }

  function fmtSolveDate(ts){
    if (!ts) return '—';
    try {
      var d = new Date(ts);
  ...
  // openQuickNoteModal kept as alias for backwards-compat
  function openQuickNoteModal(solveNumber){ openSolveModal(solveNumber); }

  function openSolveModal(solveNumber){
    // Close any prior solve modal
    var prior = document.getElementById('t3sm-overlay');
    if (prior) prior.remove();

    var info = getSolveByNumber(solveNumber);
    if (!info){ console.warn('[SolveModal] solve not found', solveNumber); return; }
    ...
    function getMaxNumber(){
      var arr = getSolvesForPuzzle(info.puzzle);
      return arr.length;
    }

    function reload(num){
      var nu = getSolveByNumber(num);
      if (!nu) return;
      info = nu;
      render();
    }
    ...
    function render(){
      var solve = info.solve;
      var pen = solve.pen || 0;
      var timeStr = fmtSolveTime(solve.t, pen);
      var dateStr = fmtSolveDate(solve.d);
      var note = solve.note || '';
      var hasPrev = solveNumber > 1;
      var hasNext = solveNumber < getMaxNumber();

      ov.innerHTML = '<div class="t3-auth-modal" id="t3sm-modal" style="max-width:520px;width:96vw;padding:24px">'
        // Header
        + '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:18px">'
          + '<h3 style="color:#fff;margin:0;font-size:20px;font-weight:700">Solve #'+solveNumber+'</h3>'
          + '<button id="t3sm-close" style="background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.08);color:#cbd5e1;width:32px;height:32px;border-radius:8px;cursor:pointer;font-size:14px;display:flex;align-items:center;justify-content:center">✕</button>'
        + '</div>'

        // Scramble card
        + (solve.s ? '<div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.06);border-radius:14px;padding:14px 16px;margin-bottom:18px">'
            + '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">'
              + '<div style="display:flex;align-items:center;gap:8px;color:#94a3b8;font-size:13px;font-weight:600">'
                + '<span style="color:#3b82f6;font-size:14px">🧊</span> Scramble'
              + '</div>'
              + '<button id="t3sm-copy" title="Copy scramble" style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.06);color:#cbd5e1;width:28px;height:28px;border-radius:6px;cursor:pointer;font-size:12px;display:flex;align-items:center;justify-content:center">⎘</button>'
            + '</div>'
            + '<div style="font-family:monospace;color:#e2e8f0;font-size:15px;line-height:1.6;letter-spacing:.5px;word-spacing:4px;text-align:center">'+escapeHTML(solve.s)+'</div>'
          + '</div>' : '')

        // Time + nav
        + '<div style="display:flex;align-items:center;justify-content:center;gap:18px;margin:18px 0 10px">'
          + '<button id="t3sm-prev" '+(hasPrev?'':'disabled')+' style="background:transparent;border:0;color:'+(hasPrev?'#cbd5e1':'#334155')+';font-size:24px;cursor:'+(hasPrev?'pointer':'not-allowed')+';padding:6px 10px">‹</button>'
          + '<div style="font-size:54px;font-weight:800;color:'+(pen===2?'#ef4444':'#fff')+';font-family:monospace;letter-spacing:-1px">'+timeStr+'</div>'
          + '<button id="t3sm-next" '+(hasNext?'':'disabled')+' style="background:transparent;border:0;color:'+(hasNext?'#cbd5e1':'#334155')+';font-size:24px;cursor:'+(hasNext?'pointer':'not-allowed')+';padding:6px 10px">›</button>'
        + '</div>'

        // Penalty pills
        + '<div style="display:flex;justify-content:center;gap:8px;margin-bottom:22px">'
          + '<button id="t3sm-plus2" style="background:'+(pen===1?'#fbbf24':'rgba(255,255,255,.04)')+';border:1px solid '+(pen===1?'#fbbf24':'rgba(255,255,255,.08)')+';color:'+(pen===1?'#0a0e1a':'#cbd5e1')+';padding:7px 16px;border-radius:999px;cursor:pointer;font-size:13px;font-weight:600">+2</button>'
          + '<button id="t3sm-dnf" style="background:'+(pen===2?'#ef4444':'rgba(255,255,255,.04)')+';border:1px solid '+(pen===2?'#ef4444':'rgba(255,255,255,.08)')+';color:'+(pen===2?'#fff':'#cbd5e1')+';padding:7px 16px;border-radius:999px;cursor:pointer;font-size:13px;font-weight:600">DNF</button>'
          + '<button id="t3sm-del" title="Delete solve" style="background:rgba(239,68,68,.1);border:1px solid rgba(239,68,68,.3);color:#fca5a5;padding:7px 14px;border-radius:999px;cursor:pointer;font-size:13px">🗑</button>'
        + '</div>'

        // Solve info
        + '<div style="text-transform:uppercase;color:#64748b;font-size:11px;letter-spacing:1px;font-weight:700;margin-bottom:8px">Solve Info</div>'
        + '<div style="background:rgba(255,255,255,.02);border:1px solid rgba(255,255,255,.05);border-radius:14px;overflow:hidden">'
          + '<div style="display:flex;justify-content:space-between;padding:13px 16px;border-bottom:1px solid rgba(255,255,255,.04)"><span style="color:#cbd5e1;font-size:14px">Inspection</span><span style="color:#64748b;font-size:14px">'+(solve.insp ? (solve.insp/1000).toFixed(2)+'s' : '—')+'</span></div>'
          + '<div style="display:flex;justify-content:space-between;padding:13px 16px;border-bottom:1px solid rgba(255,255,255,.04)"><span style="color:#cbd5e1;font-size:14px">Input</span><span style="color:#cbd5e1;font-size:14px">'+describeInput(solve)+'</span></div>'
          + '<div style="display:flex;justify-content:space-between;padding:13px 16px;border-bottom:1px solid rgba(255,255,255,.04)"><span style="color:#cbd5e1;font-size:14px">Date</span><span style="color:#cbd5e1;font-size:14px">'+dateStr+'</span></div>'
          // Notes section
          + '<div id="t3sm-note-row" style="padding:13px 16px;cursor:pointer">'
            + '<div style="display:flex;justify-content:space-between;align-items:center">'
              + '<span style="color:#cbd5e1;font-size:14px">Notes <span style="color:#64748b;font-size:11px">(Q)</span></span>'
              + '<button id="t3sm-note-edit" style="background:transparent;border:0;color:#cbd5e1;cursor:pointer;font-size:14px">✏</button>'
            + '</div>'
            + (note ? '<div id="t3sm-note-display" style="color:#94a3b8;font-size:13px;margin-top:6px;line-height:1.5;word-wrap:break-word">'+escapeHTML(note)+'</div>' : '<div id="t3sm-note-display" style="color:#475569;font-size:12px;font-style:italic;margin-top:6px">No note — click to add</div>')
          + '</div>'
        + '</div>'
      + '</div>';
    ...
    function bind(){
      ov.querySelector('#t3sm-close').onclick = close;
      var copyBtn = ov.querySelector('#t3sm-copy');
      if (copyBtn) copyBtn.onclick = function(){
        if (info.solve.s){
          navigator.clipboard.writeText(info.solve.s).then(function(){
            copyBtn.textContent = '✓';
            setTimeout(function(){ copyBtn.textContent = '⎘'; }, 1200);
          });
        }
      };
      var prevBtn = ov.querySelector('#t3sm-prev');
      var nextBtn = ov.querySelector('#t3sm-next');
      if (prevBtn) prevBtn.onclick = function(){ if (solveNumber > 1){ solveNumber--; reload(solveNumber); } };
      if (nextBtn) nextBtn.onclick = function(){ if (solveNumber < getMaxNumber()){ solveNumber++; reload(solveNumber); } };

      ov.querySelector('#t3sm-plus2').onclick = function(){
        var cur = info.solve.pen || 0;
        setPenalty(cur === 1 ? 0 : 1);
      };
      ov.querySelector('#t3sm-dnf').onclick = function(){
        var cur = info.solve.pen || 0;
        setPenalty(cur === 2 ? 0 : 2);
      };
      ov.querySelector('#t3sm-del').onclick = function(){
        if (confirm('Delete this solve?')) deleteSolve();
      };
      ov.querySelector('#t3sm-note-row').onclick = openNoteEditor;
    }

    function setPenalty(newPen){
      var data = JSON.parse(localStorage.getItem(STORAGE_KEY)||'{}');
      var arr = (data.sessions && data.sessions[info.puzzle]) || [];
      if (!arr[info.index]) return;
      arr[info.index].pen = newPen;
      localStorage.setItem(STORAGE_KEY, JSON.stringify(data));
      info.solve = arr[info.index];
      render();
    }
    ...
    function openNoteEditor(){
      var existing = info.solve.note || '';
      var box = document.createElement('div');
      box.style.cssText = 'position:absolute;inset:0;background:rgba(10,14,26,.85);backdrop-filter:blur(8px);z-index:10;display:flex;align-items:center;justify-content:center;padding:20px';
      box.innerHTML = '<div style="background:linear-gradient(135deg,#161a2e,#0a0e1a);border:1px solid rgba(255,255,255,.1);border-radius:14px;padding:18px;width:100%;max-width:440px">'
        + '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px">'
          + '<div style="color:#fff;font-weight:700;font-size:14px;display:flex;align-items:center;gap:8px">📝 Quick Note</div>'
          + '<button id="t3sm-note-x" style="background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.08);color:#cbd5e1;width:26px;height:26px;border-radius:6px;cursor:pointer;font-size:12px">✕</button>'
        + '</div>'
        + '<textarea id="t3sm-note-ta" maxlength="240" placeholder="What happened? (PLL skip, bad cross, lockup on F2L...)" style="width:100%;min-height:90px;background:rgba(0,0,0,.3);border:1px solid rgba(255,255,255,.1);border-radius:10px;padding:10px;color:#fff;font-size:14px;font-family:inherit;resize:vertical;box-sizing:border-box"></textarea>'
        + '<div style="display:flex;justify-content:space-between;align-items:center;margin-top:8px">'
          + '<span id="t3sm-note-cnt" style="color:#64748b;font-size:11px">'+existing.length+'/240</span>'
          + '<div style="display:flex;gap:6px">'
            + '<button id="t3sm-note-cancel" style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);color:#cbd5e1;padding:6px 14px;border-radius:7px;cursor:pointer;font-size:12px">Cancel</button>'
            + '<button id="t3sm-note-save" style="background:#fbbf24;border:0;color:#0a0e1a;padding:6px 16px;border-radius:7px;cursor:pointer;font-size:12px;font-weight:700">Save</button>'
          + '</div>'
        + '</div>'
      + '</div>';
      ov.querySelector('#t3sm-modal').appendChild(box);
      var ta = box.querySelector('#t3sm-note-ta');
      var cnt = box.querySelector('#t3sm-note-cnt');
      ta.value = existing;
      ta.focus(); ta.setSelectionRange(ta.value.length, ta.value.length);
      ta.addEventListener('input', function(){ cnt.textContent = ta.value.length+'/240'; });
      function closeNote(){ box.remove(); }
      function saveNote(){
        var text = ta.value.trim();
        var data = JSON.parse(localStorage.getItem(STORAGE_KEY)||'{}');
        var arr = (data.sessions && data.sessions[info.puzzle]) || [];
        if (!arr[info.index]) return;
        if (text) arr[info.index].note = text;
        else delete arr[info.index].note;
        window.__t3SkipNextSync = true;
        localStorage.setItem(STORAGE_KEY, JSON.stringify(data));
        info.solve = arr[info.index];
        syncSolveNote(info.puzzle, info.solve, text);
        renderNoteIndicators();
        closeNote();
        render();
      }
      box.querySelector('#t3sm-note-x').onclick = closeNote;
      box.querySelector('#t3sm-note-cancel').onclick = closeNote;
      box.querySelector('#t3sm-note-save').onclick = saveNote;
      box.addEventListener('keydown', function(e){
        if (e.key === 'Escape'){ e.stopPropagation(); closeNote(); }
        else if (e.key === 'Enter' && (e.metaKey || e.ctrlKey)){ saveNote(); }
      });
    }
    ...
    function keyHandler(e){
      ...
      else if (e.key === 'ArrowLeft' && solveNumber > 1){ solveNumber--; reload(solveNumber); }
      else if (e.key === 'ArrowRight' && solveNumber < getMaxNumber()){ solveNumber++; reload(solveNumber); }
      else if ((e.key === 'q' || e.key === 'Q') && !document.getElementById('t3sm-note-ta')){ openNoteEditor(); }
    }
  ...
  function syncSolveNote(puzzle, solve, noteText){
    if (!session || !sb) return;
    sb.from('solves').update({note: noteText || null}).match({
      user_id: session.user.id,
      puzzle: puzzle,
      time_ms: Math.round(solve.t),
      solve_date: Math.round(solve.d)
    }).then(function(r){
      if (r.error){
        // Note column doesn't exist yet — silent fallback to localStorage only
        if (/note.*column|column.*note|schema cache/i.test(r.error.message||'')){
          console.info('[QuickNote] Stored locally (note column not yet in Supabase)');
        }
      }
    });
  }
  ...
  function renderNoteIndicators(){
    var rows = document.querySelectorAll('.t3-times-row');
    ...
    rows.forEach(function(row){
      ...
      if (!numMatch) return;
      var solveNumber = parseInt(numMatch[1], 10);
      var idx = solveNumber - 1;  // #1 = arr[0]
      var solve = (idx >= 0 && idx < arr.length) ? arr[idx] : null;
      var hasNote = solve && solve.note;
      // Tooltip: show note content if present, otherwise hint at click
      row.title = hasNote ? '📝 '+solve.note : 'Click to view solve details';
      // Gradient color: green=top25%, yellow=mid, red=bot25% (skip DNFs)
      if (solve && (solve.pen||0) !== 2){
        var validTimes = arr.filter(function(s){ return (s.pen||0) !== 2; })
          .map(function(s){ return s.t + ((s.pen||0)===1?2000:0); }).sort(function(a,b){ return a-b; });
        if (validTimes.length >= 3){
          var t = solve.t + ((solve.pen||0)===1?2000:0);
          var rank = validTimes.indexOf(t);
          var pct = rank / (validTimes.length - 1);  // 0=fastest, 1=slowest
          var color;
          if (pct <= 0.25) color = '#22c55e';        // top 25% green
          else if (pct >= 0.75) color = '#ef4444';   // bottom 25% red
          else if (pct <= 0.5) {
            // green → yellow gradient
            var k = (pct - 0.25) / 0.25;
            color = mix('#22c55e', '#eab308', k);
          } else {
            var k2 = (pct - 0.5) / 0.25;
            color = mix('#eab308', '#ef4444', k2);
          }
          // Apply to the time text in row (last span/strong containing the time)
          var timeNode = row.querySelector('.t3-times-time') || row.lastElementChild;
          if (timeNode) timeNode.style.color = color;
        }
      }
      // Add/remove note dot
      var dot = row.querySelector('.t3qn-dot');
      if (hasNote && !dot){
        dot = document.createElement('span');
        dot.className = 't3qn-dot';
        dot.textContent = '📝';
        dot.style.cssText = 'font-size:11px;margin-left:6px;opacity:.8';
        row.appendChild(dot);
      } else if (!hasNote && dot){
        dot.remove();
      }
  ...
  /* ===== PBs MODAL ===== */
  function showPBsModal(){
    // Pull from local storage (already synced with cloud)
    var data = readLocal();
    var sessions = (data && data.sessions) || {};

    var puzzleNames = {
      '333':'3x3x3','222':'2x2x2','444':'4x4x4','555':'5x5x5','666':'6x6x6','777':'7x7x7',
      '333oh':'3x3 OH','333bld':'3x3 BLD','333fm':'FMC','444bld':'4x4 BLD','555bld':'5x5 BLD',
      '333mbf':'Multi BLD','pyram':'Pyraminx','skewb':'Skewb','clock':'Clock',
      'minx':'Megaminx','sq1':'Square-1'
    };
    var puzzleOrder = ['333','222','444','555','666','777','333oh','pyram','skewb','sq1','minx','clock','333bld','333fm'];
    ...
    function eff(s){ if ((s.pen||0)===2) return Infinity; return s.t + ((s.pen||0)===1?2000:0); }

    var rows = '';
    var anyData = false;
    puzzleOrder.forEach(function(p){
      var arr = sessions[p];
      if (!arr || !arr.length) return;
      anyData = true;
      var validTimes = arr.map(eff).filter(function(t){ return t !== Infinity; });
      var single = validTimes.length ? Math.min.apply(null, validTimes) : null;
      var bestAo5 = bestRollingAO(arr, 5);
      var bestAo12 = bestRollingAO(arr, 12);
      var total = arr.length;
      // Find date of best single
      var pbSolve = arr.find(function(s){ return eff(s) === single; });
      var pbDate = pbSolve && pbSolve.d ? new Date(pbSolve.d).toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'}) : '';

      rows += '<tr style="border-bottom:1px solid rgba(255,255,255,.05)">'
        + '<td style="padding:12px 10px;color:#fff;font-weight:600">'+(puzzleNames[p]||p)+'</td>'
        + '<td style="padding:12px 10px;text-align:right;font-family:monospace;color:#fbbf24;font-size:15px;font-weight:700">'+fmt(single)+'</td>'
        + '<td style="padding:12px 10px;text-align:right;font-family:monospace;color:#cbd5e1">'+fmt(bestAo5)+'</td>'
        + '<td style="padding:12px 10px;text-align:right;font-family:monospace;color:#94a3b8">'+fmt(bestAo12)+'</td>'
        + '<td style="padding:12px 10px;text-align:right;color:#64748b;font-size:12px">'+total+'</td>'
        + '<td style="padding:12px 10px;text-align:right;color:#64748b;font-size:11px">'+pbDate+'</td>'
        + '</tr>';
    });

    // Other puzzles not in main order
    Object.keys(sessions).forEach(function(p){
      if (puzzleOrder.indexOf(p) >= 0) return;
      var arr = sessions[p];
      if (!arr || !arr.length) return;
      anyData = true;
      var validTimes = arr.map(eff).filter(function(t){ return t !== Infinity; });
      ...
      rows += '<tr style="border-bottom:1px solid rgba(255,255,255,.05)">'
        + '<td style="padding:12px 10px;color:#fff;font-weight:600">'+(puzzleNames[p]||p)+'</td>'
        + '<td style="padding:12px 10px;text-align:right;font-family:monospace;color:#fbbf24;font-weight:700">'+fmt(single)+'</td>'
        + '<td colspan="4" style="padding:12px 10px;text-align:right;color:#64748b;font-size:11px">'+arr.length+' solves</td>'
        + '</tr>';
    });
    ...
    var body = anyData
      ? '<table style="width:100%;border-collapse:collapse;font-size:14px"><thead>'
        ...
        + '<th style="text-align:left;padding:10px;color:#94a3b8;font-weight:600">Puzzle</th>'
        + '<th style="text-align:right;padding:10px;color:#fbbf24;font-weight:600">Single</th>'
        + '<th style="text-align:right;padding:10px;color:#94a3b8;font-weight:600">AO5</th>'
        + '<th style="text-align:right;padding:10px;color:#94a3b8;font-weight:600">AO12</th>'
        + '<th style="text-align:right;padding:10px;color:#94a3b8;font-weight:600">Total</th>'
        + '<th style="text-align:right;padding:10px;color:#94a3b8;font-weight:600">PB Date</th>'
        + '</tr></thead><tbody>' + rows + '</tbody></table>'
      : '<div style="text-align:center;padding:40px;color:#64748b">No solves yet — start solving to see your PBs! 🎲</div>';

    var ov = document.createElement('div');
    ov.className = 't3-auth-overlay';
    ov.innerHTML = '<div class="t3-auth-modal" style="max-width:780px;width:96vw;max-height:88vh;overflow-y:auto">'
      + '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px">'
      + '<h3 style="color:#fff;margin:0;font-size:22px;display:flex;align-items:center;gap:10px">🏆 My Personal Bests <span style="color:#64748b;font-size:12px;font-weight:400;margin-left:6px">click row to share</span></h3>'
      + '<button id="t3pb-close" style="background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);color:#fff;width:32px;height:32px;border-radius:8px;cursor:pointer;font-size:16px">✕</button>'
      + '</div>'
      + body
      + '</div>';
    document.body.appendChild(ov);
    function close(){ ov.remove(); }
    ov.querySelector('#t3pb-close').onclick = close;
    // Click on a PB row → open share card for that puzzle's single PB
    ov.querySelectorAll('tbody tr').forEach(function(tr){
      tr.style.cursor = 'pointer';
      tr.onclick = function(){
        var puzzleName = (tr.children[0] && tr.children[0].textContent || '').trim();
        // Reverse-lookup puzzle key
        var lookup = Object.keys(puzzleNames).find(function(k){ return puzzleNames[k] === puzzleName; });
        var puzzle = lookup || '333';
        var arr = sessions[puzzle];
        if (!arr || !arr.length) return;
        var validTimes = arr.filter(function(s){ return (s.pen||0)!==2; });
        if (!validTimes.length) return;
        var best = validTimes.reduce(function(a,b){
          return ((a.t + (a.pen===1?2000:0)) <= (b.t + (b.pen===1?2000:0))) ? a : b;
        });
        close();
        buildShareCard({
          type: 'SINGLE',
          time: best.t + (best.pen===1?2000:0),
          puzzle: puzzle,
          scramble: best.s || ''
        });
      };
  ...
  function checkAndCelebratePB(puzzle, newSolve, prevArr){
    if (!prevArr || prevArr.length === 0){
      // First solve ever on this puzzle - mini celebration only
      celebrateFirstSolve(puzzle);
      return;
    }
    // Compute effective time including +2 penalty
    var effective = function(s){ return s.t + ((s.pen||0)===1 ? 2000 : 0); };
    var newEff = effective(newSolve);

    // Single PB?
    var validPrev = prevArr.filter(function(s){ return (s.pen||0) !== 2; });
    if (validPrev.length === 0){
      celebrateFirstSolve(puzzle);
      return;
    }
    var prevBest = Math.min.apply(null, validPrev.map(effective));
    if (newEff < prevBest){
      celebratePB('SINGLE', newEff, puzzle);
      return;
    }

    // AO5 PB?  (compute on last 5 inc. new one)
    var allWithNew = prevArr.concat([newSolve]);
    if (allWithNew.length >= 5){
      var newAo5 = computeAOX(allWithNew, 5);
      if (newAo5 != null){
        // Best previous AO5 = sliding window over prevArr
        var bestPrevAo5 = bestRollingAO(prevArr, 5);
        if (bestPrevAo5 == null || newAo5 < bestPrevAo5){
          celebratePB('AO5', newAo5, puzzle);
          return;
        }
      }
    }
    // AO12 PB?
    if (allWithNew.length >= 12){
      var newAo12 = computeAOX(allWithNew, 12);
      if (newAo12 != null){
        var bestPrevAo12 = bestRollingAO(prevArr, 12);
        if (bestPrevAo12 == null || newAo12 < bestPrevAo12){
          celebratePB('AO12', newAo12, puzzle);
          return;
        }
      }
    }
    // Sub-AO5 milestone? (e.g. first time under 20s on 3x3)
    checkSubMilestone(puzzle, newEff, prevArr);
  }

  function computeAOX(solves, n){
    if (solves.length < n) return null;
    var last = solves.slice(-n).map(function(s){
      if ((s.pen||0)===2) return Infinity;
      return s.t + ((s.pen||0)===1 ? 2000 : 0);
    });
    var dnfs = last.filter(function(t){ return t===Infinity; }).length;
    if (dnfs >= 2) return null;  // AO is DNF
    var trimCount = Math.max(1, Math.floor(n/20)+1);  // 5→1 trim, 12→1, 100→6
    var sorted = last.slice().sort(function(a,b){ return a-b; });
    var trimmed = sorted.slice(trimCount, n - trimCount);
    if (trimmed.some(function(t){ return t===Infinity; })) return null;
    var sum = trimmed.reduce(function(a,b){ return a+b; }, 0);
    return sum / trimmed.length;
  }
  function bestRollingAO(solves, n){
    if (solves.length < n) return null;
    var best = null;
    for (var i = 0; i + n <= solves.length; i++){
      var window = solves.slice(i, i+n);
      var ao = computeAOX(window, n);
      if (ao != null && (best == null || ao < best)) best = ao;
    }
    return best;
  }

  function checkSubMilestone(puzzle, timeMs, prevArr){
    // Milestones: under-30, under-20, under-15, under-12, under-10 for 3x3
    var milestones = {
      '333':  [30000, 20000, 15000, 12000, 10000, 8000, 6000],
      '222':  [10000, 7000, 5000, 4000, 3000, 2000],
      '444':  [120000, 90000, 60000, 45000, 35000],
      '555':  [180000, 150000, 120000, 90000, 70000],
      '333oh':[60000, 40000, 30000, 20000, 15000]
    };
    var list = milestones[puzzle];
    if (!list) return;
    var beatThis = list.find(function(m){ return timeMs < m; });
    if (beatThis == null) return;
    var prevBestUnder = prevArr.filter(function(s){ return (s.pen||0)!==2; })
      .map(function(s){ return s.t + ((s.pen||0)===1?2000:0); })
      .filter(function(t){ return t < beatThis; });
    if (prevBestUnder.length === 0){
      // First time under this milestone!
      celebrateMilestone(beatThis, puzzle);
    }
  }
  ...
  function celebratePB(type, time, puzzle){
    spawnConfetti(150);
    showCelebrationBanner('🏆 NEW '+type+' PB!', fmtTimeShort(time), '#fbbf24');
    pulseTimer('#fbbf24');
    playSound('pb');
    // Auto-prompt share dialog 1.5s later
    setTimeout(function(){ offerShare(type, time, puzzle); }, 2500);
  }

  function offerShare(type, time, puzzle){
    // Find the latest solve to grab its scramble
    try {
      var data = JSON.parse(localStorage.getItem(STORAGE_KEY)||'{}');
      var arr = (data.sessions && data.sessions[puzzle]) || [];
      var latest = arr[arr.length-1];
      var scramble = latest && latest.s ? latest.s : '';
      buildShareCard({type: type, time: time, puzzle: puzzle, scramble: scramble});
  ...
  function buildShareCard(data){
    ...
    ov.className = 't3-auth-overlay';
    ov.id = 't3sh-overlay';
    ov.innerHTML = '<div class="t3-auth-modal" style="max-width:600px;width:96vw;padding:20px">'
      + '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">'
        + '<h3 style="color:#fff;margin:0;font-size:18px">📤 Share your '+data.type+' PB</h3>'
        + '<button id="t3sh-close" style="background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.08);color:#cbd5e1;width:30px;height:30px;border-radius:7px;cursor:pointer;font-size:13px">✕</button>'
      + '</div>'
      + '<canvas id="t3sh-canvas" width="1080" height="1080" style="width:100%;border-radius:12px;background:#0a0e1a;display:block"></canvas>'
      + '<div style="display:flex;gap:8px;margin-top:14px;justify-content:flex-end">'
        + '<button id="t3sh-cancel" style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);color:#cbd5e1;padding:9px 18px;border-radius:8px;cursor:pointer;font-size:13px">Skip</button>'
        + '<button id="t3sh-copy" style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);color:#cbd5e1;padding:9px 18px;border-radius:8px;cursor:pointer;font-size:13px">Copy image</button>'
        + '<button id="t3sh-download" style="background:#fbbf24;border:0;color:#0a0e1a;padding:9px 22px;border-radius:8px;cursor:pointer;font-size:13px;font-weight:700">⬇ Download</button>'
      + '</div>'
    + '</div>';
    document.body.appendChild(ov);

    var canvas = ov.querySelector('#t3sh-canvas');
    drawShareCard(canvas, data);

    function close(){ ov.remove(); }
    ov.querySelector('#t3sh-close').onclick = close;
    ov.querySelector('#t3sh-cancel').onclick = close;
    ov.querySelector('#t3sh-download').onclick = function(){
      canvas.toBlob(function(blob){
        var a = document.createElement('a');
        a.href = URL.createObjectURL(blob);
        a.download = '3den-pb-'+data.puzzle+'-'+Date.now()+'.png';
        a.click();
        setTimeout(function(){ URL.revokeObjectURL(a.href); }, 1000);
      }, 'image/png');
    };
    ov.querySelector('#t3sh-copy').onclick = function(){
      canvas.toBlob(function(blob){
        if (navigator.clipboard && window.ClipboardItem){
          navigator.clipboard.write([new ClipboardItem({'image/png': blob})]).then(function(){
            var btn = ov.querySelector('#t3sh-copy');
            btn.textContent = '✓ Copied';
            setTimeout(function(){ btn.textContent = 'Copy image'; }, 1500);
          });
        } else {
  ...
  function drawShareCard(canvas, data){
    ...
    ctx.textAlign = 'left';
    ctx.fillText('3D', 80, 130);
    ctx.fillStyle = '#fff';
    ctx.fillText('EN', 175, 130);
    ctx.fillStyle = '#94a3b8';
    ctx.font = '20px system-ui';
    ctx.fillText('Speedcubing Timer', 80, 165);

    // PB badge
    ctx.fillStyle = 'rgba(251,191,36,.12)';
    roundRect(ctx, W-380, 80, 300, 80, 16, true, false);
    ctx.fillStyle = '#fbbf24';
    ctx.font = 'bold 28px system-ui';
    ctx.textAlign = 'center';
    ctx.fillText('🏆 NEW '+data.type+' PB', W-230, 132);

    // Username + flag
    var username = '';
    var flag = '';
    try {
      var prof = window.__t3denProfile || (function(){
        try { return JSON.parse(localStorage.getItem('t3den_profile_cache')||'null'); } catch(e){ return null; }
      })();
      if (prof){
        username = '@' + (prof.username || 'cuber');
        flag = prof.country ? countryFlagEmoji(prof.country) : '';
      } else {
        username = '@cuber';
      }
    } catch(e){ username = '@cuber'; }

    // Center: huge time
    ctx.fillStyle = '#fff';
    ctx.font = 'bold 240px system-ui, monospace';
    ctx.textAlign = 'center';
    ctx.fillText(fmtTimeShort(data.time), W/2, H/2 + 30);
    ...
    ctx.fillStyle = '#fbbf24';
    ctx.font = 'bold 56px system-ui';
    ctx.fillText(puzzleNames[data.puzzle] || data.puzzle, W/2, H/2 + 110);

    // Username
    ctx.fillStyle = '#cbd5e1';
    ctx.font = 'bold 38px system-ui';
    ctx.fillText((flag ? flag+'  ' : '') + username, W/2, H/2 - 250);

    // Scramble (if fits)
    if (data.scramble){
      ctx.fillStyle = '#64748b';
      ctx.font = '22px monospace';
      var scr = data.scramble.length > 70 ? data.scramble.substring(0, 67)+'...' : data.scramble;
      ctx.fillText(scr, W/2, H/2 + 200);
    }

    // Bottom strip — just date
  ...
  function celebrateMilestone(threshold, puzzle){
    spawnConfetti(80);
    showCelebrationBanner('🎯 SUB-'+Math.round(threshold/1000)+'!', 'First time!', '#22c55e');
    pulseTimer('#22c55e');
    playSound('milestone');
  }
  function celebrateFirstSolve(puzzle){
    spawnConfetti(40);
    showCelebrationBanner('🎉 FIRST SOLVE!', 'Welcome to '+puzzle, '#3b82f6');
  }
  function celebrateDNF(){
    var t = document.getElementById('t3-timer');
    if (!t) return;
    t.style.animation = 't3-shake .4s';
    setTimeout(function(){ t.style.animation=''; }, 500);
  }

  function pulseTimer(color){
    var t = document.getElementById('t3-timer');
    if (!t) return;
    var orig = t.style.textShadow;
    t.style.transition = 'text-shadow .3s, transform .3s';
    t.style.textShadow = '0 0 30px '+color+', 0 0 60px '+color;
    t.style.transform = 'scale(1.08)';
    setTimeout(function(){
      t.style.textShadow = orig || '';
      t.style.transform = '';
    }, 1500);
  }

  function showCelebrationBanner(title, subtitle, color){
    var b = document.createElement('div');
    b.style.cssText = 'position:fixed;top:50%;left:50%;transform:translate(-50%,-50%) scale(.5);z-index:99999;background:linear-gradient(135deg,rgba(15,23,42,.98),rgba(30,41,59,.98));border:2px solid '+color+';border-radius:20px;padding:32px 48px;text-align:center;box-shadow:0 25px 80px '+color+'66,0 0 120px '+color+'33;animation:t3-celebrate-in .6s cubic-bezier(.34,1.8,.64,1) forwards;backdrop-filter:blur(10px);pointer-events:none';
    b.innerHTML = '<div style="font-size:42px;font-weight:800;color:'+color+';margin-bottom:8px;text-shadow:0 0 20px '+color+'aa">'+title+'</div>'
      + '<div style="font-size:24px;color:#fff;font-family:monospace;font-weight:600">'+subtitle+'</div>';
    document.body.appendChild(b);
    setTimeout(function(){
      b.style.animation = 't3-celebrate-out .4s ease forwards';
      setTimeout(function(){ b.remove(); }, 400);
    }, 2200);
  }

  function spawnConfetti(count){
    var colors = ['#ef4444','#f59e0b','#fbbf24','#22c55e','#3b82f6','#8b5cf6','#ec4899'];
    ...
    for (var i = 0; i < count; i++){
      (function(idx){
        ...
        var startX = 50 + (Math.random()-0.5)*30;  // start near center
        var endX = startX + (Math.random()-0.5)*120;
        var endY = 110 + Math.random()*20;
        var rot = Math.random()*720 - 360;
        var dur = 2 + Math.random()*1.5;
        var delay = Math.random()*0.3;
        c.style.cssText = 'position:absolute;left:'+startX+'%;top:30%;width:'+size+'px;height:'+size+'px;background:'+color+';border-radius:'+(Math.random()>0.5?'50%':'2px')+';opacity:0;animation:t3-confetti-'+idx+' '+dur+'s '+delay+'s ease-out forwards;will-change:transform,opacity';
        var styleEl = document.createElement('style');
        styleEl.textContent = '@keyframes t3-confetti-'+idx+'{0%{opacity:1;transform:translate(0,0) rotate(0)}80%{opacity:1}100%{opacity:0;transform:translate('+(endX-startX)+'vw,'+(endY-30)+'vh) rotate('+rot+'deg)}}';
        document.head.appendChild(styleEl);
        container.appendChild(c);
        setTimeout(function(){ styleEl.remove(); }, (dur+delay+0.1)*1000);
      })(i);
    }
  ...
  function playSound(kind){
    // Tiny WebAudio jingle, no assets
    try {
      var ctx = new (window.AudioContext||window.webkitAudioContext)();
      var notes;
      if (kind === 'pb')        notes = [[523,0],[659,.1],[784,.2],[1046,.3]];   // C-E-G-C
      else if (kind === 'milestone') notes = [[523,0],[784,.12]];                 // C-G
      else                       notes = [[440,0]];
  ...
  function renderLeaderboard(){
    ...
    page.innerHTML = '<div style="max-width:1200px;margin:0 auto">'
      ...
      + '  <a href="/timer" style="color:#94a3b8;text-decoration:none;font-size:14px">← Back to timer</a>'
  ...
  function renderOverviewTab(container, p, allSolves){
    ...
    // GitHub-style heatmap with day-of-week + month labels
    // Build month label row from first cell of each week
    var dayLabels = ['Mon','Wed','Fri']; // show only 3 to save space
    var monthLabels = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
    // Each column is one week (7 cells, top=Sunday). Label months at the column where the 1st of the month falls.
    var weeks = Math.ceil(heatmap.cells.length / 7);
    var monthHeader = '';
    var lastMonthShown = -1;
    for (var w = 0; w < weeks; w++){
      var firstCell = heatmap.cells[w*7];
      if (!firstCell){ monthHeader += '<div></div>'; continue; }
      var label = '';
      // Pick month from first day of this column
      var colDate = new Date(firstCell.date);
      var monthOfCol = colDate.getMonth();
      // Show label only when the month CHANGES vs previous shown column,
      // and only once per month (skip duplicates from week-overlap)
      if (monthOfCol !== lastMonthShown){
        label = monthLabels[monthOfCol];
        lastMonthShown = monthOfCol;
      }
      monthHeader += '<div style="font-size:10px;color:#64748b;text-align:left;height:14px">'+label+'</div>';
    }
    var html = '<div class="t3p-card">'
      + '<h3>🔥 Activity '+heatmap.totalSolves+' solves in '+heatmap.activeDays+' days</h3>'
      + '<div style="overflow-x:auto"><div style="display:inline-block;min-width:max-content">'
      // Month label row (above the heatmap, aligned with each week-column)
      + '<div style="display:grid;grid-template-columns:28px repeat('+weeks+',12px);gap:3px;margin-bottom:4px"><div></div>'+monthHeader+'</div>'
      // Day labels column + heatmap grid
      + '<div style="display:grid;grid-template-columns:28px 1fr;gap:6px;align-items:start">'
      + '<div style="display:grid;grid-template-rows:repeat(7,12px);gap:3px;font-size:10px;color:#64748b;line-height:12px">'
      ...
      + heatmap.cells.map(function(c){
          ...
          var human = dow + ' ' + dt.toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'});
          return '<div title="'+human+': '+c.count+' solves" style="width:12px;height:12px;background:rgba(239,68,68,'+alpha+');border-radius:2px"></div>';
        }).join('')
      + '</div></div></div></div></div>'
  ...
  function renderWCATab(container, p, wca, isOwn){
    ...
    var html = '<div class="t3p-wca">'
      ...
      + '</div></div>';
    if (prs){
      // WCA API returns time in centiseconds (1/100 s).
      // Format: <60s -> "12.34"   >=60s -> "1:44.85"
      // Special events: FMC (333fm) -> moves count   BLD multi -> "MM/TT TIME"
      function fmtWcaTime(centis, eventKey){
        if (centis == null || centis === 0) return '—';
        if (centis === -1) return 'DNF';
        if (centis === -2) return 'DNS';
        // FMC: stored as moves*100 (single) or moves*100 (average has decimals)
        if (eventKey === '333fm' || eventKey === 'FMC' || /fm/i.test(eventKey||'')){
          return (centis / 100).toFixed(2).replace(/\.00$/, '');
        }
        // Multi-blind has special encoding (DDDTTTTTTMM) — show raw for now
        if (eventKey === '333mbf' || /mbf|multi/i.test(eventKey||'')){
          return String(centis);
        }
        var totalSec = centis / 100;
        if (totalSec < 60) return totalSec.toFixed(2);
        var m = Math.floor(totalSec / 60);
        var rest = totalSec - m * 60;
        return m + ':' + (rest < 10 ? '0' : '') + rest.toFixed(2);
      }
      // WCA event code -> display name (matches WCA website)
      var WCA_EVENT_NAMES = {
        '222':       '2x2x2 Cube',
        '333':       '3x3x3 Cube',
        '444':       '4x4x4 Cube',
      ...
      function eventLabel(ev){ return WCA_EVENT_NAMES[ev] || ev; }
      // Order events like the WCA profile does
      var eventOrder = ['333','222','444','555','666','777','333bf','333fm','333oh','clock','minx','pyram','skewb','sq1','444bf','555bf','333mbf','333ft','magic','mmagic'];
      var sortedKeys = Object.keys(prs).sort(function(a,b){
        var ia = eventOrder.indexOf(a); var ib = eventOrder.indexOf(b);
        if (ia === -1) ia = 999; if (ib === -1) ib = 999;
        return ia - ib;
      });
      var rows = sortedKeys.map(function(ev){
        var r = prs[ev];
        ...
        return '<tr style="border-bottom:1px solid rgba(255,255,255,.04)"><td style="padding:10px;color:#fff;font-weight:600">'+escapeHTML(eventLabel(ev))+'</td>'
          + '<td style="padding:10px;text-align:right;font-family:monospace;color:#fbbf24">'+fmtWcaTime(single, ev)+'</td>'
          + '<td style="padding:10px;text-align:right;color:#64748b;font-size:11px">'+(singleRank?'WR #'+singleRank:'')+'</td>'
          + '<td style="padding:10px;text-align:right;font-family:monospace;color:#cbd5e1">'+fmtWcaTime(avg, ev)+'</td>'
  ...
  function renderWeekly(){
    ...
    page.id = 't3-weekly';
    page.style.cssText = 'position:fixed;inset:0;background:#0a0e1a;color:#fff;overflow-y:auto;z-index:9999;padding:40px 20px;box-sizing:border-box';
    page.innerHTML = '<div style="max-width:900px;margin:0 auto"><div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:32px;flex-wrap:wrap;gap:12px"><a href="/timer" style="color:#94a3b8;text-decoration:none;font-size:14px">← Back to timer</a><h1 style="margin:0;font-size:28px;font-weight:800">🏆 <span style="color:#a855f7">Weekly Competition</span></h1><div style="width:100px"></div></div><div id="t3w-content"><div style="padding:40px;text-align:center;color:#64748b">Loading...</div></div></div>';
    document.body.appendChild(page);
  ...
  function renderAlgsPLL(){
    ...
    var html = '<div style="max-width:1200px;margin:0 auto">'
      ...
      + '</div>'
      + '<p style="color:#94a3b8;font-size:14px;margin-bottom:16px">21 cases · Click a case to learn the algorithm and see 3D animation</p>'
      + '<div id="t3-pll-stats" style="display:flex;gap:16px;margin-bottom:16px;flex-wrap:wrap;font-size:13px"></div>'
      + '<input id="t3-alg-search" type="text" placeholder="Search by name, number or notation..." style="width:100%;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.1);border-radius:10px;padding:11px 14px;color:#fff;font-size:14px;box-sizing:border-box;margin-bottom:14px;font-family:inherit"/>'
      + '<div id="t3-cat-filter" style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:20px"></div>'
      ...
      + PLL_CASES.map(function(c){
          return '<a href="/algs?set=pll&c='+c.id+'" class="t3-alg-card" data-id="'+c.id+'" data-group="'+escapeHTML(c.difficulty||'')+'" data-name="'+escapeHTML((c.name||'')+' '+(c.alg||'')).toLowerCase()+'">'
    ...
    setupAlgFilters(page, PLL_CASES, 'difficulty');
  }
  ...
  function renderAlgsOLL(){
    ...
    var html = '<div style="max-width:1300px;margin:0 auto">'
      ...
      + '</div>'
      + '<p style="color:#94a3b8;font-size:14px;margin-bottom:16px">57 cases · Click a case to learn the algorithm</p>'
      + '<div id="t3-oll-stats" style="display:flex;gap:12px;margin-bottom:16px;flex-wrap:wrap;font-size:13px"></div>'
      + '<input id="t3-alg-search" type="text" placeholder="Search by name, number or notation..." style="width:100%;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.1);border-radius:10px;padding:11px 14px;color:#fff;font-size:14px;box-sizing:border-box;margin-bottom:14px;font-family:inherit"/>'
      + '<div id="t3-cat-filter" style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:20px"></div>'
      ...
      + OLL_CASES.map(function(c){
          return '<a href="/algs?set=oll&c='+c.id+'" class="t3-alg-card" data-id="'+c.id+'" data-group="'+escapeHTML(c.group||'')+'" data-name="'+escapeHTML((c.name||'')+' OLL '+c.id+' '+(c.alg||'')).toLowerCase()+'">'
    ...
    setupAlgFilters(page, OLL_CASES);
  }

  /* Build clickable category filter chips + wire search for an algorithm library page */
  function setupAlgFilters(page, cases, groupKey){
    groupKey = groupKey || 'group';
    var filterWrap = page.querySelector('#t3-cat-filter');
    var searchInput = page.querySelector('#t3-alg-search');
    var cards = Array.from(page.querySelectorAll('.t3-alg-card'));
    if (!filterWrap) return;

    // Count cases per group
    var groups = {};
    cases.forEach(function(c){
      var g = c[groupKey] || 'Other';
      groups[g] = (groups[g] || 0) + 1;
    });
    var groupNames = Object.keys(groups);

    var activeCat = 'All';
    function chip(label, count, isAll){
      var b = document.createElement('button');
      b.className = 't3-cat-chip';
      b.dataset.cat = isAll ? 'All' : label;
      b.style.cssText = 'background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);color:#cbd5e1;padding:7px 13px;border-radius:999px;cursor:pointer;font-size:12px;font-weight:600;font-family:inherit;display:flex;align-items:center;gap:6px;transition:all .12s';
      b.innerHTML = label + (count != null ? ' <span style="color:#64748b;font-size:11px">'+count+'</span>' : '');
      return b;
    }
    filterWrap.appendChild(chip('All', cases.length, true));
    groupNames.forEach(function(g){ filterWrap.appendChild(chip(g, groups[g], false)); });

    function applyFilters(){
      var q = (searchInput && searchInput.value || '').trim().toLowerCase();
      cards.forEach(function(card){
        var matchCat = (activeCat === 'All') || (card.dataset.group === activeCat);
        var matchSearch = !q || (card.dataset.name || '').indexOf(q) >= 0;
        card.style.display = (matchCat && matchSearch) ? '' : 'none';
      });
    }
    function refreshChips(){
      filterWrap.querySelectorAll('.t3-cat-chip').forEach(function(b){
        var on = b.dataset.cat === activeCat;
        b.style.background = on ? 'rgba(239,68,68,.9)' : 'rgba(255,255,255,.04)';
        b.style.color = on ? '#fff' : '#cbd5e1';
        b.style.borderColor = on ? '#ef4444' : 'rgba(255,255,255,.08)';
      });
    }
    filterWrap.querySelectorAll('.t3-cat-chip').forEach(function(b){
      b.onclick = function(){ activeCat = b.dataset.cat; refreshChips(); applyFilters(); };
    });
  ...
  function renderFeed(){
    ...
    page.innerHTML = '<div style="max-width:700px;margin:0 auto">'
      + '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:32px"><a href="/timer" style="color:#94a3b8;text-decoration:none;font-size:14px">← Back to timer</a><h1 style="margin:0;font-size:28px;font-weight:800">📰 <span style="color:#ef4444">Feed</span></h1><div style="width:100px"></div></div>'
  ...
  function buildShortcutsModal(){
    ...
    // Timer shortcuts (always relevant)
    if (path === '/' || path === '/timer'){
      sections.push({
        title: '⏱️ Timer',
        items: [
          ['SPACE (hold)', 'Start timer'],
          ['SPACE (release)', 'Stop timer'],
          ['N', 'New scramble'],
          ['D', 'Delete last solve'],
          ['Q', 'Quick note on last solve 📝'],
          ['P', 'Toggle +2 penalty on last'],
          ['M', 'Manual time entry (StackMat) ⌨'],
          ['DNF', 'Mark last as DNF'],
          ['I', 'Toggle 15s WCA inspection']
        ]
      });
    }
    ...
    if (path === '/trainer'){
      sections.push({
        ...
        items: [
          ['SPACE (hold)', 'Start timer'],
          ['SPACE (release)', 'Stop & save time'],
          ['A', 'Show / hide algorithm'],
          ['D', 'Open case in library'],
          ['Backspace / Delete', 'Delete last time']
        ]
      });
    }
    
    // Navigation shortcuts (always)
    sections.push({
      title: '🧭 Navigation',
      items: [
    ...
    sections.push({
      ...
      items: [
        ['📶 icon', 'Connect Gan Smart Timer'],
        ['Auto', 'Reconnects automatically when timer turns on'],
        ['Auto', 'Times saved as solves automatically']
      ]
    });
    ...
    sections.push({
      title: '🌐 Quick Links',
      items: [
        ['📚 icon', 'Algorithm library (PLL/OLL)'],
        ['💬 icon', 'Direct messages inbox'],
        ['🏆 icon (purple)', 'Weekly competition'],
        ['⚡ icon', 'Daily challenge'],
        ['🔔 icon', 'Notifications'],
        ['📰 icon', 'Feed (PRs from following)'],
        ['🏆 icon (gold)', 'Global leaderboard'],
        ['Profile badge', 'Click @username to view profile'],
        ['⚙ icon', 'Edit your profile']
      ]
    });
    
    var html = '<div class="t3-auth-modal" style="max-width:560px;max-height:90vh">'
      + '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:18px">'
      + '<h2 style="color:#fff;margin:0;font-size:20px">⌨️ Keyboard Shortcuts</h2>'
      + '<button id="t3sk-close" style="background:none;border:0;color:#94a3b8;font-size:24px;cursor:pointer;padding:0 8px">×</button>'
      + '</div>'
      + '<div style="overflow-y:auto;max-height:65vh">'
      + sections.map(function(s){
          return '<h3 style="color:#cbd5e1;margin:0 0 10px;font-size:14px;text-transform:uppercase;letter-spacing:.5px;font-weight:700">'+s.title+'</h3>'
            + '<div style="display:flex;flex-direction:column;gap:4px;margin-bottom:18px">'
            + s.items.map(function(it){
                return '<div style="display:flex;align-items:center;justify-content:space-between;padding:8px 12px;background:rgba(255,255,255,.02);border-radius:8px;border:1px solid rgba(255,255,255,.04)">'
                  + '<span style="color:#cbd5e1;font-size:13px">'+escapeHTML(it[1])+'</span>'
                  + '<kbd style="background:rgba(0,0,0,.4);border:1px solid rgba(255,255,255,.15);border-radius:6px;padding:3px 10px;color:#fbbf24;font-family:monospace;font-size:12px;font-weight:600">'+escapeHTML(it[0])+'</kbd>'
                  + '</div>';
              }).join('')
            + '</div>';
        }).join('')
      + '</div>'
      + '<div style="text-align:center;color:#64748b;font-size:11px;margin-top:8px">Press <kbd style="background:rgba(0,0,0,.4);border:1px solid rgba(255,255,255,.15);border-radius:4px;padding:2px 6px;color:#fbbf24;font-family:monospace;font-size:11px">?</kbd> anytime to open this menu</div>'
      + '</div>';
    ...
    ov.innerHTML = html;
    document.body.appendChild(ov);
    ov.querySelector('#t3sk-close').onclick = closeModal;
    ov.onclick = function(e){ if (e.target === ov) closeModal(); };
  }

  // Global key listener for shortcuts help
  if (!window.__t3ShortcutsBound){
    window.__t3ShortcutsBound = true;
    document.addEventListener('keydown', function(e){
      ...
      if (e.key === '?' || (e.shiftKey && e.code === 'Slash')){
        ...
        var existing = document.getElementById('t3-auth-overlay');
        if (existing){
          // If this is the shortcuts modal, close it
          if (existing.querySelector('#t3sk-close')){
            closeModal();
            return;
          }
        }
        ...
        var ov = document.getElementById('t3-auth-overlay');
        if (ov && ov.querySelector('#t3sk-close')){
          closeModal();
        }
      }
    });
  ...
  /* ===== THEME SWITCHER ===== */
  var THEMES = {
    red: {accent: '#ef4444', accentDark: '#dc2626', accentBg: 'rgba(239,68,68,0.15)', accentBorder: 'rgba(239,68,68,0.3)'},
    blue: {accent: '#3b82f6', accentDark: '#2563eb', accentBg: 'rgba(59,130,246,0.15)', accentBorder: 'rgba(59,130,246,0.3)'}
  };

  function getCurrentTheme(){
    return localStorage.getItem('t3den_theme') || 'red';
  }

  function applyTheme(themeName){
    var theme = THEMES[themeName] || THEMES.red;
    var styleId = 't3-theme-override';
    var existing = document.getElementById(styleId);
    if (existing) existing.remove();
    var style = document.createElement('style');
    style.id = styleId;
    
    // Override the red color (#ef4444 and variants) with the new theme color
    // Apply to: timer accent, scramble color, primary buttons, sticker red, charts
    style.textContent = ''
      + ':root{--t3-accent:'+theme.accent+';--t3-accent-dark:'+theme.accentDark+';--t3-accent-bg:'+theme.accentBg+';--t3-accent-border:'+theme.accentBorder+';}'
      // Timer page red elements - aggressive override
      + '#t3-puzzles button.active,#t3-puzzles button[style*="ef4444"],#t3-puzzles button[style*="rgb(239"],#t3-puzzles button[style*="background"]{background:'+theme.accent+' !important;color:#fff !important;border-color:'+theme.accent+' !important}'
      + '#t3-puzzles.t3-vertical button.active,#t3-puzzles.t3-vertical button[style*="ef4444"],#t3-puzzles.t3-vertical button[style*="rgb(239"]{background:'+theme.accentBg+' !important;color:'+theme.accent+' !important;border:1px solid '+theme.accent+' !important}'
      + '/* Override any inline style with red — EXCLUDE cube preview AND theme switcher dots (they must keep their original colors) */ '
      + ':not(#t3-preview-wrap):not(#t3-preview-wrap *):not(.t3-cube-svg):not(.t3-cube-svg *):not(.t3-stickers):not(.t3-stickers *):not(.t3-theme-btn)[style*="background-color: rgb(239, 68, 68)"],'
      + ':not(#t3-preview-wrap):not(#t3-preview-wrap *):not(.t3-cube-svg):not(.t3-cube-svg *):not(.t3-stickers):not(.t3-stickers *):not(.t3-theme-btn)[style*="background:#ef4444"],'
      + ':not(#t3-preview-wrap):not(#t3-preview-wrap *):not(.t3-cube-svg):not(.t3-cube-svg *):not(.t3-stickers):not(.t3-stickers *):not(.t3-theme-btn)[style*="background: rgb(239, 68, 68)"]{background:'+theme.accent+' !important;background-color:'+theme.accent+' !important}'
      + ':not(#t3-preview-wrap):not(#t3-preview-wrap *):not(.t3-theme-btn)[style*="color: rgb(239, 68, 68)"],:not(#t3-preview-wrap):not(#t3-preview-wrap *):not(.t3-theme-btn)[style*="color:#ef4444"]{color:'+theme.accent+' !important}'
      + ':not(#t3-preview-wrap):not(#t3-preview-wrap *):not(.t3-theme-btn)[style*="border-color: rgb(239, 68, 68)"],:not(#t3-preview-wrap):not(#t3-preview-wrap *):not(.t3-theme-btn)[style*="border-color:#ef4444"]{border-color:'+theme.accent+' !important}'
      + '/* Force cube preview stickers to keep red */ '
      + '#t3-preview-wrap [style*="rgb(239"],#t3-preview-wrap [style*="ef4444"]{background:#ef4444 !important;background-color:#ef4444 !important}'
      + '/* Theme switcher dots ALWAYS show their assigned color, regardless of active theme */ '
      + '.t3-theme-btn[data-theme="red"]{background:#ef4444 !important}'
      + '.t3-theme-btn[data-theme="blue"]{background:#3b82f6 !important}'
      + '/* Charts use red for line and bars */ '
      + 'canvas#t3-chart-line,canvas#t3-chart-hist{filter:hue-rotate('+(themeName==='blue'?'220deg':'0deg')+') saturate('+(themeName==='blue'?'1.1':'1')+')}'
      + '#t3-timer{color:#fff}'
      // Scramble keeps its accent so it's readable + visually distinctive (red/blue depending on theme)
      // BUT: do not propagate theme to cube visualization or 3D preview
      + '#t3-scramble{color:'+theme.accent+' !important}'
      // Protect cube visualizations & sticker grids from being recolored by hue-rotate or accent overrides
      + '.t3-cube-svg,.t3-cube-svg *,.t3-stickers,.t3-stickers *,#t3-cube-preview,#t3-cube-preview *,.t3-mini-cube,.t3-mini-cube *,svg.cube,svg.cube *{filter:none !important}'
      // Charts red lines
      
      // Auth UI
      + '.t3-auth-tab.active{background:'+theme.accentBg+' !important;color:'+theme.accent+' !important}'
      + '.t3-auth-btn{background:'+theme.accent+' !important}'
      + '.t3-auth-btn:hover:not(:disabled){background:'+theme.accentDark+' !important}'
      + '.t3-auth-input:focus{border-color:'+theme.accent+' !important}'
      + '.t3-auth-modal{border-color:'+theme.accentBorder+' !important;box-shadow:0 25px 50px -12px '+theme.accentBg+' !important}'
      + '.t3-auth-msg.error{background:rgba(239,68,68,.15);color:#fca5a5;border-color:rgba(239,68,68,.3)}'
      // Logo
      + '.t3-auth-logo h1 span{color:'+theme.accent+' !important}'
      // Profile borders
      + '.t3lb-avatar,.t3p-feed-avatar,.t3-auth-preview-circle{border-color:'+theme.accent+' !important}'
      // Follow button
      + '.t3p-follow-btn:not(.following){background:'+theme.accent+' !important}'
      + '.t3p-follow-btn:not(.following):hover{background:'+theme.accentDark+' !important}'
      // Reaction buttons (mine)
      + '.t3-react-btn[style*="rgba(239,68,68"]{background:'+theme.accentBg+' !important;border-color:'+theme.accent+' !important}'
      // Library/algs accent
      + '.t3-alg-card:hover{background:'+theme.accentBg+' !important;border-color:'+theme.accent+' !important}'
      // Trainer
      + '#t3tr-cube.selected{border-color:'+theme.accent+' !important}'
      // Animation keyframes overrides for follow buttons (use accent color in pulse glow)
      + '#t3-puzzles.t3-vertical button:hover{color:'+theme.accent+' !important}';
    document.head.appendChild(style);
    
    // Update body classes for downstream use
    document.body.setAttribute('data-t3-theme', themeName);
  }

  function setTheme(themeName){
    if (!THEMES[themeName]) return;
    localStorage.setItem('t3den_theme', themeName);
    applyTheme(themeName);
    // Update button highlights
    document.querySelectorAll('.t3-theme-btn').forEach(function(b){
      var active = b.getAttribute('data-theme') === themeName;
      b.style.borderColor = active ? '#fff' : 'rgba(255,255,255,.2)';
      b.style.transform = active ? 'scale(1.15)' : 'scale(1)';
    });
  }

  function injectThemeSwitcher(){
    // Find ALL/1D bar in timer
    var path = location.pathname.replace(/^\/[a-z]{2}\//, '/');
    if (path !== '/timer' && path !== '/') return;
    if (document.getElementById('t3-theme-switcher')) return;
    
    // Find the period buttons (ALL, 1D)
    var allBtn = Array.from(document.querySelectorAll('button')).find(function(b){
      return b.textContent.trim() === 'ALL';
    });
    if (!allBtn) return;
    var bar = allBtn.parentElement;
    if (!bar) return;
    
    var wrap = document.createElement('div');
    wrap.id = 't3-theme-switcher';
    wrap.style.cssText = 'display:inline-flex;gap:6px;align-items:center;margin-left:8px;padding-left:10px;border-left:1px solid rgba(255,255,255,.1)';
    
    Object.keys(THEMES).forEach(function(name){
      var btn = document.createElement('button');
      btn.className = 't3-theme-btn';
      btn.setAttribute('data-theme', name);
      btn.title = name.charAt(0).toUpperCase()+name.slice(1)+' theme';
      btn.style.cssText = 'width:24px;height:24px;border-radius:50%;background:'+THEMES[name].accent+';cursor:pointer;border:2px solid rgba(255,255,255,.2);transition:all .15s;padding:0;flex-shrink:0';
      btn.onclick = function(e){ e.preventDefault(); e.stopPropagation(); setTheme(name); };
      wrap.appendChild(btn);
    });
    ...
    setTimeout(function(){
      document.querySelectorAll('.t3-theme-btn').forEach(function(b){
        var active = b.getAttribute('data-theme') === current;
        b.style.borderColor = active ? '#fff' : 'rgba(255,255,255,.2)';
        b.style.transform = active ? 'scale(1.15)' : 'scale(1)';
      });
    }, 50);
  ...
  // Gan Timer BLE protocol (public spec, used by cstimer/twisty timer)
  var GAN_TIMER_SERVICE = '0000fff0-0000-1000-8000-00805f9b34fb';
  var GAN_TIMER_STATE_CHAR = '0000fff7-0000-1000-8000-00805f9b34fb';
  ...
  var bleState = {
    ...
    characteristic: null,
    connected: false,
    timerState: 'idle', // idle | handsOn | ready | inspecting | running | finished
    currentTime: 0,
    autoReconnect: true,
    reconnectTimer: null
  };
  ...
  function parseGanTimerData(data){
    // Gan-STv4944 protocol observed format:
    //   fe XX 01 SS [TIME...] CRC CRC
    //   XX = length (04 = state-only, 08 = state+time)
    //   01 = type byte
    //   SS = state byte
    //   For 8-byte messages: bytes 4=min, 5=sec, 6-7=ms (little-endian uint16)
    //   Last 2 bytes = CRC (ignored)
    //
    // States observed (byte index 3, after fe XX 01):
    //   06 = idle (heartbeat A)
    //   02 = idle (heartbeat B - alternates)
    //   01 = hands on pad
    //   03 = ready (green light)
    //   05 = running (live tick, if sent)
    //   04 = solve finished (with 8-byte time payload)
    //   07 = reset/stopped
    ...
    // Verify Gan-ST header
    if (bytes[0] !== 0xfe) return null;
    if (bytes[2] !== 0x01) return null;

    var stateByte = bytes[3];
    var state = 'idle';
    var time_ms = 0;

    if (stateByte === 0x06 || stateByte === 0x02){
      state = 'idle';
    } else if (stateByte === 0x01){
      state = 'handsOn';        // hands placed on pad
    } else if (stateByte === 0x03){
      state = 'ready';          // green light - release to start
    } else if (stateByte === 0x05){
      state = 'running';        // timer running
    } else if (stateByte === 0x04){
      state = 'finished';       // solve complete, with time
      // 8-byte msg: fe 08 01 04 MM SS LL HH CRC CRC
      if (bytes.length >= 8){
        var minutes = bytes[4];
        var seconds = bytes[5];
        var ms_lo = bytes[6];
        var ms_hi = bytes[7];
        var ms = ms_lo | (ms_hi << 8);   // little-endian uint16
        time_ms = minutes * 60000 + seconds * 1000 + ms;

        // BCD fallback if value out of range
        if (time_ms > 600000 || time_ms < 0){
          var bcd = function(b){ return ((b >> 4) * 10) + (b & 0xf); };
          var m2 = bcd(bytes[4]);
          var s2 = bcd(bytes[5]);
          var ms2 = (bcd(bytes[6]) * 10) + Math.floor(bcd(bytes[7])/10);
          time_ms = m2 * 60000 + s2 * 1000 + ms2;
        }
      }
    } else if (stateByte === 0x07){
      state = 'reset';
    }

    return {state: state, time_ms: time_ms, raw: bytes};
  }

  function onGanNotification(event){
    var data = event.target.value;
    
    // Always log raw bytes to the debug panel
    var bytes = [];
    for (var i = 0; i < data.byteLength; i++) bytes.push(data.getUint8(i).toString(16).padStart(2,'0'));
    var rawHex = bytes.join(' ');
    appendBleDebug(rawHex);
    
    var parsed = parseGanTimerData(data);
    if (!parsed) return;

    // Log parsed result for debugging (state + time)
    if (parsed.state === 'finished'){
      appendBleDebug('  → state=' + parsed.state + '  time=' + (parsed.time_ms/1000).toFixed(3) + 's');
    } else {
      appendBleDebug('  → state=' + parsed.state);
    }

    var prevState = bleState.timerState;
    bleState.timerState = parsed.state;
    bleState.currentTime = parsed.time_ms;

    updateBleStatusBadge();

    var timerEl = document.getElementById('t3-timer');
    if (!timerEl) return;

    // ----- LIVE TICK STATE MACHINE -----
    // Gan-STv4944 only sends state events, no live ticks during solve.
    // We detect the moment hands are released (ready → idle/anything) and run our own clock.

    function stopLiveTick(){
      if (bleState._tickInterval){
        clearInterval(bleState._tickInterval);
        bleState._tickInterval = null;
      }
    }

    function startLiveTick(){
      stopLiveTick();
      bleState._tickStart = performance.now();
      bleState._tickInterval = setInterval(function(){
        var ms = performance.now() - bleState._tickStart;
        var s = ms / 1000;
        if (s >= 60){
          var m = Math.floor(s/60);
          var rest = s - m*60;
          timerEl.textContent = m + ':' + (rest<10?'0':'') + rest.toFixed(2);
        } else {
          timerEl.textContent = s.toFixed(2);
        }
      }, 30);
    }

    if (parsed.state === 'handsOn'){
      stopLiveTick();
      clearTimeout(bleState._readyToRunTimer);
      timerEl.style.color = '#ef4444';
      timerEl.textContent = 'Hands on';
    } else if (parsed.state === 'ready'){
      stopLiveTick();
      timerEl.style.color = '#22c55e';
      timerEl.textContent = 'Ready';
      // Gan-ST V4 doesn't emit a "hands-released / running" event — it goes straight
      // from ready (03) to finished (04) once the solve is done. So we kick off our
      // own clock ~150ms after seeing ready, assuming the user is about to release.
      // If a non-ready, non-running event comes back, we cancel.
      clearTimeout(bleState._readyToRunTimer);
      bleState._readyToRunTimer = setTimeout(function(){
        if (bleState.timerState === 'ready'){
          timerEl.style.color = '#fff';
          startLiveTick();
          appendBleDebug('  ▶ LIVE TICK (auto-start after ready)');
        }
      }, 150);
    } else if (parsed.state === 'inspecting'){
      stopLiveTick();
      clearTimeout(bleState._readyToRunTimer);
      timerEl.style.color = '#fbbf24';
      timerEl.textContent = 'Inspecting...';
    } else if (parsed.state === 'idle'){
      // Transition: ready → idle = hands released, solve started (alt path)
      if (prevState === 'ready'){
        clearTimeout(bleState._readyToRunTimer);
        timerEl.style.color = '#fff';
        if (!bleState._tickInterval) startLiveTick();
        appendBleDebug('  ▶ START');
      }
      // (heartbeat idle when nothing is running - leave display as-is)
    } else if (parsed.state === 'running'){
      timerEl.style.color = '#fff';
      if (!bleState._tickInterval) startLiveTick();
    } else if (parsed.state === 'finished'){
      clearTimeout(bleState._readyToRunTimer);
      // Snapshot the live clock BEFORE we stop it — we'll use it as fallback / source of truth.
      var liveMs = bleState._tickStart ? Math.round(performance.now() - bleState._tickStart) : 0;
      stopLiveTick();
      timerEl.style.color = '#fff';

      var finalMs = parsed.time_ms;
      // Sanity check: anything under 500ms from Gan-ST is almost certainly a bogus packet
      // (real 3x3 solves are always >1s). Fall back to our own live clock.
      if (finalMs < 500 && liveMs > 500){
        appendBleDebug('  ⚠ Gan-ST reported ' + parsed.time_ms + 'ms — using live clock: ' + liveMs + 'ms');
        finalMs = liveMs;
      }
      ...
      // Save only when this looks like a real solve completion:
      //   1) we weren't already in 'finished' (no duplicate save)
      //   2) the previous state suggests an actual solve was running
      //   3) the time is at least 1 second (real 3x3 solves are always >1s)
      var realFinish = prevState === 'ready' || prevState === 'running' || prevState === 'idle' || prevState === 'handsOn';
      if (prevState !== 'finished' && realFinish && finalMs >= 1000){
        appendBleDebug('  💾 saving ' + seconds.toFixed(3) + 's');
        saveBleSolve(finalMs);
      } else {
        appendBleDebug('  ✗ skipping save (prevState=' + prevState + ', ms=' + finalMs + ')');
      }
    } else if (parsed.state === 'reset'){
      stopLiveTick();
      // keep last time visible
    }
  }

  function saveBleSolve(timeMs){
    // Read current scramble from Odoo timer DOM (multiple possible IDs/classes)
    var scrambleEl = document.getElementById('t3-scramble')
      || document.querySelector('#t3-scramble')
      || document.querySelector('[data-scramble]')
      || document.querySelector('.scramble');
    var scramble = scrambleEl ? (scrambleEl.textContent || '').trim() : '';

    // Determine active puzzle — single source of truth (reads state.puzzle, then data-p).
    var activePuzzle = getActivePuzzle();

    appendBleDebug('  → puzzle=' + activePuzzle + '  scramble=' + (scramble.substring(0,30) || '(empty)'));

    // Append to localStorage in 3DEN format
    try {
      var stored = JSON.parse(localStorage.getItem('t3den_v4') || '{}');
      if (!stored.sessions) stored.sessions = {};
      if (!stored.sessions[activePuzzle]) stored.sessions[activePuzzle] = [];
      // Odoo timer stores solve time in MILLISECONDS (integer).
      var solve = {
        t: Math.round(timeMs),
        s: scramble,
        d: Date.now(),
        pen: 0,
        ble: 1   // mark as Bluetooth solve
      };
      stored.sessions[activePuzzle].push(solve);
      localStorage.setItem('t3den_v4', JSON.stringify(stored));

      appendBleDebug('  ✅ saved (' + stored.sessions[activePuzzle].length + ' total in ' + activePuzzle + ')');
      console.log('[BLE Gan] Solve saved: ' + (timeMs/1000).toFixed(2) + 's on ' + activePuzzle);

      // Force Odoo timer to fully reload Time List from localStorage:
      // click another puzzle then click back. Clicking the same puzzle is a no-op,
      // but the switch-away + switch-back forces Odoo to re-read solves for activePuzzle.
      try {
        var sidebar = document.getElementById('t3-puzzles');
        if (sidebar){
          var buttons = Array.from(sidebar.querySelectorAll('button'));
      ...
      appendBleDebug('  ❌ save error: ' + e.message);
      console.error('[BLE Gan] save error', e);
    }
  }

  // Multiple Gan Timer service UUIDs (different models use different ones)
  var GAN_SERVICES = [
    '0000fff0-0000-1000-8000-00805f9b34fb',  // Gan Original / Halo (FFF0)
    '6e400001-b5a3-f393-e0a9-e50e24dcca9e',  // Gan ST V4 (Nordic UART)
    '0000180f-0000-1000-8000-00805f9b34fb',  // Battery service (fallback)
    '0000fff7-0000-1000-8000-00805f9b34fb',  // Alt
    '00001801-0000-1000-8000-00805f9b34fb'   // Generic Attribute
  ];

  async function connectGanTimer(){
    if (!bleSupported()){
      alert('Bluetooth not supported. Use Chrome, Edge, or Brave.');
      return;
    }
    try {
      updateBleStatusBadge('connecting');
      appendBleDebug('Loading gan-web-bluetooth library…');

      // Use the battle-tested gan-web-bluetooth library by afedotov.
      // Same library that powers cstimer / cubeast for Gan Smart Timer.
      var mod = await import('https://cdn.jsdelivr.net/npm/gan-web-bluetooth/+esm');
      var connectGanTimerLib = mod.connectGanTimer;
      var GanTimerState = mod.GanTimerState;

      appendBleDebug('Requesting Gan Smart Timer…');
      var conn = await connectGanTimerLib();
      bleState.connection = conn;
      bleState.GanTimerState = GanTimerState;
      bleState.connected = true;
      bleState.autoReconnect = true;
      updateBleStatusBadge('connected');
      appendBleDebug('✓ Connected: ' + (conn.deviceName || 'Gan Smart Timer'));
      if (conn.deviceName) localStorage.setItem('t3den_ble_lastdevice', conn.deviceName);

      var liveStart = 0;
      var lastState = null;
      var timerEl = function(){ return document.getElementById('t3-timer'); };

      function stopLiveTickLocal(){
        if (bleState._tickInterval){
          clearInterval(bleState._tickInterval);
          bleState._tickInterval = null;
        }
      }
      function startLiveTickLocal(){
        stopLiveTickLocal();
        liveStart = performance.now();
        bleState._tickInterval = setInterval(function(){
          var t = timerEl(); if (!t) return;
          var ms = performance.now() - liveStart;
          var s = ms / 1000;
          if (s >= 60){
            var m = Math.floor(s/60), rest = s - m*60;
            t.textContent = m + ':' + (rest<10?'0':'') + rest.toFixed(2);
          } else {
      ...
      conn.events$.subscribe(function(evt){
        var state = evt.state;
        var stateName = Object.keys(GanTimerState).find(function(k){ return GanTimerState[k] === state; }) || ('?'+state);
        appendBleDebug('event: ' + stateName + (evt.recordedTime != null ? ' time=' + evt.recordedTime + 'ms' : ''));

        var t = timerEl(); if (!t) return;

        // Hands placed on pads
        if (stateName === 'HANDS_ON' || stateName === 'HANDS_ON_TIMER'){
          stopLiveTickLocal();
          t.style.color = '#ef4444';
          t.textContent = 'Hands on';
        }
        // Green light — ready to release
        else if (stateName === 'GET_SET' || stateName === 'READY'){
          stopLiveTickLocal();
          t.style.color = '#22c55e';
          t.textContent = 'Ready';
        }
        // Hands released — solve starts, live counter ticking
        else if (stateName === 'RUNNING'){
          t.style.color = '#fff';
          startLiveTickLocal();
        }
        // Timer stopped — final time arrives
        else if (stateName === 'STOPPED' || stateName === 'FINISHED'){
          stopLiveTickLocal();
          t.style.color = '#fff';
          var liveMs = liveStart ? Math.round(performance.now() - liveStart) : 0;
          var finalMs = (evt.recordedTime != null && evt.recordedTime >= 500) ? evt.recordedTime : liveMs;
          if (finalMs >= 1000){
            var sec = finalMs / 1000;
            if (sec >= 60){
              var m2 = Math.floor(sec/60), rest2 = sec - m2*60;
              t.textContent = m2 + ':' + (rest2<10?'0':'') + rest2.toFixed(2);
            } else {
              t.textContent = sec.toFixed(2);
            }
            if (lastState !== 'STOPPED' && lastState !== 'FINISHED'){
              appendBleDebug('💾 saving ' + (finalMs/1000).toFixed(3) + 's');
              saveBleSolve(finalMs);
            }
          } else {
            appendBleDebug('✗ skip — too short (' + finalMs + 'ms)');
          }
          liveStart = 0;
        }
        // IDLE heartbeat — leave display alone
        lastState = stateName;
      });

      // Watch for disconnect
      if (conn.disconnect$ && conn.disconnect$.subscribe){
        conn.disconnect$.subscribe(function(){ onBleDisconnect(); });
      }

      console.log('[BLE Gan] Connected via gan-web-bluetooth');
    } catch(e){
      console.error('[BLE Gan] connect error', e);
      appendBleDebug('Connect error: '+e.message);
      updateBleStatusBadge('error');
      if (e.name !== 'NotFoundError' && (e.message||'').indexOf('cancelled') < 0){
  ...
  function onBleDisconnect(){
    console.log('[BLE Gan] disconnected');
    bleState.connected = false;
    updateBleStatusBadge('disconnected');
    
    if (bleState.autoReconnect){
      // Try reconnect after 3s
      if (bleState.reconnectTimer) clearTimeout(bleState.reconnectTimer);
      bleState.reconnectTimer = setTimeout(reconnectGanTimer, 3000);
    }
  }

  async function reconnectGanTimer(){
    if (!bleState.device || bleState.connected) return;
    try {
      updateBleStatusBadge('connecting');
      bleState.server = await bleState.device.gatt.connect();
      var services = await bleState.server.getPrimaryServices();
        ...
        try {
          ...
          for (var c = 0; c < chars.length; c++){
            if (chars[c].properties.notify || chars[c].properties.indicate){
              ...
              chars[c].addEventListener('characteristicvaluechanged', onGanNotification);
              if (!bleState.characteristic) bleState.characteristic = chars[c];
            }
          }
        } catch(e){}
      }
      bleState.connected = true;
      updateBleStatusBadge('connected');
      console.log('[BLE Gan] Reconnected');
    } catch(e){
      console.warn('[BLE Gan] reconnect failed, retrying...', e.message);
      if (bleState.autoReconnect){
        bleState.reconnectTimer = setTimeout(reconnectGanTimer, 5000);
      }
    }
  }

  function disconnectGanTimer(){
    bleState.autoReconnect = false;
  ...
  function updateBleStatusBadge(state){
    state = state || (bleState.connected ? 'connected' : 'disconnected');
    var btn = document.getElementById('t3-ble-icon');
    if (!btn) return;
    
    var dot = btn.querySelector('.t3-ble-dot');
    var emoji;
    var color;
    var bg;
    if (state === 'connected'){
      emoji = '📶'; color = '#22c55e'; bg = 'rgba(34,197,94,0.15)';
      btn.title = 'Gan Timer: ● Connected (' + (bleState.device ? bleState.device.name : '') + ') — click to disconnect';
    } else if (state === 'connecting'){
      emoji = '📶'; color = '#fbbf24'; bg = 'rgba(251,191,36,0.15)';
      btn.title = 'Connecting...';
    } else if (state === 'error'){
      emoji = '📶'; color = '#ef4444'; bg = 'rgba(239,68,68,0.15)';
      btn.title = 'Connection error';
    } else {
      emoji = '📶'; color = '#94a3b8'; bg = 'rgba(0,0,0,0.5)';
      btn.title = 'Bluetooth Timer (click to connect)';
    }
    btn.innerHTML = emoji;
  ...
  function appendBleDebug(line){
    if (!bleState.debugLog) bleState.debugLog = [];
    var ts = new Date().toLocaleTimeString();
    bleState.debugLog.push(ts + ' ' + line);
    if (bleState.debugLog.length > 100) bleState.debugLog.shift();
    var panel = document.getElementById('t3-ble-debug-panel');
    if (panel){
      var pre = panel.querySelector('pre');
      if (pre){
        pre.textContent = bleState.debugLog.join('\n');
        pre.scrollTop = pre.scrollHeight;
      }
    }
  }
  
  function showBleDebugPanel(){
    var existing = document.getElementById('t3-ble-debug-panel');
    if (existing){ existing.remove(); return; }
    var panel = document.createElement('div');
    panel.id = 't3-ble-debug-panel';
    panel.style.cssText = 'position:fixed;bottom:14px;left:14px;width:min(700px,90vw);max-height:50vh;background:#0a0e1a;border:2px solid #fbbf24;border-radius:12px;padding:14px;z-index:99999;color:#fff;font-family:monospace;font-size:11px;display:flex;flex-direction:column';
    panel.innerHTML = '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px"><strong style="color:#fbbf24">🔬 BLE Debug Log</strong>'
      + '<div style="display:flex;gap:8px"><button id="t3-ble-copy" style="background:#22c55e;color:#000;border:0;padding:5px 12px;border-radius:6px;cursor:pointer;font-weight:700">📋 Copy all</button>'
      + '<button id="t3-ble-clear" style="background:rgba(255,255,255,.1);color:#fff;border:0;padding:5px 10px;border-radius:6px;cursor:pointer">Clear</button>'
      + '<button id="t3-ble-close" style="background:none;border:0;color:#94a3b8;font-size:18px;cursor:pointer;padding:0 6px">×</button></div></div>'
      + '<pre style="background:rgba(0,0,0,.4);padding:10px;border-radius:6px;overflow-y:auto;max-height:38vh;white-space:pre-wrap;word-break:break-all;color:#22c55e;line-height:1.4;margin:0">'+(bleState.debugLog||['Waiting for BLE data... Connect timer + place hands.']).join('\n')+'</pre>';
    document.body.appendChild(panel);
    panel.querySelector('#t3-ble-close').onclick = function(){ panel.remove(); };
    panel.querySelector('#t3-ble-clear').onclick = function(){
      bleState.debugLog = [];
      panel.querySelector('pre').textContent = 'Cleared. Waiting for BLE data...';
    };
    panel.querySelector('#t3-ble-copy').onclick = function(){
      var text = (bleState.debugLog || []).join('\n');
      try {
        navigator.clipboard.writeText(text).then(function(){
          var b = panel.querySelector('#t3-ble-copy');
  ...
  function buildBleButton(){
    if (document.getElementById('t3-ble-icon')) return;
    if (!bleSupported()) return; // Only show on browsers that support it
    
    var btn = document.createElement('div');
    btn.className = 't3-trophy';
    btn.id = 't3-ble-icon';
    btn.style.right = '264px'; // grouped with streak/tools near the ☰ menu
    ...
    btn.onclick = function(){
      if (bleState.connected){
        if (confirm('Disconnect Gan Timer?')){
          disconnectGanTimer();
        }
      } else {
        connectGanTimer();
    ...
    // Long-press also shows debug (for mobile)
    var pressTimer = null;
    btn.addEventListener('mousedown', function(){
      pressTimer = setTimeout(showBleDebugPanel, 800);
    });
  ...
  function removeSplash(){
    var sp = document.getElementById('t3-splash');
    if (!sp) return;
    sp.style.opacity = '0';
    sp.style.pointerEvents = 'none';
    setTimeout(function(){ if (sp.parentNode) sp.remove(); }, 500);
  }
  ...
  function injectMobileCSS(){
    var prev = document.getElementById('t3-mobile-css');
    if (prev) prev.remove();
    var st = document.createElement('style');
    st.id = 't3-mobile-css';
    st.textContent =
      // Hide the desktop vertical sidebar
      'body.t3-mobile #t3-puzzles{display:none !important}'
      + 'body.t3-mobile{overflow-x:hidden !important}'
      // #t3den-app is a 2-column grid (main + side panel) — collapse to 1 column on mobile
      + 'body.t3-mobile #t3den-app{display:block !important;grid-template-columns:1fr !important;margin-left:0 !important;width:100% !important;max-width:100% !important;padding:108px 6px 24px !important;box-sizing:border-box !important}'
      // Mobile toolbar — flex row of icons, NO scrollbar
      + '#t3-mobile-toolbar{position:fixed;top:8px;left:8px;right:8px;z-index:9100;display:flex;align-items:center;gap:6px;justify-content:center;flex-wrap:nowrap;overflow-x:auto;scrollbar-width:none;-ms-overflow-style:none}'
      + '#t3-mobile-toolbar::-webkit-scrollbar{display:none;width:0;height:0}'
      + 'body.t3-mobile .t3-trophy,body.t3-mobile .t3-sync{position:static !important;width:34px !important;height:34px !important;font-size:14px !important;flex-shrink:0;margin:0 !important}'
      + 'body.t3-mobile #t3-auth-badge{position:static !important;transform:none !important;flex-shrink:0;order:99;max-width:none !important;overflow:visible !important;gap:6px;padding:4px 8px}'
      + 'body.t3-mobile #t3-auth-badge a{display:none !important}'   // hide username text to save space
      + 'body.t3-mobile #t3-auth-badge .dot{display:inline-block !important}'
      + 'body.t3-mobile #t3-auth-badge span{flex-shrink:0}'         // keep avatar visible
      + 'body.t3-mobile #t3-auth-badge button{padding:3px 8px;font-size:11px}'
      + 'body.t3-mobile #t3-theme-switcher{flex-shrink:0}'
      // Custom puzzle dropdown button
      + '#t3-mpz-btn{position:fixed;top:50px;left:8px;right:8px;z-index:9100;height:42px;background:#161a2e;color:#fff;border:1px solid rgba(239,68,68,.45);border-radius:11px;font-size:15px;font-weight:700;padding:0 16px;display:flex;align-items:center;justify-content:space-between;cursor:pointer}'
      + '#t3-mpz-btn .t3-mpz-arrow{color:#ef4444;font-size:13px;transition:transform .2s}'
      + '#t3-mpz-list{position:fixed;top:96px;left:8px;right:8px;z-index:9200;background:#0a0e1a;border:1px solid rgba(255,255,255,.14);border-radius:12px;padding:6px;max-height:60vh;overflow-y:auto;box-shadow:0 18px 50px rgba(0,0,0,.8)}'
      + '#t3-mpz-list button{display:block;width:100%;background:transparent;border:0;color:#e2e8f0;padding:12px 14px;text-align:left;font-size:14px;font-weight:600;border-radius:8px;cursor:pointer}'
      + '#t3-mpz-list button.active{background:rgba(239,68,68,.18);color:#ef4444}'
      // === Mobile: keep ONLY timer + scramble + charts ===
      + 'body.t3-mobile .t3-side{display:none !important}'              // Session Overview + Time List
      + 'body.t3-mobile .t3-stat-row{display:none !important}'          // AO5 / vs AO100 / preview mini-cards
      + 'body.t3-mobile .t3-chart-card{width:100% !important;max-width:100% !important;box-sizing:border-box !important;margin:0 0 12px !important;min-height:220px}'
      + 'body.t3-mobile .t3-charts,body.t3-mobile [class*="chart-row"]{display:flex !important;flex-direction:column !important;gap:12px !important;width:100% !important}'
      // Timer + scramble sizing
      + 'body.t3-mobile #t3-timer{font-size:76px !important}'
      + 'body.t3-mobile #t3-scramble{font-size:15px !important;padding:0 10px !important;line-height:1.5}';
    document.head.appendChild(st);
  }

  function setupMobileLayout(){
    var mobile = isMobileViewport();
    document.body.classList.toggle('t3-mobile', mobile);
    injectMobileCSS();

    var path = location.pathname.replace(/^\/[a-z]{2}\//, '/');
    var onTimer = (path === '/timer' || path === '/');

    if (mobile && onTimer){
      buildMobilePuzzleDropdown();
      buildMobileToolbar();
      fixHoldText(true);
      enableTouchTimer();
      // Hide the histogram chart on mobile — keep only the line chart
      var hist = document.getElementById('t3-chart-hist');
      if (hist){
        var hcard = hist.closest('.t3-chart-card');
        if (hcard) hcard.style.display = 'none';
      }
    } else {
      // Desktop: restore histogram
      var histD = document.getElementById('t3-chart-hist');
      if (histD){
        var hcardD = histD.closest('.t3-chart-card');
        if (hcardD) hcardD.style.display = '';
      }
      // Desktop: remove mobile-only elements, restore text
      var d = document.getElementById('t3-mpz-btn'); if (d) d.remove();
      var dl = document.getElementById('t3-mpz-list'); if (dl) dl.remove();
      var tb = document.getElementById('t3-mobile-toolbar');
      if (tb){
        Array.from(tb.children).forEach(function(c){ document.body.appendChild(c); });
        tb.remove();
      }
      fixHoldText(false);
    }
  ...
  function currentPuzzleLabel(){
    // Sidebar buttons show emoji only — pair the emoji with the readable name.
    var sidebar = document.getElementById('t3-puzzles');
    if (!sidebar) return '3×3';
    var active = Array.from(sidebar.querySelectorAll('button')).find(function(b){
      return b.classList.contains('active')
        || (b.style.background && (b.style.background.indexOf('239')>=0 || b.style.background.indexOf('ef4444')>=0 || b.style.background.indexOf('3b82f6')>=0));
    });
    if (!active) return '3×3';
    var icon = (active.textContent||'').trim();
    var key = active.getAttribute('data-p');
    return key ? (icon + '  ' + puzzleKeyLabel(key)) : icon;
  }

  function buildMobilePuzzleDropdown(){
    var sidebar = document.getElementById('t3-puzzles');
    if (!sidebar) return;
    if (document.getElementById('t3-mpz-btn')) {
      // refresh label
      var lbl = document.querySelector('#t3-mpz-btn .t3-mpz-label');
      if (lbl) lbl.textContent = currentPuzzleLabel();
      return;
    }
    var btn = document.createElement('div');
    btn.id = 't3-mpz-btn';
    btn.innerHTML = '<span class="t3-mpz-label">'+currentPuzzleLabel()+'</span><span class="t3-mpz-arrow">▼</span>';
    btn.onclick = function(e){ e.stopPropagation(); toggleMobilePuzzleList(); };
    document.body.appendChild(btn);
  }

  function toggleMobilePuzzleList(){
    var existing = document.getElementById('t3-mpz-list');
    if (existing){ existing.remove(); return; }
    var sidebar = document.getElementById('t3-puzzles');
    if (!sidebar) return;
    var list = document.createElement('div');
    list.id = 't3-mpz-list';
    var activeKey = getActivePuzzle();
    Array.from(sidebar.querySelectorAll('button')).forEach(function(b){
      var icon = (b.textContent||'').trim();
      var key = b.getAttribute('data-p');
      if (!icon || icon === '+' || !key) return;
      var text = icon + '  ' + puzzleKeyLabel(key);
      var item = document.createElement('button');
      item.textContent = text;
      if (key === activeKey) item.className = 'active';
    ...
    setTimeout(function(){
      document.addEventListener('click', function close(e){
        if (!list.contains(e.target) && e.target.id !== 't3-mpz-btn' && !e.target.closest('#t3-mpz-btn')){
          list.remove(); document.removeEventListener('click', close);
        }
      });
    }, 50);
  }
  ...
  function buildMobileToolbar(){
    if (document.getElementById('t3-mobile-toolbar')) return;
    var bar = document.createElement('div');
    bar.id = 't3-mobile-toolbar';
    document.body.appendChild(bar);
    // Gather all toolbar icons + badge + theme switcher into the bar
    function grab(sel){
    ...
    grab('.t3-trophy');
    grab('.t3-sync');
    var badge = document.getElementById('t3-auth-badge');
    if (badge) bar.appendChild(badge);
  }

  function fixHoldText(mobile){
    // Find the "HOLD SPACE TO START" hint and swap wording on mobile
    document.querySelectorAll('div,span,p').forEach(function(el){
      if (el.childNodes.length !== 1 || el.childNodes[0].nodeType !== 3) return;
      var t = (el.textContent||'').trim();
      if (/^HOLD SPACE TO START$/i.test(t)){
        el.dataset.t3OrigHold = 'HOLD SPACE TO START';
        if (mobile) el.textContent = 'TAP & HOLD TO START';
      } else if (/^TAP & HOLD TO START$/i.test(t) && !mobile && el.dataset.t3OrigHold){
        el.textContent = el.dataset.t3OrigHold;
      }
    });
  }

  function enableTouchTimer(){
    if (window.__t3TouchTimer) return;
    window.__t3TouchTimer = true;
    // Translate touch on the timer area into SPACE keydown/keyup so Odoo's timer engine reacts.
    function timerZone(){
      return document.getElementById('t3-timer') ||
             document.querySelector('.t3-timer-wrap') ||
             document.querySelector('#t3den-app');
    }
    function fireSpace(type){
      var ev = new KeyboardEvent(type, {key:' ', code:'Space', keyCode:32, which:32, bubbles:true, cancelable:true});
    ...
    document.addEventListener('touchstart', function(e){
      var z = timerZone();
      if (!z) return;
      // Ignore touches on buttons/inputs/links/sidebar/menus
      if (e.target.closest('button,a,select,input,textarea,#t3-mobile-toolbar,#t3-mobile-puzzle,.t3-auth-overlay,#t3-nav-menu,#t3-tools-menu')) return;
      var rect = z.getBoundingClientRect();
      // Only the central timer band counts
      var t = e.touches[0];
      if (t && t.clientY > 100 && t.clientY < window.innerHeight - 120){
        fireSpace('keydown');
      }
    }, {passive:true});
  ...
  function purgeBogusSolves(){
    // Strip any saved solve with t < 1000ms (=1s). Odoo timer stores t in milliseconds.
    // Anything under 1 second is ghost/Gan-ST garbage that no human can solve in.
    try {
      var data = JSON.parse(localStorage.getItem(STORAGE_KEY)||'{}');
      if (!data.sessions) return;
      var removed = 0;
      Object.keys(data.sessions).forEach(function(p){
        var before = data.sessions[p].length;
        data.sessions[p] = data.sessions[p].filter(function(s){ return s && typeof s.t === 'number' && s.t >= 1000; });
        removed += before - data.sessions[p].length;
      });
      if (removed > 0){
        localStorage.setItem(STORAGE_KEY, JSON.stringify(data));
        console.log('[3DEN] Purged ' + removed + ' bogus sub-1s solves');
      }
    } catch(e){}
  }
  ...
  function init(){
    ...
    // Dismiss the loading splash once the 3DEN UI is set up
    // (kept longer so the progress bar finishes — feels intentional, like a boot screen)
    setTimeout(removeSplash, 2600);
    // Failsafe: never let splash linger
    setTimeout(removeSplash, 7000);
    // Mobile layout: dropdown puzzle picker, toolbar, touch-to-start
    setupMobileLayout();
    setTimeout(setupMobileLayout, 800);
    setTimeout(setupMobileLayout, 2000);
    var __t3rsz;
    window.addEventListener('resize', function(){
      clearTimeout(__t3rsz);
      __t3rsz = setTimeout(setupMobileLayout, 200);
    });
    injectThemeSwitcher();
    setTimeout(injectThemeSwitcher, 500);
    setTimeout(injectThemeSwitcher, 1500);
    setTimeout(injectThemeSwitcher, 3000);
  ...
  /* ===========================================================
   * MEGA UPDATE: sound, highlight, drag, badges, race, compare,
   * daily-goal, sub-X tracker, dance cube, particles, predict.
   * =========================================================== */

  /* ---- SOUND EFFECTS ---- */
  var SOUND_ENABLED = (localStorage.getItem('t3den_sound') !== '0');  // default ON
  function setSoundEnabled(on){
    SOUND_ENABLED = !!on;
    localStorage.setItem('t3den_sound', on ? '1' : '0');
  }
  ...
  // Hook running timer: tick on key seconds during a live solve
  function hookSolveSounds(){
    // DISABLED — the 8s/12s/15s beeps were meant for WCA inspection countdown,
    // not for the solve timer itself. Removed at Eden's request; nothing plays
    // during a live solve now. Inspection sounds are handled elsewhere.
    return;
  }

  /* ---- BEST / WORST HIGHLIGHT ---- */
  // Reuses MutationObserver; once per render we mark fastest = gold border, slowest = soft red
  function highlightBestWorst(){
    var list = document.querySelector('.t3-times-list');
    if (!list) return;
    var rows = list.querySelectorAll('.t3-times-row');
    ...
    var best = Math.min.apply(null, validTimes);
    var worst = Math.max.apply(null, validTimes);
    rows.forEach(function(row){
      ...
      var t = s.t + ((s.pen||0)===1?2000:0);
      if (t === best) row.style.borderLeft = '3px solid #fbbf24';
      else if (t === worst) row.style.borderLeft = '3px solid rgba(239,68,68,.5)';
      else row.style.borderLeft = '';
    });
  }
  setInterval(highlightBestWorst, 800);

  /* ---- DRAG & DROP puzzles in sidebar ---- */
  // Persist order in localStorage, reorder on load
  function getPuzzleOrder(){
    try { return JSON.parse(localStorage.getItem('t3den_puzzle_order')||'null'); } catch(e){ return null; }
  }
  function setPuzzleOrder(arr){
    localStorage.setItem('t3den_puzzle_order', JSON.stringify(arr));
  }
  function installDragDropPuzzles(){
    if (window.__t3DragDropPuzzles) return;
    var sidebar = document.getElementById('t3-puzzles');
    if (!sidebar) { setTimeout(installDragDropPuzzles, 400); return; }
    window.__t3DragDropPuzzles = true;
    var btns = Array.from(sidebar.querySelectorAll('button'));
    // Apply saved order
    var saved = getPuzzleOrder();
    if (saved){
      var byLabel = {};
      btns.forEach(function(b){ byLabel[(b.textContent||'').trim()] = b; });
      saved.forEach(function(lbl){
        if (byLabel[lbl]) sidebar.appendChild(byLabel[lbl]);
      });
      // append leftover not in saved order (e.g. new puzzles added later)
      btns.forEach(function(b){
        if (saved.indexOf((b.textContent||'').trim()) === -1) sidebar.appendChild(b);
      });
      // re-append custom-sessions wrap at bottom
      var wrap = document.getElementById('t3cs-wrap');
      if (wrap) sidebar.appendChild(wrap);
    }
    // Enable drag
    Array.from(sidebar.querySelectorAll('button')).forEach(function(b){
      if (b.__t3DragBound) return;
      b.__t3DragBound = true;
      b.draggable = true;
      b.addEventListener('dragstart', function(e){
        e.dataTransfer.setData('text/plain', (b.textContent||'').trim());
        b.style.opacity = '.4';
      });
      b.addEventListener('dragend', function(){ b.style.opacity = ''; saveCurrentOrder(); });
      b.addEventListener('dragover', function(e){
        e.preventDefault();
        var rect = b.getBoundingClientRect();
        var below = e.clientY > rect.top + rect.height/2;
        b.style.borderTop = below ? '' : '2px solid #fbbf24';
        b.style.borderBottom = below ? '2px solid #fbbf24' : '';
      });
      b.addEventListener('dragleave', function(){ b.style.borderTop=''; b.style.borderBottom=''; });
      b.addEventListener('drop', function(e){
        e.preventDefault();
        b.style.borderTop=''; b.style.borderBottom='';
        var dragged = e.dataTransfer.getData('text/plain');
        var draggedEl = Array.from(sidebar.querySelectorAll('button')).find(function(x){ return (x.textContent||'').trim() === dragged; });
        if (!draggedEl || draggedEl === b) return;
        var rect = b.getBoundingClientRect();
        var below = e.clientY > rect.top + rect.height/2;
        sidebar.insertBefore(draggedEl, below ? b.nextSibling : b);
        saveCurrentOrder();
      });
    });
  }
  function saveCurrentOrder(){
    var sidebar = document.getElementById('t3-puzzles');
    if (!sidebar) return;
    var order = Array.from(sidebar.querySelectorAll('button')).map(function(b){ return (b.textContent||'').trim(); }).filter(Boolean);
    setPuzzleOrder(order);
  }

  /* ---- STREAK GOALS + BADGES ---- */
  // Badges live in t3den_badges; computed from total solves + streak + PBs
  var BADGE_DEFS = [
    {id:'first', name:'First Solve', emoji:'🎯', condition:function(stats){ return stats.totalSolves >= 1; }},
    {id:'ten', name:'10 Solves', emoji:'🔟', condition:function(stats){ return stats.totalSolves >= 10; }},
    {id:'hundred', name:'100 Solves', emoji:'💯', condition:function(stats){ return stats.totalSolves >= 100; }},
    {id:'k', name:'1000 Solves', emoji:'🏛', condition:function(stats){ return stats.totalSolves >= 1000; }},
    {id:'streak7', name:'7-Day Streak', emoji:'🔥', condition:function(stats){ return stats.streak >= 7; }},
    {id:'streak30', name:'30-Day Streak', emoji:'🌋', condition:function(stats){ return stats.streak >= 30; }},
    {id:'streak100', name:'100-Day Streak', emoji:'☄', condition:function(stats){ return stats.streak >= 100; }},
    {id:'sub30', name:'Sub-30', emoji:'⚡', condition:function(stats){ return stats.bestSingle333 && stats.bestSingle333 < 30000; }},
    {id:'sub20', name:'Sub-20', emoji:'🚀', condition:function(stats){ return stats.bestSingle333 && stats.bestSingle333 < 20000; }},
    {id:'sub15', name:'Sub-15', emoji:'🏆', condition:function(stats){ return stats.bestSingle333 && stats.bestSingle333 < 15000; }},
    {id:'sub10', name:'Sub-10', emoji:'🦅', condition:function(stats){ return stats.bestSingle333 && stats.bestSingle333 < 10000; }},
    {id:'allpuzzles', name:'Polyglot', emoji:'🎨', condition:function(stats){ return stats.puzzlesTried >= 5; }}
  ];
  function computeBadgeStats(){
    ...
    var puzzlesTried = 0;
    Object.keys(sessions).forEach(function(p){
      if ((sessions[p]||[]).length > 0) puzzlesTried++;
      totalSolves += (sessions[p]||[]).length;
    });
    var arr333 = sessions['333']||[];
    var validBest = arr333.filter(function(s){ return (s.pen||0)!==2; }).map(function(s){ return s.t + ((s.pen||0)===1?2000:0); });
    var bestSingle333 = validBest.length ? Math.min.apply(null, validBest) : null;
    var streak = computeStreak().streak;
    return {totalSolves:totalSolves, puzzlesTried:puzzlesTried, bestSingle333:bestSingle333, streak:streak};
  }
  function getEarnedBadges(){
    var stats = computeBadgeStats();
    return BADGE_DEFS.filter(function(b){ return b.condition(stats); });
  }
  function showBadgesModal(){
    ...
    var html = BADGE_DEFS.map(function(b){
      var got = earnedIds.indexOf(b.id) >= 0;
      return '<div style="display:flex;align-items:center;gap:14px;padding:14px;background:'+(got?'rgba(251,191,36,.08)':'rgba(255,255,255,.02)')+';border:1px solid '+(got?'rgba(251,191,36,.3)':'rgba(255,255,255,.05)')+';border-radius:12px">'
        + '<div style="font-size:32px;opacity:'+(got?'1':'.25')+';filter:'+(got?'none':'grayscale(1)')+'">'+b.emoji+'</div>'
        + '<div style="flex:1">'
          + '<div style="color:'+(got?'#fff':'#64748b')+';font-weight:600;font-size:14px">'+b.name+'</div>'
          + '<div style="color:'+(got?'#fbbf24':'#475569')+';font-size:11px;margin-top:2px">'+(got?'✓ Earned':'Locked')+'</div>'
        + '</div>'
      + '</div>';
    }).join('');
    var ov = document.createElement('div');
    ov.className = 't3-auth-overlay';
    ov.innerHTML = '<div class="t3-auth-modal" style="max-width:540px;width:96vw;padding:24px">'
      + '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:18px">'
        + '<h3 style="color:#fff;margin:0;font-size:20px">🎖 Badges <span style="color:#64748b;font-size:13px;font-weight:400">'+earned.length+'/'+BADGE_DEFS.length+'</span></h3>'
        + '<button id="t3bg-x" style="background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.08);color:#cbd5e1;width:30px;height:30px;border-radius:7px;cursor:pointer">✕</button>'
      + '</div>'
      + '<div style="display:grid;gap:8px">' + html + '</div>'
    + '</div>';
    document.body.appendChild(ov);
    ov.querySelector('#t3bg-x').onclick = function(){ ov.remove(); };
    ov.onclick = function(e){ if (e.target === ov) ov.remove(); };
  }

  /* ---- CUBE RACE MODE: target time tracker ---- */
  function getRaceTarget(){
    var v = localStorage.getItem('t3den_race_target_'+getActivePuzzle());
    return v ? parseInt(v,10) : null;
  }
  function setRaceTarget(ms){
    if (ms) localStorage.setItem('t3den_race_target_'+getActivePuzzle(), String(ms));
    else localStorage.removeItem('t3den_race_target_'+getActivePuzzle());
  }
  function showRaceModal(){
    var puzzle = getActivePuzzle();
    var cur = getRaceTarget();
    var arr = getSolvesForPuzzle(puzzle);
    var last4 = arr.slice(-4).filter(function(s){return (s.pen||0)!==2;}).map(function(s){return s.t + ((s.pen||0)===1?2000:0);});
    var advice = '';
    if (cur && last4.length === 4){
      // What time on 5th solve to get AO5 < target?
      // AO5 = trim worst+best of 5, average middle 3. We need avg(middle3) < target.
      var sorted = last4.slice().sort(function(a,b){return a-b;});
      var newSorted = sorted.slice();
      // Hard math: try various 5th times until AO5 < target
      // Or simpler: required time so that excluding best+worst of 5, avg < target * 3
      // Best approach: brute force test = 1ms to 60s in steps of 100ms
      var found = null;
      for (var t = 100; t < 60000; t += 50){
        var five = last4.concat([t]).sort(function(a,b){return a-b;});
        var trimmed = five.slice(1,4);
        var avg = (trimmed[0]+trimmed[1]+trimmed[2])/3;
        if (avg < cur){ found = t; break; }
      }
      if (found){
        advice = 'Next solve must be < <b style="color:#fbbf24">'+(found/1000).toFixed(2)+'s</b> to get AO5 below target.';
      } else {
        advice = 'Cannot reach target on next solve alone — keep going!';
      }
    } else if (cur && last4.length < 4){
      advice = 'Do '+ (4-last4.length) + ' more valid solves to start receiving live coaching.';
    }
    var ov = document.createElement('div');
    ov.className = 't3-auth-overlay';
    ov.innerHTML = '<div class="t3-auth-modal" style="max-width:440px;padding:24px">'
      + '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px">'
        + '<h3 style="color:#fff;margin:0;font-size:18px">🏁 Cube Race</h3>'
        + '<button id="t3rc-x" style="background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.08);color:#cbd5e1;width:30px;height:30px;border-radius:7px;cursor:pointer">✕</button>'
      + '</div>'
      + '<div style="color:#64748b;font-size:12px;margin-bottom:12px">Set a target time for '+puzzle+' (e.g. 15s for sub-15).</div>'
      + '<label style="display:block;color:#cbd5e1;font-size:12px;margin-bottom:6px">Target time (seconds)</label>'
      + '<input id="t3rc-target" type="number" step="0.01" placeholder="15.00" value="'+(cur ? (cur/1000).toFixed(2) : '')+'" style="width:100%;background:rgba(0,0,0,.3);border:1px solid rgba(255,255,255,.1);border-radius:8px;padding:10px;color:#fff;font-size:18px;font-family:monospace;text-align:center;box-sizing:border-box"/>'
      + (advice ? '<div style="background:rgba(34,197,94,.05);border:1px solid rgba(34,197,94,.2);border-radius:10px;padding:12px;margin-top:14px;color:#cbd5e1;font-size:13px;line-height:1.5">'+advice+'</div>' : '')
      + '<div style="display:flex;justify-content:space-between;gap:8px;margin-top:18px">'
        + '<button id="t3rc-clear" style="background:rgba(239,68,68,.1);border:1px solid rgba(239,68,68,.2);color:#fca5a5;padding:8px 14px;border-radius:8px;cursor:pointer;font-size:12px">Clear target</button>'
        + '<button id="t3rc-save" style="background:#fbbf24;border:0;color:#0a0e1a;padding:9px 22px;border-radius:8px;cursor:pointer;font-size:13px;font-weight:700">Save</button>'
      + '</div>'
    + '</div>';
    document.body.appendChild(ov);
    ov.querySelector('#t3rc-x').onclick = function(){ ov.remove(); };
    ov.querySelector('#t3rc-clear').onclick = function(){ setRaceTarget(null); ov.remove(); };
    ov.querySelector('#t3rc-save').onclick = function(){
      var v = parseFloat(ov.querySelector('#t3rc-target').value);
      if (!isNaN(v) && v > 0) setRaceTarget(Math.round(v*1000));
      ov.remove();
    };
    ov.onclick = function(e){ if (e.target === ov) ov.remove(); };
  }
  ...
  function showCompareModal(){
    ...
    var arr = getSolvesForPuzzle(puzzle);
    if (arr.length < 5){
      alert('Need at least 5 solves on '+puzzle+' to compare.');
      return;
    }
    var now = Date.now();
    var oneDay = 86400000;
    function rangeStats(fromTs, toTs){
      var slice = arr.filter(function(s){ return s.d >= fromTs && s.d < toTs && (s.pen||0)!==2; });
      var times = slice.map(function(s){ return s.t + ((s.pen||0)===1?2000:0); });
      if (!times.length) return null;
      var best = Math.min.apply(null, times);
      var avg = times.reduce(function(a,b){return a+b;}, 0) / times.length;
      var ao5 = times.length >= 5 ? computeAOX(slice, 5) : null;
      return {count: times.length, best: best, avg: avg, ao5: ao5};
    }
    var today = rangeStats(now - oneDay, now);
    var week = rangeStats(now - 7*oneDay, now - oneDay);
    var month = rangeStats(now - 30*oneDay, now - 7*oneDay);
    ...
    function row(label, period){
      if (!period) return '<tr><td style="padding:10px;color:#cbd5e1">'+label+'</td><td colspan="3" style="padding:10px;color:#475569">No data</td></tr>';
      return '<tr><td style="padding:10px;color:#cbd5e1">'+label+'</td>'
        + '<td style="padding:10px;text-align:right;color:#fbbf24;font-family:monospace">'+fmt(period.best)+'</td>'
        + '<td style="padding:10px;text-align:right;color:#fff;font-family:monospace">'+fmt(period.avg)+'</td>'
        + '<td style="padding:10px;text-align:right;color:#cbd5e1;font-size:12px">'+period.count+'</td></tr>';
    }
    var ov = document.createElement('div');
    ov.className = 't3-auth-overlay';
    ov.innerHTML = '<div class="t3-auth-modal" style="max-width:520px;padding:24px">'
      + '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:18px">'
        + '<h3 style="color:#fff;margin:0;font-size:18px">📊 Session Compare — '+puzzle+'</h3>'
        + '<button id="t3cp-x" style="background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.08);color:#cbd5e1;width:30px;height:30px;border-radius:7px;cursor:pointer">✕</button>'
      + '</div>'
      + '<table style="width:100%;border-collapse:collapse;font-size:14px"><thead>'
        + '<tr style="border-bottom:1px solid rgba(255,255,255,.1)">'
        + '<th style="text-align:left;padding:10px;color:#94a3b8">Period</th>'
        + '<th style="text-align:right;padding:10px;color:#94a3b8">Best</th>'
        + '<th style="text-align:right;padding:10px;color:#94a3b8">Avg</th>'
        + '<th style="text-align:right;padding:10px;color:#94a3b8">#</th></tr></thead><tbody>'
        + row('Today',  today)
        + row('Last 7d', week)
        + row('30d ago', month)
      + '</tbody></table>'
    + '</div>';
    document.body.appendChild(ov);
    ov.querySelector('#t3cp-x').onclick = function(){ ov.remove(); };
    ov.onclick = function(e){ if (e.target === ov) ov.remove(); };
  }

  /* ---- DAILY GOAL ---- */
  function getDailyGoal(){ return parseInt(localStorage.getItem('t3den_daily_goal')||'20', 10); }
  function setDailyGoal(n){ localStorage.setItem('t3den_daily_goal', String(n)); }
  function getTodaySolveCount(){
    ...
    Object.keys(sessions).forEach(function(p){
      (sessions[p]||[]).forEach(function(s){
        if (s.d && s.d >= today.getTime()) count++;
      });
    });
    return count;
  }
  ...
  /* ---- SUB-X TRACKER ---- */
  function showSubXModal(){
    var puzzle = getActivePuzzle();
    var arr = getSolvesForPuzzle(puzzle);
    var target = getRaceTarget();
    if (!target){
      alert('Set a Cube Race target first (🏁 button).');
      return;
    }
    var sub = arr.filter(function(s){
      if ((s.pen||0)===2) return false;
      var t = s.t + ((s.pen||0)===1?2000:0);
      return t < target;
    });
    var pct = arr.length ? (sub.length / arr.length * 100).toFixed(1) : 0;
    // Best running streak of sub-X
    var running = 0, longest = 0;
    arr.forEach(function(s){
      if ((s.pen||0)===2) { running = 0; return; }
      var t = s.t + ((s.pen||0)===1?2000:0);
      if (t < target){ running++; if (running > longest) longest = running; }
      else running = 0;
    });
    function fmt(ms){
      var s = ms/1000;
    ...
    var ov = document.createElement('div');
    ov.className = 't3-auth-overlay';
    ov.innerHTML = '<div class="t3-auth-modal" style="max-width:440px;padding:24px;text-align:center">'
      + '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:18px">'
        + '<h3 style="color:#fff;margin:0;font-size:18px">🎯 Sub-'+fmt(target)+' Tracker</h3>'
        + '<button id="t3sx-x" style="background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.08);color:#cbd5e1;width:30px;height:30px;border-radius:7px;cursor:pointer">✕</button>'
      + '</div>'
      + '<div style="background:linear-gradient(135deg,rgba(34,197,94,.15),rgba(34,197,94,.05));border:1px solid rgba(34,197,94,.3);border-radius:14px;padding:24px;margin-bottom:14px">'
        + '<div style="font-size:42px;font-weight:800;color:#22c55e;font-family:monospace">'+sub.length+'</div>'
        + '<div style="color:#cbd5e1;font-size:12px;text-transform:uppercase;letter-spacing:1px;margin-top:4px">Sub-'+fmt(target)+' solves</div>'
        + '<div style="color:#94a3b8;font-size:11px;margin-top:8px">'+pct+'% of '+arr.length+' total</div>'
      + '</div>'
      + '<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">'
        + '<div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.06);border-radius:10px;padding:14px"><div style="font-size:22px;font-weight:800;color:#fbbf24">'+longest+'</div><div style="color:#64748b;font-size:11px;text-transform:uppercase;margin-top:2px">Best streak</div></div>'
        + '<div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.06);border-radius:10px;padding:14px"><div style="font-size:22px;font-weight:800;color:#fb923c">'+running+'</div><div style="color:#64748b;font-size:11px;text-transform:uppercase;margin-top:2px">Current streak</div></div>'
      + '</div>'
    + '</div>';
    document.body.appendChild(ov);
    ov.querySelector('#t3sx-x').onclick = function(){ ov.remove(); };
    ov.onclick = function(e){ if (e.target === ov) ov.remove(); };
  }

  /* ---- DANCE CUBE on PB + PARTICLE EFFECT during solve ---- */
  function spawnDanceCube(){
    var d = document.createElement('div');
    d.textContent = '🧊';
    d.style.cssText = 'position:fixed;left:50%;bottom:20%;transform:translateX(-50%);font-size:120px;z-index:99999;pointer-events:none;animation:t3-dance-cube 2s ease-out forwards';
    document.body.appendChild(d);
    setTimeout(function(){ d.remove(); }, 2100);
  }

  function installParticleEffect(){
    if (window.__t3ParticleHook) return;
    window.__t3ParticleHook = true;
    var active = false;
    document.addEventListener('mousemove', function(e){
      if (!active) return;
      if (Math.random() > 0.5) return;  // throttle
      var p = document.createElement('div');
      var colors = ['#fbbf24','#22c55e','#3b82f6','#ef4444','#a855f7'];
      var color = colors[Math.floor(Math.random()*colors.length)];
      p.style.cssText = 'position:fixed;left:'+e.clientX+'px;top:'+e.clientY+'px;width:6px;height:6px;background:'+color+';border-radius:50%;pointer-events:none;z-index:99998;animation:t3-particle 0.8s ease-out forwards';
      document.body.appendChild(p);
      setTimeout(function(){ p.remove(); }, 900);
    });
    // Toggle by watching timer state
    setInterval(function(){
      var t = document.getElementById('t3-timer');
      if (!t) return;
      var txt = (t.textContent||'').trim();
      var running = /^\d+(\.\d+)?$|^\d+:\d+/.test(txt) && txt !== '0.00';
      var isWhite = t.style.color === '' || t.style.color === 'rgb(255, 255, 255)' || t.style.color === '#fff';
      active = running && isWhite && txt.indexOf('.') >= 0;
    }, 100);
  }

  /* ---- PREDICT YOUR TIME mini-game ---- */
  var PREDICT_XP = parseInt(localStorage.getItem('t3den_predict_xp')||'0', 10);
  function addPredictXP(n){
    PREDICT_XP += n;
    localStorage.setItem('t3den_predict_xp', String(PREDICT_XP));
  }
  function openPredictModal(){
    var ov = document.createElement('div');
    ov.className = 't3-auth-overlay';
    ov.id = 't3pr-overlay';
    ov.innerHTML = '<div class="t3-auth-modal" style="max-width:380px;padding:22px;text-align:center">'
      + '<h3 style="color:#fff;margin:0 0 8px;font-size:18px">🔮 Predict your time</h3>'
      + '<div style="color:#64748b;font-size:12px;margin-bottom:16px">±2.00s = +20 XP · ±1.00s = +50 XP · ±0.50s = +100 XP</div>'
      + '<input id="t3pr-input" type="number" step="0.01" placeholder="12.50" style="width:100%;background:rgba(0,0,0,.3);border:1px solid rgba(255,255,255,.1);border-radius:10px;padding:14px;color:#fff;font-size:24px;font-family:monospace;text-align:center;box-sizing:border-box"/>'
      + '<div style="display:flex;justify-content:space-between;align-items:center;margin-top:14px">'
        + '<span style="color:#fbbf24;font-size:13px">XP: <b>'+PREDICT_XP+'</b></span>'
        + '<button id="t3pr-save" style="background:#fbbf24;border:0;color:#0a0e1a;padding:8px 18px;border-radius:8px;cursor:pointer;font-size:13px;font-weight:700">Lock in</button>'
      + '</div>'
    + '</div>';
    document.body.appendChild(ov);
    var input = ov.querySelector('#t3pr-input');
    input.focus();
    function lockIn(){
      var v = parseFloat(input.value);
      if (isNaN(v) || v <= 0) return;
      window.__t3PendingPrediction = Math.round(v * 1000);
      ov.remove();
    }
    ov.querySelector('#t3pr-save').onclick = lockIn;
    ov.onclick = function(e){ if (e.target === ov) ov.remove(); };
    document.addEventListener('keydown', function esc(e){
      if (e.key === 'Escape'){ ov.remove(); document.removeEventListener('keydown', esc); }
      else if (e.key === 'Enter'){ lockIn(); document.removeEventListener('keydown', esc); }
    });
  }
  // Hook: when a new solve lands, if we had a prediction, score it
  function scorePredictionFromSolve(solve){
    if (!window.__t3PendingPrediction) return;
    if ((solve.pen||0) === 2){ window.__t3PendingPrediction = null; return; }
    var actual = solve.t + ((solve.pen||0)===1?2000:0);
    var diff = Math.abs(actual - window.__t3PendingPrediction) / 1000;
    var xp = 0;
    if (diff <= 0.5) xp = 100;
    else if (diff <= 1) xp = 50;
    else if (diff <= 2) xp = 20;
    if (xp){
      addPredictXP(xp);
      beep(880, 0.15, 0.18);
      showCelebrationBanner('🔮 +'+xp+' XP', 'Δ '+diff.toFixed(2)+'s · Total: '+PREDICT_XP, '#a855f7');
    }
    window.__t3PendingPrediction = null;
  }

  /* ---- TOOLBAR ICON: 🎮 single menu (collapses 6 tools into popup) ---- */
  function injectMegaIcons(){
    if (window.__t3MegaIcons) return;
    if (!document.body) { setTimeout(injectMegaIcons, 300); return; }
    window.__t3MegaIcons = true;
    var btn = document.createElement('div');
    btn.id = 't3-tools-icon';
    btn.className = 't3-trophy';
    btn.title = 'Tools: Race / Badges / Compare / Sub-X / Predict / Sound';
    btn.style.right = '164px';
  ...
  function toggleToolsMenu(anchor){
    var existing = document.getElementById('t3-tools-menu');
    if (existing){ existing.remove(); return; }
    var rect = anchor.getBoundingClientRect();
    var menu = document.createElement('div');
    ...
    function item(emoji, label, click, color){
      ...
      i.onmouseenter = function(){ i.style.setProperty('background','rgba(255,255,255,.08)','important'); i.style.setProperty('background-color','rgba(255,255,255,.08)','important'); };
      i.onmouseleave = function(){ i.style.setProperty('background','transparent','important'); i.style.setProperty('background-color','transparent','important'); };
      i.innerHTML = '<span style="font-size:18px;width:24px;text-align:center;color:'+(color||'#cbd5e1')+'">'+emoji+'</span><span style="color:#e2e8f0 !important">'+label+'</span>';
      i.onclick = function(){ menu.remove(); click(); };
      return i;
    }
    menu.appendChild(item('🏁', 'Cube Race target', showRaceModal, '#22c55e'));
    menu.appendChild(item('🎖', 'Badges & Goals', showBadgesModal, '#fbbf24'));
    menu.appendChild(item('📊', 'Session Compare', showCompareModal, '#3b82f6'));
    menu.appendChild(item('🎯', 'Sub-X Tracker', showSubXModal, '#a855f7'));
    menu.appendChild(item('🔮', 'Predict your time', openPredictModal, '#ec4899'));
    menu.appendChild(item(SOUND_ENABLED?'🔊':'🔇', SOUND_ENABLED?'Sound: ON':'Sound: OFF', function(){
  ...
  /* ---- HOOK: trigger dance cube on PB, score predictions ---- */
  var __origCelebratePB = celebratePB;
  celebratePB = function(type, time, puzzle){
    spawnDanceCube();
    __origCelebratePB(type, time, puzzle);
  };
  var __origDiffSolves = diffSolves;
  diffSolves = function(prev, curr){
    var out = __origDiffSolves(prev, curr);
    if (out && out.length){
      // score prediction against newest solve
      var newest = out[out.length-1];
      if (newest && newest.solve) scorePredictionFromSolve(newest.solve);
    }
    return out;
  };

  /* ---- ANIMATION KEYFRAMES (append to existing CSS) ---- */
  function injectMegaKeyframes(){
    if (document.getElementById('t3-mega-keyframes')) return;
    var st = document.createElement('style');
    st.id = 't3-mega-keyframes';
    st.textContent = '@keyframes t3-toolsmenu-in{0%{opacity:0;transform:translateY(-8px) scale(.96)}100%{opacity:1;transform:translateY(0) scale(1)}}'
      + '@keyframes t3-dance-cube{0%{transform:translateX(-50%) translateY(0) rotate(0) scale(.5);opacity:0}10%{opacity:1}30%{transform:translateX(-50%) translateY(-30px) rotate(20deg) scale(1.1)}50%{transform:translateX(-50%) translateY(-60px) rotate(-20deg) scale(1.2)}70%{transform:translateX(-50%) translateY(-30px) rotate(15deg) scale(1.1)}100%{transform:translateX(-50%) translateY(0) rotate(0) scale(1);opacity:0}}'
      + '@keyframes t3-particle{0%{opacity:1;transform:translate(0,0) scale(1)}100%{opacity:0;transform:translate('+'calc(-50% + '+'-20px),'+'-40px) scale(.3)}}'
      + '@keyframes t3-particle{0%{opacity:1;transform:translate(0,0) scale(1)}100%{opacity:0;transform:translate(-20px,-40px) scale(.3)}}';
    document.head.appendChild(st);
  }

  /* ---- INIT MEGA UPDATE ---- */
  function initMegaUpdate(){
    injectMegaKeyframes();
    injectMegaIcons();
    hookSolveSounds();
    installParticleEffect();
    installDragDropPuzzles();
    // Re-run drag/drop install when sidebar mutates (custom sessions)
    var sb = document.getElementById('t3-puzzles');
    if (sb){
      new MutationObserver(function(){
        window.__t3DragDropPuzzles = false;
        installDragDropPuzzles();
      }).observe(sb, {childList:true});
    }
  }
  // Run after main init (delay to ensure other features have set up)
  setTimeout(initMegaUpdate, 1500);
  setTimeout(initMegaUpdate, 3500);
