...
(function(){
  ...
  function colorMap(c){
    return {U:'#fbbf24', D:'#f8fafc', F:'#22c55e', B:'#3b82f6', L:'#f97316', R:'#ef4444'}[c] || '#374151';
  }
  
  function makePLLSvg(caseId, opts){
    opts = opts || {};
    var size = opts.size || 100;
    var caseData = (PLL_CASES.find(function(x){return x.id===caseId;}));
    if (!caseData){
      var div = document.createElement('div');
    ...
    var cube = newCube();
    cube = applyScramble(cube, caseData.setup);
    ...
    svg.style.cssText = 'display:block;background:#3a3a3a;border-radius:8px';
    
    // Layout: 5x5 grid where cells [1..3][1..3] are U face
    // Top row [1..3] = B face (closest to U, viewed from above)
    // Bottom row [1..3] = F face
    // Left col [1..3] = L face
    // Right col [1..3] = R face
    // Corners are dark gray
    
    var pad = 4;        // outer padding
    var stickerGap = 1.5; // gap between stickers
    var cell = (100 - 2*pad) / 5; // ~18.4
    var s = cell - stickerGap;
    var rx = 2; // sticker corner radius
    ...
    for (var i=0; i<3; i++){
      for (var j=0; j<3; j++){
        rect(pad + (1+j)*cell + stickerGap/2, pad + (1+i)*cell + stickerGap/2, colorMap(cube.U[i*3+j]));
      }
    }
    // B face top row visible (closest to U) - indices [2,1,0] left-to-right
    for (var i=0; i<3; i++){
      rect(pad + (1+i)*cell + stickerGap/2, pad + 0*cell + stickerGap/2, colorMap(cube.B[2-i]));
    }
    // F face top row (closest to U)
    for (var i=0; i<3; i++){
      rect(pad + (1+i)*cell + stickerGap/2, pad + 4*cell + stickerGap/2, colorMap(cube.F[i]));
    }
    // L face top row visible - top to bottom = indices [0,1,2]
    for (var i=0; i<3; i++){
      rect(pad + 0*cell + stickerGap/2, pad + (1+i)*cell + stickerGap/2, colorMap(cube.L[i]));
    }
    // R face top row visible - top to bottom = indices [2,1,0] (mirrored)
    for (var i=0; i<3; i++){
      rect(pad + 4*cell + stickerGap/2, pad + (1+i)*cell + stickerGap/2, colorMap(cube.R[2-i]));
    }
