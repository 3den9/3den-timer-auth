...
(function(){
  ...
  function makePLLSvg(caseId, opts){
    ...
    // Simulate the case by applying setup to a solved cube
    ...
    var pad = cell * 0.5;
    // U face 3x3 from simulated cube
    for (var i=0; i<3; i++){
      for (var j=0; j<3; j++){
        rect(pad + cell + j*cell, pad + cell + i*cell, colorMap(cube.U[i*3+j]));
      }
    }
    // B face top row visible (closest to U) - indices [2,1,0] left-to-right visually
    for (var i=0; i<3; i++){
      rect(pad + cell + i*cell, pad, colorMap(cube.B[2-i]));
    }
    // F face top row (closest to U) - indices [0,1,2] left-to-right
    for (var i=0; i<3; i++){
      rect(pad + cell + i*cell, pad + 4*cell, colorMap(cube.F[i]));
    }
    // L face top row visible - indices [0,1,2] top-to-bottom
    for (var i=0; i<3; i++){
      rect(pad, pad + cell + i*cell, colorMap(cube.L[i]));
    }
    // R face top row - indices [2,1,0] top-to-bottom (mirrored vs L)
    for (var i=0; i<3; i++){
      rect(pad + 4*cell, pad + cell + i*cell, colorMap(cube.R[2-i]));
    }
