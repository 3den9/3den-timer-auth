...
(function(){
  ...
  function installScrambleWatcher(){
    ...
    function maybeReplace(){
      var puzzle = getActivePuzzle();
      // Only override for big cubes where Odoo's generator differs from WCA notation
      if (!SCRAMBLE_LENGTHS[puzzle]) return;
      var current = (scrEl.textContent||'').trim();
      // Detect Odoo's non-WCA format: presence of Dw, Bw, Lw, 3-letter wide for puzzles where it shouldn't be common
      // Or always replace to ensure cstimer-style consistency
      if (current && !current.__t3Marked){
        var fresh = generateWCAScramble(puzzle);
        if (fresh){
          scrEl.textContent = fresh;
          scrEl.dataset.t3Marked = '1';
        }
      }
    }
    // Observer for any text change
    var mo = new MutationObserver(function(){
      // Reset marker so next Odoo change triggers replace
      delete scrEl.dataset.t3Marked;
      maybeReplace();
    });
    mo.observe(scrEl, {childList:true, subtree:true, characterData:true});
    // Initial replace
    setTimeout(maybeReplace, 100);
  }
