...
(function(){
  ...
  function setupQuickNotes(){
    ...
    // Click handler delegated on the times list
    document.addEventListener('click', function(e){
      var row = e.target.closest('.t3-times-row');
      if (!row) return;
      // Don't trigger if clicked on penalty buttons or other actions
      if (e.target.closest('button')) return;
      // Get solve index from row position
      var list = row.parentElement;
      if (!list || !list.classList.contains('t3-times-list')) return;
      // Read solve number from row text (e.g. "5." at start)
      var numMatch = (row.textContent||'').match(/^(\d+)\./);
      if (!numMatch) return;
      var solveNumber = parseInt(numMatch[1], 10);
      openQuickNoteModal(solveNumber);
    });
  ...
  function renderNoteIndicators(){
    ...
    rows.forEach(function(row){
      ...
      // Add tooltip
      row.title = hasNote ? '📝 '+solve.note : 'Click to add a note';
