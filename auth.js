...
(function(){
  ...
  function saveManualSolve(timeMs, penalty, scramble){
    ...
    // Click the active puzzle button — Odoo re-reads localStorage and re-renders the Time List.
    try {
      var sidebar = document.getElementById('t3-puzzles');
      var active = sidebar && (sidebar.querySelector('button.active') ||
        Array.from(sidebar.querySelectorAll('button')).find(function(b){
      ...
      if (active) setTimeout(function(){ active.click(); }, 100);
    } catch(e){}
  }
  ...
  function saveBleSolve(timeMs){
    ...
    try {
      ...
      // Force the Odoo timer to re-read localStorage and re-render the Time List —
      // clicking the currently-active puzzle button is a no-op state change that
      // triggers Odoo's puzzle handler, which reloads solves from localStorage.
      try {
        var sidebar = document.getElementById('t3-puzzles');
        if (sidebar){
