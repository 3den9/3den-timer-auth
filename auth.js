...
(function(){
  ...
  function parseGanTimerData(data){
    // data: DataView with 19 bytes
    // Bytes 0: header (usually 0x00)
    // Bytes 1: state byte (varies)
    // Bytes 2-3: minutes
    // Bytes 4-5: seconds
    // Bytes 6-8: milliseconds
    // Format varies by Gan firmware - we'll handle the common one
    if (!data || data.byteLength < 8) return null;
    
    var bytes = [];
    for (var i = 0; i < data.byteLength; i++) bytes.push(data.getUint8(i));
    
    // Try common Gan Timer parsing:
    // [0]=signature, [1]=state, [2-7]=time digits packed
    var stateByte = bytes[1];
    var state = 'idle';
    if (stateByte === 0 || stateByte === 1) state = 'idle';
    else if (stateByte === 2) state = 'handsOn';      // hands placed
    else if (stateByte === 3) state = 'ready';         // green light
    else if (stateByte === 4) state = 'inspecting';
    else if (stateByte === 5) state = 'running';       // timer running
    else if (stateByte === 6 || stateByte === 8) state = 'finished';
    
    // Time parsing: try multiple formats
    var minutes = bytes[2];
    var seconds = bytes[3];
    var ms = (bytes[4] << 8) | bytes[5];
    var totalMs = minutes * 60000 + seconds * 1000 + ms;
    
    // Sanity check - if totalMs is unreasonable, try alternate parsing
    if (totalMs > 600000 || totalMs < 0){
      // Alternate: BCD encoding
      function bcd(b){ return ((b >> 4) * 10) + (b & 0xf); }
      try {
        minutes = bcd(bytes[2]);
        seconds = bcd(bytes[3]);
        var hundredths = bcd(bytes[4]);
        var thousandths = bcd(bytes[5] || 0);
        totalMs = minutes * 60000 + seconds * 1000 + hundredths * 10 + thousandths;
      } catch(e){}
    }
    
    return {state: state, time_ms: totalMs, raw: bytes};
  }
