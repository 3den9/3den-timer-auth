...
(function(){
  ...
  function onGanNotification(event){
    ...
    // Update timer display via state changes
    ...
    if (timerEl){
      if (parsed.state === 'handsOn'){
        ...
        // Could also live-update from local timing
  ...
  function saveBleSolve(timeMs){
    // Find current scramble + puzzle from Odoo timer
    var scrambleEl = document.getElementById('t3-scramble');
    var scramble = scrambleEl ? scrambleEl.textContent.trim() : '';
    
    // Determine puzzle from active button
    var activePuzzle = '333';
    var puzzles = document.querySelectorAll('#t3-puzzles button');
    puzzles.forEach(function(b){
      if (b.classList.contains('active') || (b.style.background && b.style.background.indexOf('239') >= 0)){
        var label = b.textContent.trim();
        var map = {'3x3':'333','2x2':'222','4x4':'444','5x5':'555','6x6':'666','7x7':'777','3x3 OH':'333oh','3BLD':'333bld','FMC':'333fm','4BLD':'444bld','5BLD':'555bld','MBLD':'333mbf','Pyraminx':'pyram','Skewb':'skewb','Clock':'clock','Megaminx':'minx','Square-1':'sq1'};
        if (map[label]) activePuzzle = map[label];
      }
    });
