...
(function(){
  ...
  async function connectGanTimer(){
    if (!bleSupported()){
      alert('Bluetooth not supported in this browser. Use Chrome, Edge, or Brave.');
      return;
    }
    try {
      ...
      var service = await bleState.server.getPrimaryService(GAN_TIMER_SERVICE);
      bleState.characteristic = await service.getCharacteristic(GAN_TIMER_STATE_CHAR);
      
      await bleState.characteristic.startNotifications();
      bleState.characteristic.addEventListener('characteristicvaluechanged', onGanNotification);
