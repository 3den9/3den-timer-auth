...
(function(){
  ...
  function getActivePuzzle(){
    var nameMap = {'3x3':'333','2x2':'222','4x4':'444','5x5':'555','6x6':'666','7x7':'777',
      '3x3 OH':'333oh','3BLD':'333bld','FMC':'333fm','4BLD':'444bld','5BLD':'555bld','MBLD':'333mbf',
      'Pyraminx':'pyram','Skewb':'skewb','Clock':'clock','Megaminx':'minx','Square-1':'sq1'};
    var btns = document.querySelectorAll('#t3-puzzles button');
    var found = '333';
    btns.forEach(function(b){
      ...
      if (isActive){
        ...
        if (nameMap[label]) found = nameMap[label];
      }
    });
    return found;
  }
  ...
  function saveBleSolve(timeMs){
    ...
    // Determine active puzzle (try several detection strategies)
    var activePuzzle = '333';
    var puzzles = document.querySelectorAll('#t3-puzzles button, .t3-puzzle-btn');
    var nameMap = {
      '3x3':'333','2x2':'222','4x4':'444','5x5':'555','6x6':'666','7x7':'777',
      '3x3 OH':'333oh','3BLD':'333bld','FMC':'333fm','4BLD':'444bld','5BLD':'555bld',
      'MBLD':'333mbf','Pyraminx':'pyram','Skewb':'skewb','Clock':'clock',
      'Megaminx':'minx','Square-1':'sq1'
    };
    var found = false;
    puzzles.forEach(function(b){
      if (found) return;
      var isActive = b.classList.contains('active')
        || b.classList.contains('t3-active')
      ...
      if (isActive){
        var label = (b.textContent || '').trim();
        if (nameMap[label]){ activePuzzle = nameMap[label]; found = true; }
      }
    });
    // Fallback: read from localStorage last selection
    if (!found){
      try {
        var lastP = localStorage.getItem('t3den_active_puzzle');
        if (lastP) activePuzzle = lastP;
      } catch(e){}
    }
  ...
  function hookSolveSounds(){
    if (window.__t3SolveSoundsHooked) return;
    window.__t3SolveSoundsHooked = true;
    var lastSec = -1;
    setInterval(function(){
      if (!SOUND_ENABLED) return;
      var t = document.getElementById('t3-timer');
      ...
      if (!m) { lastSec = -1; return; }
      var sec = m[2] ? parseInt(m[1],10)*60 + parseInt(m[2],10) : parseInt(m[1],10);
      // Only when timer color is white (running)
      if (t.style.color && t.style.color !== 'rgb(255, 255, 255)' && t.style.color !== '#fff') return;
      if (sec !== lastSec){
        if (sec === 8) beep(660, 0.08, 0.10);          // soft tick at 8s
        else if (sec === 12) beep(880, 0.12, 0.12);    // warning at 12s
        else if (sec === 15) beep(1100, 0.15, 0.14);   // serious at 15s
        else if (sec > 0 && sec % 10 === 0 && sec > 15) beep(440, 0.06, 0.08);
        lastSec = sec;
      }
    }, 80);
  }
