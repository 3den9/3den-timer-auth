...
(function(){
  ...
  function setupQuickNotes(){
    ...
    // Press 'Q' to open Quick Note for the most recent solve.
    document.addEventListener('keydown', function(e){
      // Skip if user is typing in a text field or modal is open
      var t = e.target;
      if (t && (t.tagName === 'INPUT' || t.tagName === 'TEXTAREA' || t.isContentEditable)) return;
      if (document.querySelector('.t3-auth-overlay')) return;  // some modal already open
      ...
      if (e.key === 'q' || e.key === 'Q'){
        ...
        e.preventDefault();
        // Most recent solve = highest number in the list = arr[arr.length-1]
        // openQuickNoteModal expects "solve number" as displayed (newest = arr.length)
        openQuickNoteModal(arr.length);
      }
    });
  ...
  function openQuickNoteModal(solveNumber){
    var info = getSolveByNumber(solveNumber);
    if (!info){ console.warn('[QuickNote] solve not found', solveNumber); return; }
    var solve = info.solve;
    var existing = solve.note || '';
    ...
    if ((solve.pen||0) === 2) timeStr = 'DNF';

    var ov = document.createElement('div');
    ov.className = 't3-auth-overlay';
    ov.innerHTML = '<div class="t3-auth-modal" style="max-width:480px">'
      + '<div style="display:flex;align-items:start;justify-content:space-between;margin-bottom:6px">'
      + '<div>'
      + '<h3 style="color:#fff;margin:0;font-size:18px;display:flex;align-items:center;gap:8px">📝 Quick Note</h3>'
      + '<div style="color:#64748b;font-size:13px;margin-top:4px">Solve #'+solveNumber+' · '+timeStr+'</div>'
      + '</div>'
      + '<button id="t3qn-close" style="background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);color:#fff;width:32px;height:32px;border-radius:8px;cursor:pointer;font-size:14px">✕</button>'
      + '</div>'
      + '<textarea id="t3qn-text" maxlength="240" placeholder="What happened on this solve? (e.g. PLL skip, bad cross, locked up on F2L...)" style="width:100%;min-height:90px;background:rgba(0,0,0,.3);border:1px solid rgba(255,255,255,.1);border-radius:10px;padding:12px;color:#fff;font-size:14px;font-family:inherit;resize:vertical;margin-top:14px;box-sizing:border-box"></textarea>'
      + '<div style="display:flex;justify-content:space-between;align-items:center;margin-top:6px">'
      + '<span id="t3qn-counter" style="color:#64748b;font-size:12px">'+existing.length+'/240</span>'
      + '<div style="display:flex;gap:8px">'
      + '<button id="t3qn-cancel" style="background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);color:#cbd5e1;padding:8px 16px;border-radius:8px;cursor:pointer;font-size:13px">Cancel</button>'
      + (existing ? '<button id="t3qn-delete" style="background:rgba(239,68,68,.15);border:1px solid rgba(239,68,68,.3);color:#fca5a5;padding:8px 16px;border-radius:8px;cursor:pointer;font-size:13px">Delete</button>' : '')
      + '<button id="t3qn-save" style="background:#fbbf24;border:1px solid #fbbf24;color:#0a0e1a;padding:8px 18px;border-radius:8px;cursor:pointer;font-size:13px;font-weight:700">Save</button>'
      + '</div>'
      + '</div>'
      + '</div>';
    document.body.appendChild(ov);
    var ta = ov.querySelector('#t3qn-text');
    var counter = ov.querySelector('#t3qn-counter');
    ta.value = existing;
    setTimeout(function(){ ta.focus(); ta.setSelectionRange(ta.value.length, ta.value.length); }, 50);
    ta.addEventListener('input', function(){ counter.textContent = ta.value.length+'/240'; });

    function close(){ ov.remove(); }
    function save(text){
      var data = JSON.parse(localStorage.getItem(STORAGE_KEY)||'{}');
      if (!data.sessions || !data.sessions[info.puzzle]) return;
      var s = data.sessions[info.puzzle][info.index];
      if (!s) return;
      if (text){ s.note = text; }
      else delete s.note;
      // Avoid triggering the diff-as-new-solve sync; mark as edit
      window.__t3SkipNextSync = true;
      localStorage.setItem(STORAGE_KEY, JSON.stringify(data));
      // Sync note to Supabase if column exists
      syncSolveNote(info.puzzle, s, text);
      close();
      if (window.__t3denRender) try { window.__t3denRender(); } catch(e){}
      renderNoteIndicators();
    }

    ov.querySelector('#t3qn-close').onclick = close;
    ov.querySelector('#t3qn-cancel').onclick = close;
    ov.querySelector('#t3qn-save').onclick = function(){ save(ta.value.trim()); };
    var delBtn = ov.querySelector('#t3qn-delete');
    if (delBtn) delBtn.onclick = function(){ save(''); };
    ov.onclick = function(e){ if (e.target === ov) close(); };
    document.addEventListener('keydown', function esc(e){
      if (e.key === 'Escape'){ close(); document.removeEventListener('keydown', esc); }
      else if (e.key === 'Enter' && (e.metaKey || e.ctrlKey)){ save(ta.value.trim()); document.removeEventListener('keydown', esc); }
    });
  }
