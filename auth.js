...
(function(){
  ...
  function saveManualSolve(timeMs, penalty, scramble){
    ...
    // IMPORTANT: the Odoo timer stores solve time in SECONDS (float), not milliseconds.
    var solve = {
      t: timeMs / 1000,
    ...
    // The Odoo timer does not watch localStorage — reload so it re-renders the Time List.
  ...
  function saveBleSolve(timeMs){
    ...
    try {
      ...
      // Odoo timer stores solve time in SECONDS (float), not milliseconds.
      var solve = {
        t: timeMs / 1000,
        s: scramble,
        d: Date.now(),
  ...
  function purgeBogusSolves(){
    // Strip any saved solve with t < 0.5s — those are ghost/Gan-ST garbage that nobody can really solve in.
    try {
      var data = JSON.parse(localStorage.getItem(STORAGE_KEY)||'{}');
      ...
      Object.keys(data.sessions).forEach(function(p){
        ...
        data.sessions[p] = data.sessions[p].filter(function(s){ return s && typeof s.t === 'number' && s.t >= 0.5; });
        removed += before - data.sessions[p].length;
      });
      if (removed > 0){
        localStorage.setItem(STORAGE_KEY, JSON.stringify(data));
        console.log('[3DEN] Purged ' + removed + ' bogus sub-0.5s solves');
      }
    } catch(e){}
  }
