...
(function(){
  ...
  async function connectGanTimer(){
    ...
    try {
      ...
      bleState.device.addEventListener('gattserverdisconnected', onBleDisconnect);
      
      appendBleDebug('Device: ' + bleState.device.name);
      bleState.server = await bleState.device.gatt.connect();
      
      // Discover ALL services and characteristics
      var services = await bleState.server.getPrimaryServices();
      appendBleDebug('Found '+services.length+' services');
      
      var notifyChars = [];
      for (var s = 0; s < services.length; s++){
        var svc = services[s];
        appendBleDebug('Service: ' + svc.uuid);
        try {
          ...
          for (var c = 0; c < chars.length; c++){
            ...
            appendBleDebug('  Char: '+ch.uuid+' ['+props.join(',')+']');
            if (ch.properties.notify || ch.properties.indicate){
              notifyChars.push(ch);
            }
          }
        } catch(e){
          appendBleDebug('  err getting chars: '+e.message);
        }
      }
      
      appendBleDebug('Found '+notifyChars.length+' notify-capable chars');
      
      // Subscribe to ALL notify characteristics
      for (var i = 0; i < notifyChars.length; i++){
        var nc = notifyChars[i];
        try {
          await nc.startNotifications();
          nc.addEventListener('characteristicvaluechanged', onGanNotification);
          appendBleDebug('Subscribed to '+nc.uuid);
          if (!bleState.characteristic) bleState.characteristic = nc;
        } catch(e){
          appendBleDebug('Failed to subscribe to '+nc.uuid+': '+e.message);
        }
      }
      ...
      if (bleState.device.name){
        localStorage.setItem('t3den_ble_lastdevice', bleState.device.name);
      }
      
      console.log('[BLE Gan] Connected to', bleState.device.name);
    } catch(e){
