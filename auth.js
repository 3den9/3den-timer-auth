...
(function(){
  ...
  function saveManualSolve(timeMs, penalty, scramble){
    ...
    // Trigger UI render
    if (window.__t3denRender) try { window.__t3denRender(); } catch(e){}
    // Trigger new scramble
    try {
      var nBtn = Array.from(document.querySelectorAll('button')).find(function(b){ return (b.textContent||'').trim()==='N' || (b.title||'').toLowerCase().indexOf('scramble')>=0; });
      // Or simulate N key
      var ev = new KeyboardEvent('keydown', {key:'n', code:'KeyN', bubbles:true});
  ...
  function onGanNotification(event){
    ...
    if (parsed.state === 'handsOn'){
      ...
      // Transition: ready → idle = hands released, solve started!
      if (prevState === 'ready'){
  ...
  function saveBleSolve(timeMs){
    ...
    try {
      ...
      // Trigger UI render (try every known render hook)
      var rendered = false;
      ['__t3denRender','__t3denRenderAll','__t3denRefresh','t3denRender'].forEach(function(fn){
        if (window[fn]) try { window[fn](); rendered = true; } catch(e){}
      });
      // Fallback: dispatch storage event so listeners react
      if (!rendered){
        try {
          window.dispatchEvent(new StorageEvent('storage', {key:'t3den_v4'}));
        } catch(e){}
      }

      // Trigger new scramble (click the scramble area or call generator)
      try {
        if (window.__t3denNewScramble) window.__t3denNewScramble();
        else {
          var scrEl = document.getElementById('t3-scramble');
          if (scrEl) scrEl.click();
        }
      } catch(e){}
